...
export function useGoLogic(props = {}) {
  const [state, setStateRaw] = useState({
      faqs: [
        { q: 'How to get started?', a: "Check the Quickstart Guide. With a single Docker command you can spin up OLake and access the UI." },
        { q: 'Is OLake really open source?', a: "Yes. OLake is fully open source under the Apache 2.0 license. You can explore the GitHub repository (already starred by 1K+ developers) and use it freely without hidden costs." },
        { q: 'Is there any enterprise plan?', a: "We're actively working on providing enterprise support, from professional assistance and pilot programs to helping teams scale OLake in production. You can reach out at hello@olake.io to learn more." },
        { q: 'How can I contribute?', a: "Join our Slack community, review the Contribution Guide, and explore \"Good First Issues\" on GitHub. Contributors can get their pull requests merged and be part of building the fastest open-source Iceberg-native ingestion tool." },
        { q: 'Why should I use OLake?', a: "OLake makes data replication into Apache Iceberg seamless, faster, and cost-efficient. It handles real-time CDC, schema and partition evolution, full and incremental syncs, and compaction, all without vendor lock-in, so your Iceberg tables stay open, scalable, and ready for analytics." },
        { q: 'What data platforms and tools does OLake integrate with?', a: "As of now, we integrate with Apache Iceberg as a destination. You can query it from most big data platforms like Snowflake, Databricks, Redshift and BigQuery." }
      ],
      openFaq: -1,
      activeFeature: 0,
      progress: 0,
  ...
  const selectSource = (i) => { setState({ activeSource: i }); }

  const buildBenchmarks = () => {
    const G = '#2E9E44', GB = '#EAF6E9';
    const connector = CONNECTORS[stateRef.current.activeSource] || CONNECTORS[0];
    const dataset = stateRef.current.benchmarkMode === 'cdc'
      ? CONNECTOR_CDC_BENCHMARKS
      : CONNECTOR_BENCHMARKS;
    const bench = dataset[connector.id];
    const comps = competitorKeys(bench);
    const cols = [
      { name: 'Metrics', sub: '', hasSub: false, olake: false, color: '#10173A', bg: 'transparent', align: 'flex-start' },
      { name: 'OLake Go', sub: TOOLS.olake.description, hasSub: true, olake: true, color: '#10173A', bg: GB, align: 'center' },
      ...comps.map(key => ({ name: TOOLS[key].name, sub: '', hasSub: false, olake: false, color: '#10173A', bg: 'transparent', align: 'center' }))
    ];
    ...
    const table = BENCHMARK_METRICS.map(metric => {
      ...
      return {
        ...
        sub: metric === 'cost' ? 'OLake is OSS and self-hosted — only pay for your infrastructure.' : '',
        hasSub: metric === 'cost',
  ...
  const selectBenchmarkMode = (mode) => { setState({ benchmarkMode: mode }); }
  ...
  const toggleBenchmarkInfo = () => { setState(s => ({ benchmarksInfoOpen: !s.benchmarksInfoOpen })); }
  ...
  const renderVals = () => {
    ...
    return {
      icebergStyle,
      problemSentences: [
        { top: 22, left: 42, rot: -3, delay: 0, words: sentence(['Slow', 'syncs', 'BLOCK', 'your', 'analytics'], ['Slow', 'syncs', 'BLOCK'], 28) },
        { top: 43, left: 57, rot: 2, delay: 0.6, words: sentence(['Legacy', 'ETL', 'costs', 'PILE UP', 'fast'], ['costs', 'PILE UP'], 24) },
        { top: 64, left: 46, rot: -2, delay: 1.2, words: sentence(['CDC', 'pipelines', 'BREAK', 'silently', 'in', 'production'], ['CDC', 'BREAK'], 25) },
        { top: 85, left: 55, rot: 3, delay: 0.3, words: sentence(['Schema', 'drift', 'stalls', 'ingestion'], ['Schema', 'drift'], 23) }
      ],
      features: [0, 1, 2, 3].map(i => {
        const list = [
          { title: 'FULL, INCREMENTAL & CDC SYNCS', body: 'Run full loads, incremental pulls, or real-time change data capture, whatever each table needs, all from a single tool.', tag: 'tiered' },
          { title: 'SCHEMA & PARTITION EVOLUTION', body: 'Source schemas change and partitions grow. OLake Go evolves your Iceberg tables automatically so pipelines never break.', tag: 'decay' },
          { title: 'PARALLELISED CHUNKING', body: 'Large collections are split into virtual chunks read in parallel, dramatically cutting the time for full snapshots of big datasets.', tag: 'chunk', stats: ['Parallel reads', 'Virtual chunks', 'Faster snapshots'] },
          { title: 'STATEFUL, RESUMABLE SYNCS', body: 'Syncs checkpoint their progress and resume automatically after crashes, network failures, or pauses, never from scratch.', tag: 'resume', stats: ['Checkpointed', 'Auto-resume', 'Fault-tolerant'] }
        ][i];
        const active = stateRef.current.activeFeature === i;
        const builtIn = ['tiered', 'decay', 'config', 'hosted'];
        return { ...list, active, titleColor: active ? '#fff' : '#BFC7F2', progress: active ? stateRef.current.progress : 0,
          isTiered: list.tag === 'tiered', isDecay: list.tag === 'decay', isConfig: list.tag === 'config', isHosted: list.tag === 'hosted',
          isChunk: list.tag === 'chunk', isResume: list.tag === 'resume',
          isGeneric: !builtIn.includes(list.tag), hasStats: Array.isArray(list.stats),
          onSelect: () => selectFeature(i) };
      }),
      benchSources: (() => {
        const act = stateRef.current.activeSource;
        return CONNECTORS.map((connector, i) => ({
          name: connector.name, onSelect: () => selectSource(i),
      ...
      benchModeIndicatorLeft: stateRef.current.benchmarkMode === 'full_load' ? '4px' : 'calc(50% + 0px)',
      benchCols: bm.cols,
      benchTable: bm.table,
      benchComingSoon: bm.comingSoon,
      benchHasData: !bm.comingSoon,
      benchSourceName: bm.sourceName,
      faqs: stateRef.current.faqs.map((f, i) => ({ ...f, open: stateRef.current.openFaq === i, sign: stateRef.current.openFaq === i ? '\u2191' : '\u2193', onToggle: () => toggleFaq(i) })),
  ...
  useEffect(() => {
    ...
    self._drawLinks = () => {
      ...
      srcs.forEach((s, i) => {
        ...
        const d = 'M' + p.x + ',' + p.y + ' C' + (p.x + dx) + ',' + p.y + ' ' + (nodeL.x - dx) + ',' + nodeL.y + ' ' + nodeL.x + ',' + nodeL.y;
        paths += '<path d="' + d + '" fill="none" stroke="url(#gIn)" stroke-width="2"></path>';
        dots += '<circle r="3.5" fill="#3D4FF0"><animateMotion dur="2.4s" begin="' + (i * 0.28) + 's" repeatCount="indefinite" path="' + d + '"></animateMotion></circle>';
      });
      dests.forEach((s, i) => {
        const r = s.getBoundingClientRect();
        const p = { x: r.left - c.left + 8, y: r.top - c.top + r.height / 2 };
        const dx = Math.max(28, (p.x - nodeR.x) * 0.5);
        const d = 'M' + nodeR.x + ',' + nodeR.y + ' C' + (nodeR.x + dx) + ',' + nodeR.y + ' ' + (p.x - dx) + ',' + p.y + ' ' + p.x + ',' + p.y;
        paths += '<path d="' + d + '" fill="none" stroke="url(#gOut)" stroke-width="2"></path>';
        dots += '<circle r="4" fill="#3D4FF0"><animateMotion dur="2.4s" begin="' + (0.6 + i * 0.6) + 's" repeatCount="indefinite" path="' + d + '"></animateMotion></circle>';
      });
