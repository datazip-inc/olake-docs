...
title: The Ultimate Data Lake Face-Off (Databricks vs Apache Iceberg)
description: Benchmarking on Iceberg and Databricks to find out which is faster and cheaper.
...
When we talk about TPC-H benchmarking at 1TB scale, we're not just comparing query engines—we're comparing entire data ecosystems. The real performance story lies in how each architecture handles the complete journey from source data ingestion to final query execution. Let's examine how both architectures orchestrate this complex dance of data movement, storage optimization, and compute execution.

Looking for a deeper understanding of **Apache Iceberg and Delta Lake?** Read our guide: [Apache Iceberg vs Delta Lake: Ultimate Guide for Data Lakes](/blog/apache-iceberg-vs-delta-lake-guide).
...
![Databricks Architecture](/img/blog/2025/10/databricks_arch.png)
...
This is where Databricks shows its architectural sophistication. As TPC-H data lands in Delta Lake, multiple processes work simultaneously:

- **Adaptive File Sizing:** Automatically determines optimal file sizes based on query patterns and cluster configuration.

- **Z-Ordering:** Intelligently co-locates related data within files to minimize I/O during analytical queries.

- **Auto-Compaction:** Runs background processes to optimize file structure as data lands

- **Transaction Log Management:** Maintains ACID properties while enabling concurrent reads during the load process.
...
- **Intelligent Caching:** Hot data remains in memory across query executions, dramatically reducing I/O
...
![Iceberg Architecture](/img/blog/2025/10/olake_architecture.png)
...
Unlike proprietary storage formats, Iceberg leverages standard cloud storage (S3) with sophisticated metadata management:

- **Hidden Partitioning:** Data is automatically partitioned based on query patterns without exposing complexity to users

- **File-Level Statistics:** Min/max values, null counts, and record counts enable aggressive query pruning

- **Manifest Files:** Hierarchical metadata structure that enables fast query planning even with millions of files

- **Schema Evolution:** Table schemas can evolve without rewriting historical data

**Stage 3: AWS Glue Catalog Integration**
The AWS Glue Catalog serves as the universal metadata layer:

- **Cross-Engine Compatibility:** Metadata is accessible to Spark, Trino, Presto, Flink, and other engines

- **Centralized Governance:** Schema registration, versioning, and access control managed centrally

- **Automatic Discovery:** New tables and partitions are automatically discoverable by all connected engines

- **Version Management:** Full lineage tracking and time-travel capabilities across the entire data ecosystem

**Stage 4: Multi-Engine Query Execution**
When TPC-H queries execute against Iceberg tables, the architecture's flexibility shines:

- **Engine Optimization:** Each query engine (Spark, Trino, etc.) applies its own optimizations while sharing the same data

- **Predicate Pushdown:** Filter conditions are pushed down to the storage layer, minimizing data transfer

- **Projection Pushdown:** Only required columns are read from storage, reducing I/O overhead

- **Partition Pruning:** Irrelevant partitions are eliminated at the metadata level before any data access
...
The source and the compute configurations used for the benchmarking are mentioned below:

| **Component** | **Deatils** |
|---------------|-------------|
| Source | Azure PostgrSQL (32 vCores, 128 GB RAM) |
...
- **Fixed Batch Size:** Consistent 200,000-row batches for predictable memory usage and processing patterns
...
**OLake's Architecture Advantages:**

- **Adaptive Chunking:** Dynamically adjusts batch sizes based on system performance

- **Parallel Processing:** Intelligent parallelization that maximizes throughput without overwhelming systems

- **Memory Optimization:** Purpose-built memory management for billion-row datasets

- **Retry Logic:** Automatic retry with exponential backoff for resilient transfers
...
The transfer time paints a stark contrast between the two approaches. One method feels like running a marathon—steady but slow at around 25 hours—while the other clocks in like a sprint, cutting the wall‑clock by well over half for the same dataset.

Time and cost move together in data engineering—longer runs mean higher bills. While you’ll plug in your exact figures, the pattern is clear: OLake’s high throughput reduces runtime and, by extension, infrastructure cost. Plus, OLake is open source, so you’re only paying for the compute and storage you choose to provision, not for a proprietary ingestion layer.
...
**1. Databricks Configuration (Google Cloud Platform):**

- **Master Node: 1x n2-standard-32 (32 vCPUs, 128 GB RAM)**

- **Worker Nodes: 3x n2-standard-16 (16 vCPUs each, 64 GB RAM)**

**2. Iceberg Configuration (AWS EMR):**

- **Master Node: 1x m6g.8xlarge (32 vCPUs, 128 GB RAM)**

- **Core Nodes: 3x m6g.4xlarge (16 vCPUs, 64 GB RAM)**
...
**1. Databricks Total Cost: $11.71**

**GCP Compute Cost: $9.66**

- Master node (n2-standard-32 @ $1.56/hr): $1.56 × 3.10 hours = **$4.83**
...
**2. Iceberg Total Cost: $1.96**  

**AWS EMR Cost (EC2 + EMR fees included):**

- Master node (m6g.8xlarge) @ $0.308/hr: $0.308 × 2.54 hours = **$0.78**

- Core nodes (m6g.4xlarge) (3x @ $0.154/hr each): $0.154 × 3 × 2.54 hours = **$1.17**

**The Verdict:** Iceberg cost **$9.76 less** per benchmark run—that's **83% cheaper** than Databricks while delivering **18.2% faster** execution.

### Production Impact: When Small Differences Become Big Numbers

For a single benchmark run, the $9.76 difference feels negligible. But extrapolate to production workloads, and the math becomes compelling:
...
|---------------|----------------|-------------|----------------|
| Databricks | $11.71 | $351.30 | **$4,274.15** |
| Iceberg | $1.96 | $58.63 | **$712.83** |

**Annual Savings with Iceberg: $3,561.32**
...
| **Metric** | **Databricks** | **Iceberg** | **Conclusion** |
|---------------|----------------|-------------|----------------|
| Data Transfer Speed | 25.7 hours | 12 hours | **Iceberg is 2.1x faster.** |
| Data Transfer Cost | $39 | $20 | **Iceberg is 49% cheaper.** |
| TPC-H Query Speed | 186 minutes | 152 minutes | **Iceberg is 22% faster.** |
| TPC-H Query Cost | $11.71 | $1.96 | **Iceberg is 83% cheaper.** |
| Total Cost | $50.71 | $21.96 | **Iceberg is 57% cheaper.** |
| Total Time | 211.9 hours | 164.4 hours | **Iceberg is 22% faster.** |
...
But if your team can manage that complexity—or if you use managed services like AWS Glue or Databricks' own Iceberg support—**Iceberg's 56.7% cost advantage combined with 22.4% faster end-to-end execution creates a compelling case.**
