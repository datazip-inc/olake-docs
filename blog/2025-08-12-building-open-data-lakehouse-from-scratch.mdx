...
### Q1. How do you build an open-source data lakehouse using OLake and MinIO?
 
The stack consists of five components:
 
1. **MySQL** — source operational database
2. **OLake** — CDC ingestion engine that captures changes from MySQL and writes them as Apache Iceberg tables
3. **MinIO** — S3-compatible object storage acting as the data lake, storing Parquet data files and Iceberg metadata
4. **Iceberg REST Catalog** — metadata layer that tracks table schemas, snapshot locations, and current table state
5. **PrestoDB** — distributed SQL query engine that connects to the REST catalog and queries data with sub-second latency using standard SQL
...
### Q2. What is MinIO's role in a data lakehouse architecture?
...
### Q3. How does OLake's CDC pipeline from MySQL work without needing Kafka?
...
### Q4. How do you query Apache Iceberg tables stored in MinIO using PrestoDB?
...
### Q5. What advantages does an open-source lakehouse stack offer compared to managed cloud services?
