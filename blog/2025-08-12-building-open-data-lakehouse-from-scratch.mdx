...
<Faq showHeading={false} data={[
  {
    question: "Q1. How do you build an open-source data lakehouse using OLake and MinIO?",
    answer: <div>
      <p>The stack consists of five components:</p>
      <ol>
        <li><strong>MySQL</strong> - source operational database</li>
        <li><strong>OLake</strong> - CDC ingestion engine that captures changes from MySQL and writes them as Apache Iceberg tables</li>
      ...
      <p>OLake captures changes from MySQL using Change Data Capture and writes them as Apache Iceberg snapshots into MinIO. PrestoDB connects to the Iceberg REST catalog to discover tables and executes distributed queries directly against the Parquet files in MinIO.</p>
    </div>
  },
  {
    ...
    answer: <div>
      ...
      <p>Because MinIO is fully S3-compatible, any tool that supports AWS S3 - including OLake, Trino, Spark, and DuckDB - works with MinIO out of the box, making it ideal for on-premise or self-hosted lakehouses without requiring cloud vendor dependencies.</p>
    </div>
  },
  {
    question: "Q3. How does OLake's CDC pipeline from MySQL work without needing Kafka?",
    answer: <div>
      <p>OLake connects directly to MySQL's binary log (binlog) and reads change events without requiring Kafka brokers, ZooKeeper, or Kafka Connect. Changes are written in real time as Apache Iceberg snapshots to MinIO.</p>
      <p>This eliminates the entire Debezium + Kafka + sink connector stack - reducing the number of infrastructure components from 5-6 down to <strong>3 (OLake, MinIO, and an Iceberg REST Catalog)</strong>, while maintaining ACID guarantees through Iceberg's atomic commit model.</p>
    </div>
  },
  {
...
hive.s3.endpoint=http://minio:9000
  ...
  },
  {
    ...
    answer: <div>
      <p>An open-source stack (OLake + MinIO + Iceberg REST Catalog + PrestoDB + Apache Iceberg) offers several key advantages:</p>
      <ul>
