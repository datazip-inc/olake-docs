...
description: "How OLake Go works under the hood: eight sources driven by one sync engine, chunked parallel backfills, native CDC, state management, the Arrow write path to Apache Iceberg, and Parquet on S3-compatible storage."
...
- One **sync engine** drives all eight sources. A source is just a small driver interface; parallel execution, retries, state tracking, and exactly-once delivery are all built once in the shared engine, not per source.
...
2. The **sync engine** orchestrates: it decides what each stream needs (full load, incremental catch-up, or CDC), fans chunks out across parallel workers, enforces connection limits against the source, retries failures, and tracks state.
...
<img
  src="/img/blog/2026/8/olake-pipeline-overview.webp"
  alt="OLake Go pipeline: eight sources read into the sync engine, which classifies, plans, converts types, and tracks state, then hands records to writers that buffer, evolve schema, and commit atomically to Apache Iceberg or Parquet"
  loading="lazy"
  decoding="async"
...
Nothing persists in the process between runs. A sync loads `state.json`, does its work, writes the updated state back, and exits, so there is no always-on server holding pipeline progress in memory. Everything needed to resume (chunk lists, cursor bookmarks, log positions) lives in `state.json` and the destination table itself, which is what makes recovering from a crash a matter of rereading a file rather than rebuilding a server's internal state. Exactly what each mode writes into that file is covered in depth in [State management](#state-management).

The rest of this blog follows a record through the pipeline, starting with the sources.
...
<img
  src="/img/blog/2026/8/olake-stream-driver-engine.webp"
  alt="What a stream is and what a driver implements versus the shared sync engine"
  loading="lazy"
  decoding="async"
...
Everything else lives in the shared **sync engine**: running chunks across workers, retrying failures, managing writer lifecycles, tracking state, and coordinating the handoff from full load to CDC. That split is what keeps eight sources maintainable in one codebase, and it is why a new source immediately inherits parallelism, resumability, exactly-once delivery, and every destination the moment its driver compiles.
...
Incremental only ever emits `u`, for both new and changed rows; it never distinguishes an insert from an update, and it has no `d` at all. A row deleted from the source doesn't trigger anything, it just stops showing up in future reads and stays in the destination indefinitely. CDC doesn't have this gap: a delete is a real event on its change log.
...
Underneath, two separate concurrency groups are at work. The first starts one goroutine per selected stream immediately, with no limit: sync 50 tables on full refresh, and all 50 begin at once. This part is cheap, a goroutine that hasn't touched the database yet costs almost nothing.

Actually reading from the source is gated separately. Every one of those streams feeds its chunks into a single shared **connection group**, capped by `max_threads` in the source config. If `max_threads` is 10, at most 10 chunks, across all 50 streams combined, are ever querying the source at the same time; the rest just wait for a free slot. That's the one dial that controls how hard OLake Go hits a production database, no matter how many tables you've selected. Every unit of work that touches the source, whether it's one [chunk](#chunking-and-parallel-full-loads) of a full load or a stream's change-log session, goes through this same shared, bounded group.
...
<img
  src="/img/blog/2026/8/olake-chunking-pipeline.webp"
  alt="A source table splits into chunks tracked in state.json, chunks move through a bounded worker pool from pending to in-progress to committed, and are removed from state.json once committed"
  loading="lazy"
  decoding="async"
...
#### Sequential (PostgreSQL, MySQL)

The whole database has a single change log, so a single reader tails it and routes each event to the right stream's writer. More readers would not help; the log is one ordered sequence.

<div style={{ margin: '2.5rem 0', textAlign: 'center' }}>

<img
  src="/img/blog/2026/8/olake-cdc-sequential.webp"
  alt="Sequential topology: one change log, one reader, fanning out to each stream's own writer and table"
  loading="lazy"
  decoding="async"
  style={{ width: '80%', height: 'auto', display: 'block', margin: '0 auto' }}
/>
...
#### Concurrent (MongoDB, SQL Server)

Every stream has its own change feed, so each stream's CDC starts the moment its own full load finishes, without waiting for the other streams.

<div style={{ margin: '2.5rem 0', textAlign: 'center' }}>

<img
  src="/img/blog/2026/8/olake-cdc-concurrent.webp"
  alt="Concurrent topology: each stream has its own change feed, reader, writer, and table, none share or wait on each other"
  loading="lazy"
  decoding="async"
  style={{ width: '80%', height: 'auto', display: 'block', margin: '0 auto' }}
/>
...
Topic partitions are independent, so multiple readers consume simultaneously under one consumer group.

<div style={{ margin: '2.5rem 0', textAlign: 'center' }}>

<img
  src="/img/blog/2026/8/olake-cdc-parallel.webp"
  alt="Parallel topology: partitions from every selected topic are distributed across readers in one consumer group, each reader has its own writer per topic, and every topic's writers converge on that topic's table"
  loading="lazy"
  decoding="async"
  style={{ width: '80%', height: 'auto', display: 'block', margin: '0 auto' }}
/>
...
<img
  src="/img/blog/2026/8/olake-cdc-handoff-timeline.webp"
  alt="Handoff from full load to CDC: PreCDC pins the log position, the chunked full load runs, the log replays from the pinned position, an overlap window tags inserts 'i' for upsert dedup, then steady state tags inserts 'c' for plain append"
  loading="lazy"
  decoding="async"
...
That replay means overlap: a row inserted during the load can show up twice, once from the chunk scan, once from the log. Not a rare edge case, just how the timing works out:

- **The pin happens once.** `PreCDC` fixes the log position before the backfill starts.
- **The backfill takes time.** A chunk only opens its own transaction when its turn comes up on the worker pool, not when the backfill begins, and a big table's backfill can run for hours.
- **No driver dodges this.** A chunk only sees what's committed at the moment it runs. If a row was inserted after CDC's pin point but before that, the chunk picks it up too.
- **So:** a row inserted after the pin but before a given chunk's turn lands in that chunk's scan, and CDC already replayed the same row, since it started right at the pinned position.

OLake Go doesn't try to stop this from happening. It just tags every record by what produced it, and the tag decides how the write lands:
...
Take row 42, inserted mid-load:

- **Backfill** reads it and writes it as a plain append, op_type `r`.
- **CDC replay** picks up the same insert. Still inside the overlap window, so it gets tagged `i`.
- Both hash to the same `_olake_id`. On the `i`-tagged copy, the writer doesn't append a second row, it fires an equality delete for that ID plus a fresh data file.
- Iceberg commits both in one shot. The equality delete wipes out any existing row with that ID, wherever it came from, so the backfill's copy of row 42 is gone the moment the commit lands.
- Query the table afterward: one row for id 42, not two. Full delete mechanics are in [Upserts: equality and positional deletes](#upserts-equality-and-positional-deletes).

<div style={{ margin: '2.5rem 0', textAlign: 'center' }}>

<img
  src="/img/blog/2026/8/olake-overlap-dedup.webp"
  alt="A row inserted during backfill and replayed again by CDC, tagged i during the overlap window, deduped via an equality delete plus a new data file in one atomic Iceberg commit, ending as a single row in the destination table"
  loading="lazy"
  decoding="async"
  style={{ width: '85%', height: 'auto', display: 'block', margin: '0 auto' }}
/>
...
One more piece: how does a stream even know if it's still in the overlap window? It doesn't keep that in memory, it reads it from the same `olake_2pc` ledger the exactly-once guarantee already relies on.

- Every backfill chunk commit writes `dedup_inserts: true`, no conditions attached. Backfill still running anywhere on that stream means the window stays open.
- The first CDC session to finish writes `dedup_inserts: false` into that same property, right next to the log position it just committed.
- Every CDC session after that checks `dedup_inserts` before tagging anything. Once it reads `false`, row 42 lands as a plain `c` the next time, no delete needed.
- Process dies mid-session, before that `false` gets written? Next run reads the ledger, still finds `true` from the backfill, and stays in dedup mode. Nothing to guess.
...
<img
  src="/img/blog/2026/8/olake-exactly-once-ledger.webp"
  alt="One atomic Iceberg transaction commits data files, delete files, and the olake_2pc table property together, so the table is visible all-or-nothing, and the next run reads olake_2pc first to skip already-committed work"
  loading="lazy"
  decoding="async"
  style={{ width: '80%', height: 'auto', display: 'block', margin: '0 auto' }}
/>
...
That merge follows one rule: two types combine into the closest type that can represent both of them. Internally the types are arranged as a tree, and merging two of them is a walk up to their lowest common ancestor. Each arrow below points from a narrower type to the wider type it merges into:

<div style={{ margin: '2.5rem 0', textAlign: 'center' }}>

```mermaid
flowchart BT
    Bool["Bool (0)"] --> Int32["Int32 (1)"]
    Int32 --> Int64["Int64 (2)"]
    Int64 --> Float64["Float64 (3)"]
    Float32["Float32 (4)"] --> Float64
    Timestamp["Timestamp (6)"] --> TimestampMilli["TimestampMilli (7)"]
    TimestampMilli --> TimestampMicro["TimestampMicro (8)"]
    TimestampMicro --> TimestampNano["TimestampNano (9)"]
    Float64 --> String["String (5)"]
    TimestampNano --> String
```

</div>

Two integer widths meet at the wider integer (`Int32` + `Int64` → `Int64`), an integer and a float meet at float (`Int64` + `Float32` → `Float64`), and types on different branches meet at `String`, which sits at the root and can hold anything (`Float64` + `Timestamp` → `String`). The same rule governs discovery and, later, what happens when live data disagrees with the table it is being written to.
...
A **single engine** sits behind every source. Each driver only answers questions about its own database, and the shared engine owns parallel execution, retries, state, and the write path, so eight sources behave identically and a ninth costs almost nothing to add.
