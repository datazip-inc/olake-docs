...
While most Iceberg engines (Spark, Flink, Trino, Snowflake) support merge-on-read by reconciling delete files to handle heavy CDC volumes, Databricks is the exception.
...
In this tutorial, we'll write Iceberg tables with deletion vectors from MySQL to Snowflake Horizon Catalog using OLake Go. To spin up OLake UI, run the following command:
...
### 5. Query the table in Snowflake

![Query in Snowflake](/img/blog/2026/9/streams.webp)
