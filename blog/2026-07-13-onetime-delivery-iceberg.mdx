...
image: /img/blog/cover/onetime-delivery-iceberg-cover.webp
---

import BlogCTA from '@site/src/components/BlogCTA';

![How OLake Guarantees One-Time Delivery of Data](/img/blog/cover/onetime-delivery-iceberg-cover.webp)
...
One-time delivery fixes both. It adds one check that runs across all sync modes, Full Refresh, Incremental, and CDC, and behaves the same on every driver. Here is how it works.

## How OLake's Exactly-Once Mechanism Works
...
![Full Refresh commit and recovery flow](/img/blog/2026/7/onetime-delivery-full-refresh.webp)
...
![Global LSN CDC recovery flow for Postgres and MySQL](/img/blog/2026/7/onetime-delivery-cdc-global-lsn.webp)
...
![Per-stream LSN CDC recovery flow for MSSQL and MongoDB](/img/blog/2026/7/onetime-delivery-cdc-per-stream-lsn.webp)
...
![Incremental sync cursor recovery flow](/img/blog/2026/7/onetime-delivery-incremental.webp)
...
Whenever a sync dies, the table always ends up correct, with nothing duplicated and nothing missing. And that is what really counts: you might be running your dashboards, reports, and models straight off this Iceberg table, and with one-time delivery you can trust those numbers are right, every single time.

## FAQs

### Q1. Does OLake guarantee exactly-once delivery to Apache Iceberg?

Yes. OLake guarantees exactly-once delivery of data into Apache Iceberg across all three sync modes: Full Refresh, Incremental, and CDC. It commits data files and a progress marker to Iceberg in a single atomic operation, then reconciles its checkpoint file against that marker on every restart, so crashed syncs resume without duplicating or skipping records.

### Q2. What happens if an OLake sync crashes in the middle of a write?

On the next run, before reading anything from the source, OLake compares the progress marker stored in the Iceberg table's metadata against its `state.json` checkpoint. If the Iceberg marker is ahead, the data already committed, so OLake skips it and repairs the checkpoint to match. If they agree, the sync runs normally. Committed chunks are never reprocessed, and chunks whose commit failed are reprocessed from scratch.

### Q3. How does OLake prevent duplicate records after a crash and retry?

OLake generates a deterministic thread ID for each unit of work, built from the stream ID plus the chunk bounds (Full Refresh) or cursor value (Incremental). Because the ID is derived from content rather than a random value, a retry produces the same ID. OLake finds that ID already recorded in the Iceberg table's `olake_2pc` property and skips the work instead of writing the data twice.

### Q4. Does OLake need an external coordinator for exactly-once delivery?

No. OLake's exactly-once guarantee relies only on Apache Iceberg's atomic commits and a comparison between the table's committed metadata and OLake's checkpoint. There is no external coordinator, no transaction log, and no locks involved.

### Q5. Which databases does OLake's exactly-once delivery support?
The guarantee behaves the same on every driver. PostgreSQL and MySQL recover using a single global position (WAL LSN or binlog position), while MSSQL and MongoDB recover per stream using each table or collection's own LSN or resume token.
