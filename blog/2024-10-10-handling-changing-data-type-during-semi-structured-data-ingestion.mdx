...
<Faq showHeading={false} data={[
  ...
  },
  {
    ...
    answer: <div>
      <p>OLake Go uses Iceberg's native schema evolution to handle type changes. When a field that was previously an integer begins arriving as a string, OLake detects the incompatibility and applies the safest resolution:</p>
      <ul>
        <li>For <strong>compatible widening changes</strong> (e.g. <code>int → long</code>, <code>float → double</code>), OLake automatically promotes the column type via a metadata-only Iceberg schema update — no existing data files are rewritten</li>
        <li>For <strong>compatible widening changes</strong> (e.g. <code>int</code> to <code>long</code>, <code>float</code> to <code>double</code>), OLake automatically promotes the column type via a metadata-only Iceberg schema update, so no existing data files are rewritten.</li>      </ul>
    </div>
  },
]} />
