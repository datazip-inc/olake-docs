...
<Faq showHeading={false} data={[
  ...
  },
  {
    ...
    answer: <div>
      ...
      <ul>
        <li>In <strong>traditional warehouses</strong> (e.g. Redshift, Snowflake), some schema updates that require type changes can involve table migrations that disrupt queries. Safe widenings like <code>INT</code> to <code>BIGINT</code> are usually metadata-only, but narrowing or incompatible type changes are the ones that force a rewrite.</li>
