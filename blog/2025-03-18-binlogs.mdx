...
jsonLd: |
  {
    ...
    "@type": "TechArticle",
    "headline": "What Are Binlogs?",
    "alternativeHeadline": "Complete guide to MySQL binary logs - understanding binlogs for replication, recovery, and change data capture",
    "author": {
      "@type": "Person",
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": "https://olake.io/blog/binlogs"
    },
    "description": "Binary logs in MySQL are files that log all changes made to your database. These logs record every operation that modifies data (like INSERT, UPDATE, DELETE statements). They don't log SELECT statements or other read-only operations.",
    "keywords": "mysql, binlogs, binary logs, replication, change data capture, cdc, database, recovery, auditing, data analysis, master-slave, point-in-time recovery, mysql replication, data engineering, etl, elt",
    ...
    "about": [
      {
        "@type": "Thing",
        "name": "Binary Logs",
        "description": "MySQL files that log all changes made to the database in structured binary format"
      },
      {
        "@type": "Thing",
        "name": "Database Replication",
        "description": "Process of copying data from one database to another for backup and scaling"
      },
      {
        "@type": "Thing",
        "name": "Change Data Capture",
        "description": "Method of tracking and analyzing database changes in real-time"
      },
      {
        "@type": "Thing",
        "name": "Data Recovery",
        "description": "Process of restoring data from backups or logs after data loss"
      }
    ],
    "mentions": [
      {
    ...
    "mainEntity": {
      ...
      "mainEntity": [
        {
          "@type": "Question",
          "name": "What are MySQL binary logs?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Binary logs in MySQL are files that log all changes made to your database. These logs record every operation that modifies data (like INSERT, UPDATE, DELETE statements) in a structured binary format, but don't log SELECT statements or other read-only operations."
          }
        },
        {
          "@type": "Question",
          "name": "Why do we need binlogs in MySQL?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Binlogs are essential for MySQL replication (master-slave copying), point-in-time recovery, auditing, and data analysis. They provide a chronological record of SQL statements or row-level changes that modify the data in the database."
          }
        },
        {
          "@type": "Question",
          "name": "How do binlogs work for MySQL replication?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "In MySQL replication, the master database records every change in the binlog, and the slave databases replay those changes to stay in sync. This ensures that all replicas have the same data as the master database."
          }
        },
        {
          "@type": "Question",
          "name": "What is point-in-time recovery using binlogs?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Point-in-time recovery allows you to restore your database to any specific moment by using binlogs along with full backups. You can replay the binlog events from the backup point to the desired recovery time."
          }
        }
      ]
    }
