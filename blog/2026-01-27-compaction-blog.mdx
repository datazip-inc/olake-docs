...
<Faq showHeading={false} data={[
  {
    ...
    answer: <div>
      <p>Every write to an Iceberg table, whether from CDC, Kafka streaming, or batch jobs, creates new Parquet data files, and those files are not necessarily optimally sized. A small change writes a small file, so over time this can produce thousands or even millions of small files.</p>
