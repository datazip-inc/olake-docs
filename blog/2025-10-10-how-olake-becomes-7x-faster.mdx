...
In this blog I am going to explain how we achieved this, so let us start. If you are just interested in what boosted performance, read the section <a href="#olake-new-iceberg-writer-what-we-changed-to-make-it-fast">What We Changed</a>.
...
After identifying these issues, the first step to solving them was understanding Iceberg's distributed writing model. You might be thinking, "Iceberg sounds complex, is this going to be hard to understand?" Don't worry. While Iceberg might seem complex at first glance, the core concepts are straightforward once you understand the fundamentals.

Remember learning about ACID properties in databases? Those guarantees weren't present in legacy data lakes, which made things like concurrent writes and schema changes tricky. But Iceberg table format brings ACID properties to data lakes, making your machine learning and analytics use cases much easier. The key thing to understand here is that we need atomicity while writing to Iceberg—either all your data commits successfully, or none of it does.

You might have heard the Iceberg community suggest retrying failed commits. But here's the thing: this advice has limitations. If your data is fundamentally incompatible with the schema (for example, trying to insert a string value into an integer column), retrying won't help—the operation will fail regardless of how many times you retry. Think about it: if you're trying to put a square peg in a round hole, trying harder won't make it fit.

Also, let me set the right expectations: **Iceberg is not designed for real-time streaming use cases (until the small file problem is solved)**. Even popular streaming tools like Tableflow batch data based on time periods before writing to Iceberg. So if you're looking for sub-second latency, you'll need to look elsewhere—at least for now.

If you have worked with parquet, if there is schema change you have to close the previous parquet and use the new one. The same applies with writers in iceberg. Each writer acomodate its own schema and you can push only the same schema data to that particular writer.
...
- **Efficient processing**: Local batching allows for optimizations like schema detection and data normalization before network transmission
---

### Atomic Commits And Schema Evolution
...
We implemented one optimization: if there are type promotions (for example, from `int` to `long`), we don't close or refresh the writer until a record with a different incompatible type arrives in that chunk. This saves the overhead of refreshing and closing files unnecessarily.
...
### Removal of JSON Overhead And Type Detection
...
- **Concurrent processing**: Managing multiple writer threads that can process different partitions or tables simultaneously
- **Schema coordination**: Detecting table changes and coordinating evolution across threads
- **Normalization**: Parallel normalization of data across multiple threads 
...
So to reduce RPC calls and overhead of Java server, the identical writer schema copy is being maintained on the Go side, which is responsible to check if any schema evolution is required or not. In that way, we avoid unnecessary locks for the schema checks. 

### Partition Fanout Writer

Highly partitioned tables were another pain point we encountered. In the previous implementation, when a record with a different partition arrived, it would close the current writer and open a new one. This meant we were processing partitions sequentially—only one partition writer could be active at a time. This approach had several problems: frequent writer creation and destruction overhead, many small files being created, and poor I/O efficiency.

The issue becomes more pronounced when you have tables with many partitions. Imagine processing a table with 50-100 partitions—you're constantly creating and destroying writers, which is expensive. Plus, all those small files hurt query performance because query engines need to read metadata from many files.

In our new implementation, we maintain multiple partition writers concurrently. When memory allows, we can have several partition writers active at the same time, each handling different partitions. Each partition buffers its data to target larger, more consistent file sizes.

The benefits are clear: improved throughput especially on highly partitioned tables, consistent file sizes that lead to better query performance, and better resource utilization. This optimization is particularly important for tables with many partitions, where the traditional approach would create hundreds or thousands of small files, leading to poor query performance and storage inefficiency.
...
Behind these optimizations lies a lot of experimentation and iteration. We first considered how to make the Java Iceberg server compatible with Olake. Our CTO started with a basic version that correctly supported Iceberg writes, as there was no existing tool that could write to Iceberg in a truly native way. We then decided to optimize it specifically for Olake, with our main goal being to build the best possible and most reliable solution.

I started experimenting with a structure where each writer would have multiple flush threads, but encountered OOM (Out of Memory) issues. I also struggled to determine the optimal parameter for the number of flush threads per writer—too many flush threads with fewer writer threads would reduce database throughput. I eventually set the number to a constant value of 1 and moved forward. The next challenge was writer lifecycle management: when a schema change occurs, you must create a new Java writer, similar to creating a new Parquet file when the schema changes. We spent a day debugging this issue, primarily because the documentation didn't clearly state that each writer is based on a single schema. 
