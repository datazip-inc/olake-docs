...
<Faq showHeading={false} data={[
  {
    question: "Q1. Why was OLake's previous Iceberg writer slow?",
    answer: <div>
      <p>The previous writer used Debezium-format RPC communication with a Java Iceberg server, causing:</p>
      <ul>
        <li><strong>Unnecessary serialization and deserialization overhead</strong> data was repeatedly converted between formats crossing the Go-to-Java boundary</li>
        <li><strong>Double data buffers</strong> that inflated memory usage beyond what the workload required</li>
        <li><strong>A partition writer that closed files on every new partition record</strong> creating excessive small file overhead</li>
        <li><strong>Heavy JVM-side processing</strong> that created throughput bottlenecks at scale</li>
      </ul>
    </div>
  },
  {
    question: "Q2. How did OLake achieve a 7x performance improvement in Iceberg writes?",
    answer: <div>
      <p>OLake rebuilt its writer by making three key architectural changes:</p>
      <ol>
        <li><strong>Go/Java split</strong> shifted all data processing to Go (which excels at concurrency and fast data handling) and kept Java only for Iceberg I/O operations, where the Java Iceberg library remains the most mature implementation</li>
        <li><strong>Single 10k-record batch</strong> eliminated double data buffers in favor of one buffer with a 10k batch size, reducing memory footprint and increasing throughput</li>
        <li><strong>Full chunk commits</strong> writes complete 4GB chunks before committing (compressing to approximately 350MB files), rather than committing on every partition boundary</li>
      </ol>
      <p><strong>Apache Arrow</strong> was subsequently introduced as a further optimization on top of this refactor eliminating the Go-to-Java gRPC bridge for data writes entirely and enabling direct Parquet writes from the Go layer.</p>
    </div>
  },
  {
    question: "Q3. What is the role of Apache Arrow in OLake's new Iceberg writer?",
    answer: <div>
      <p>Apache Arrow provides a columnar in-memory data format that allows OLake's Go processing layer to write data directly to Parquet files without going through the Java server for data records.</p>
      <p>Arrow gives programs a shared memory format so they can understand the same data without translating it cutting out slow serialization work. In OLake's architecture this means:</p>
      <ul>
        <li>The Go layer handles all data path operations using Arrow's columnar format</li>
        <li>The expensive Go-to-Java gRPC bridge for data writes is eliminated entirely</li>
        <li>Java APIs are used only for Iceberg metadata management</li>
      </ul>
      <p>This cut down huge amounts of serialization overhead and JVM churn that remained after the initial 7x refactor.</p>
    </div>
  },
  {
    question: "Q4. What is schema evolution in the context of OLake Iceberg writes?",
    answer: <div>
      <p>Schema evolution means updating the Iceberg table schema when source tables gain new columns or change column types. OLake's writer handles this by:</p>
      <ol>
        <li>Detecting a schema change in the incoming CDC stream</li>
        <li>Closing the current Parquet writer cleanly</li>
        <li>Opening a new writer configured with the updated schema</li>
      </ol>
      <p>This ensures no data is lost during the transition, and queries on old snapshots still work correctly Iceberg's metadata-only schema evolution guarantees that existing data files remain valid and readable under the old schema.</p>
    </div>
  },
  {
    question: "Q5. How does OLake ensure exactly-once writes to Apache Iceberg?",
    answer: <div>
      <p>OLake follows Iceberg's atomic commit model:</p>
      <ol>
        <li>Data files are written to object storage first, independently of the metadata layer</li>
        <li>A single metadata swap then commits the new snapshot atomically</li>
      </ol>
      <p>If a write fails mid-way, the old snapshot remains intact and readers never see partial data. Go's concurrency model ensures clean retry behavior parsing, schema evolution, buffering, and batch management all happen in Go, preventing duplicate commits even under retry scenarios.</p>
    </div>
  }
]} />
