...
Highly partitioned tables were another pain point we encountered. In the previous implementation, when a record with a different partition arrived, it would close the current writer and open a new one. This meant we have to sort data in each batch and then write, which eventually create small files. This approach had several problems: frequent writer creation and destruction overhead, many small files being created, and poor I/O efficiency.
