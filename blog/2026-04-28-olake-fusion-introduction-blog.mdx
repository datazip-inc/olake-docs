...
authors: [siddharth]
image: /img/blog/2026/5/OLakeFusionBlogCoverImage.webp
---
![OLake fusion cover image](/img/blog/2026/5/OLakeFusionBlogCoverImage.webp)
...
This blog is about that second half: why Iceberg table maintenance tends to become a full-time headache, what teams are doing today to cope with it, and what we built at OLake to actually solve it.

Running Apache Iceberg with CDC pipelines means dealing with a problem most teams don't anticipate: small files and delete file accumulation. Every CDC write creates new data files and delete files. They pile up fast. After a few weeks, queries that took seconds start taking minutes, file counts climb into the tens of thousands, and someone on the team is maintaining a custom Spark maintenance script that nobody fully understands.

This is the Iceberg table maintenance problem.

Apache Iceberg compaction is one of the most important maintenance jobs for production lakehouses. It consolidates fragmented files, resolves accumulated deletes, reduces metadata overhead, and keeps query engines like Spark, Trino, and Athena fast as CDC pipelines continuously write to your tables.
...
## Introducing OLake Fusion: Multi-Tiered Apache Iceberg Compaction
...
| Compaction run status | Failed or partial runs need fast diagnosis. |
...
Table maintenance in Apache Iceberg is the set of routine operations that keep tables fast and storage-efficient as data is written, updated, and deleted. Because every write creates new files and a new snapshot, tables accumulate small files, stale snapshots, orphan files, and bloated metadata over time.

A typical maintenance workflow includes compaction (`rewrite_data_files`) to merge small files and resolve deletes, snapshot expiration (`expire_snapshots`) to drop old snapshots, orphan file removal (`remove_orphan_files`) to clean up unreferenced files, and manifest rewriting (`rewrite_manifests`) to speed up query planning. Tools like OLake Fusion automate these operations on a per-table schedule.
...
where `id = 42`"). They require the query engine to most of the data files to apply
...
### Q10. How does OLake Fusion differ from running Spark `rewrite_data_files`?
