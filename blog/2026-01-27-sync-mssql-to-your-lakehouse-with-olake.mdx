...
<Faq showHeading={false} data={[
  ...
  },
  {
    ...
    answer: <div>
      <p>OLake leverages Apache Iceberg's native schema evolution capabilities to handle changes automatically.</p>
      <ul>
        <li><strong>New columns:</strong> detected and added without rewriting existing data</li>
        <li><strong>Type promotions:</strong> (e.g., INT → BIGINT) handled automatically</li>
        <li><strong>Incompatible changes:</strong> flagged for manual review to prevent corruption</li>
      </ul>
      <p>This ensures pipelines remain stable even as source schemas evolve.</p>
    </div>
  },
  {
