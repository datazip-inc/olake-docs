...
### Q1. What are the main features of Apache Iceberg that make it the leading table format?
...
### Q2. How does Apache Iceberg support ACID transactions on a data lake?
...
- **Optimistic concurrency control** handles simultaneous writers, if two writers conflict, one commit fails and retries, preventing corrupt data races

### Q3. What is time travel in Apache Iceberg and how can I use it?
...
| Engine | Snapshot ID syntax | Timestamp syntax |
|---|---|---|
| **Spark** | `VERSION AS OF <snapshot_id>` | `TIMESTAMP AS OF '<timestamp>'` |
| **Trino** | `FOR VERSION AS OF <snapshot_id>` | `FOR TIMESTAMP AS OF TIMESTAMP '<timestamp>'` |
| **Athena** | `FOR VERSION AS OF <snapshot_id>` | `FOR TIMESTAMP AS OF '<timestamp>'` |
 
Time travel is invaluable for auditing, reproducing historical reports, and recovering from accidental data overwrites.

### Q4. How does Apache Iceberg's hidden partitioning improve query performance?
...
- **Partition strategies can be evolved over time;** switching from daily to hourly partitioning, for example, without rewriting historical data files

### Q5. Which query engines are natively compatible with Apache Iceberg?
