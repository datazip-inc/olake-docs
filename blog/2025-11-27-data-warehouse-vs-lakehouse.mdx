...
<Faq showHeading={false} data={[
  ...
  },
  {
    ...
    answer: <div>
      <p>The main ones are responsibilities the warehouse used to handle silently. The small-files problem is the biggest: ingestion can create millions of tiny files that cripple query performance, so you need scheduled compaction to consolidate them into optimally sized files (roughly 128MB to 512MB). Tools like <a href="https://github.com/datazip-inc/olake-fusion">OLake Fusion</a> automate this by monitoring Iceberg table health and running compaction for you, closing the gap with the warehouse's invisible maintenance.</p>
