...
At a high level, an Iceberg writer has two strategies:
...
Think of a library catalog. It can tell two readers where a book is located. It does not translate the book into a language one reader understands.
...
Checking only "Does this engine support Iceberg?" is too coarse.

## The Other Row-Level Delete Encodings

Equality deletes are only one way to record a row-level change. Understanding the alternatives shows why a writer's decision affects every future reader.
...
If Spark supports equality deletes and position deletes, while the other required paths support only position deletes, the shared capability is **position deletes**.
...
A native table and an externally managed table in the same product may have different capabilities.
...
Record whether each capability is generally available, in preview, read-only, or unsupported. Include the date and versions because this matrix will change.
...
Apply the equality deletes and create clean replacement data files. This copy-on-write style migration removes the outstanding equality-delete requirement from the resulting snapshot.
