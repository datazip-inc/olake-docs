...
def build_spark_session_from_writer(writer: dict) -> SparkSession:
    ...
    # Incase you are running it on AWS EMR make sure of these settings: 
    builder = builder.config("spark.dynamicAllocation.initialExecutors", "1") 
    builder = builder.config("spark.executor.cores", "2")
    builder = builder.config("spark.executor.memory", "2g")
    builder = builder.config("spark.scheduler.maxRegisteredResourcesWaitingTime", "10s")
    builder = builder.config("spark.scheduler.minRegisteredResourcesRatio", "0.0")
...
def run_compaction_cycle_for_table(mor_table_fqn: str):
    ...
    # Get catalog name from table FQN
    catalog_name, _, _ = split_fqn(mor_table_fqn)

    # Step 1: Enable WAP for COW table if it exists (idempotent)
    if cow_exists:
        enable_wap_for_table(cow_table_fqn)

    # Step 2: Check for existing WAP ID (checkpoint)
    ...
    if wap_id is None:
        ...
        # Re-publish the existing WAP ID (idempotent operation)
        ...
        # Extract truncate snapshot_id from WAP ID
        last_success_t = extract_truncate_id_from_wap_id(wap_id)
        if last_success_t is None:
            print(f"[{mor_table_fqn}] Warning: Could not extract truncate ID from WAP ID {wap_id}. Starting from beginning.")
        else:
            print(f"[{mor_table_fqn}] Last successful truncate checkpoint from WAP ID: {last_success_t}")
    ...
    if truncate_snapshot_id is not None and all(x.get("snapshot_id") != truncate_snapshot_id for x in to_process):
        ...
        """)
            ...
            if truncate_snapshot_id is not None:
                # Set WAP ID, publish changes
                spark.sql(f"SET spark.wap.id={truncate_snapshot_id}")
                publish_wap_changes(cow_table_fqn, catalog_name, str(truncate_snapshot_id))
        ...
        if not cow_exists:
            ...
            enable_wap_for_table(cow_table_fqn)
            # Set WAP ID using SQL SET (required for Spark SQL operations)
            spark.sql(f"SET spark.wap.id={truncate_snapshot_id}")
            spark.sql(f"""
        CREATE TABLE {cow_table_fqn}
        USING iceberg
        ...
        print(f"[{mor_table_fqn}] Published WAP changes with truncate {truncate_snapshot_id}.")
        return
    ...
    # Step 5: For each boundary, compact its parent snapshot, then publish WAP changes.
    ...
    for b in to_process:
        ...
        if h_id is None:
            # Nothing existed before this boundary; ensure COW exists (empty) and publish WAP.
            if not cow_exists:
                print(f"[{mor_table_fqn}] Creating empty COW table (no parent snapshot exists)...")
                enable_wap_for_table(cow_table_fqn)
                spark.sql(f"SET spark.wap.id={t_id}")
                spark.sql(f"""
                    CREATE TABLE {cow_table_fqn}
                    USING iceberg
                    LOCATION '{cow_location}'
                ...
                # Table exists but no parent, just publish WAP for this boundary
                spark.sql(f"SET spark.wap.id={t_id}")
                publish_wap_changes(cow_table_fqn, catalog_name, str(t_id))
                spark.sql("SET spark.wap.id=")
        ...
        if h_id in boundary_ids:
            ...
            if t_id is not None:
                # Still publish WAP for this boundary even though we skip the data write
                spark.sql(f"SET spark.wap.id={t_id}")
                publish_wap_changes(cow_table_fqn, catalog_name, str(t_id))
                spark.sql("SET spark.wap.id=")
            continue
...
```bash
  ...
  --conf spark.scheduler.maxRegisteredResourcesWaitingTime=10s \
  --conf spark.scheduler.minRegisteredResourcesRatio=0.0 \
```
