...
If you're running the script on AWS EMR, you need to copy both `destination.json` and the MOR to COW write script (e.g., `mor_to_cow_script.py`) to an S3 bucket that your EMR cluster can access. Then reference these S3 paths in your `spark-submit` command:
:::
