...
**1. Initial Full Refresh:** OLake performs a complete historical load of your table (e.g., an employees table) to Iceberg.
...
**Leaving blank cause need to put good content**

According to Snowflake's documentation, externally-managed Iceberg tables only support position deletes, not equality deletes. This means any CDC pipeline using equality deletes (the standard approach for streaming CDC) will produce incorrect results in Snowflake.
...
I've built a PySpark script that automates this entire process. Here's how it works:
