...
<Faq showHeading={false} data={[
  ...
  {
    question: "Q2. How do OLake and Lakekeeper work together for real-time data ingestion?",
    answer: <div>
      <p>OLake captures Change Data Capture events from operational databases (MongoDB, PostgreSQL, MySQL) and needs to commit Iceberg snapshots to object storage in a governed, discoverable way. Lakekeeper acts as the REST catalog authority throughout this process:</p>
      <ol>
        <li>OLake checks what tables exist through Lakekeeper's REST API</li>
        <li>OLake registers new tables and schema changes via Lakekeeper</li>
        <li>OLake commits new Iceberg snapshots through Lakekeeper</li>
      </ol>
  ...
  {
    question: "Q3. What is Trino's role in the Iceberg, OLake, and Lakekeeper lakehouse stack?",
    answer: <div>
      <p>Trino is the distributed SQL query engine that provides the analytics layer. After OLake writes data as Iceberg snapshots and Lakekeeper manages the metadata:</p>
      <ul>
  ...
  {
    question: "Q5. What is the complete data flow in the Iceberg, OLake, and Lakekeeper stack?",
    answer: <div>
      <p>The end-to-end flow across the four components:</p>
      <ol>
        <li><strong>OLake</strong> captures database changes (inserts, updates, deletes) from MongoDB, PostgreSQL, or MySQL via CDC and writes Iceberg snapshots - Parquet data files and metadata - directly to object storage (S3, MinIO, GCS, Azure Blob)</li>
      ...
      <p>All three components operate in real time - a CDC event committed by OLake is immediately visible to Trino via Lakekeeper with no additional sync or polling step required.</p>
    </div>
  }
]} />
