...
### Q1. What are the main methods to flatten nested JSON data for SQL analytics?
...
2. **Recursive flattening** — Recursively flatten all nested keys into dot-notation columns (e.g. `user.address.city` becomes `user_address_city`)
3. **Separate tables for arrays** — Explode array fields into separate related tables with foreign keys back to the parent
4. **pandas `json_normalize`** — Python-based flattening for moderate-scale or exploratory workflows
5. **PySpark with automatic schema inference** — Large-scale distributed flattening using `spark.read.json()` with `explode()` for arrays
 
Each method has different trade-offs in complexity, query ease, and storage efficiency.

### Q2. What are the main methods to flatten nested JSON data for SQL analytics?
...
### Q3. What is the best strategy for handling array fields in nested JSON flattening?
...
### Q4. How do ETL tools like Airbyte and Fivetran handle nested JSON flattening?
...
### Q5. When should I use recursive JSON flattening versus creating separate tables for arrays?
