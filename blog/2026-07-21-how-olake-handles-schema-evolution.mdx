---
slug: schema-evolution-apache-iceberg
...
## Introduction
Anyone who has run a production database replication pipeline long enough has run into this problem. A developer adds a column, renames a field, or widens a data type (e.g., INT → BIGINT, FLOAT → DOUBLE), and the next sync crashes outright, or worse, silently writes wrong data downstream. The source database changes shape without asking permission. Your pipeline has to take whatever comes down it.
...
A product team ships a new feature, and suddenly there's a `last_login_at` column that didn't exist yesterday. Someone finally cleans up a table and drops three deprecated fields. A column that was fine as `INT` for two years starts overflowing because the product grew, and now it needs to be `BIGINT`. These are not edge cases. This is just what happens when a production application runs on a database.
...
The first is **structural change**; the shape of the table itself shifts. A new column appears. An old one disappears. Someone renames `user_name` to `username_display`. A table gets renamed entirely. These changes affect what fields exist, not what's inside them.

The second is **data type change**; the same column exists, but what it holds has changed. An `INT` column starts receiving values that don't fit in 32 bits anymore, so it needs to widen to `BIGINT`. A `FLOAT` column gets bumped to `DOUBLE` for more precision. Or someone decides a field that used to hold numbers should now hold strings to accommodate edge cases like "N/A" or legacy IDs. These changes are not all equal: some can be applied safely at the destination, while others, like that numbers-to-strings change, are incompatible with Iceberg's type system. How OLake treats each kind is covered below.
...
Both categories happen routinely in any codebase that's actively being developed or has heavy production usage. The question isn't whether they'll hit your pipeline. It's whether your pipeline survives them when they do or whether you get paged at 2am because a sync broke.

## Why Traditional Formats Struggle

Whether a schema change is safe or destructive comes down to one thing: how the underlying table format identifies its columns. It is exactly the problem Iceberg, and therefore OLake, is built to avoid.
...
**Table renamed at source:** OLake treats a renamed table as a new table on the destination side. This is because OLake cannot distinguish with certainty between "this table was renamed" and "this table was dropped and a new one was created" from the source's change feed alone. A new table gets created at the destination; the old one remains. The old Iceberg table keeps its historical data; post-rename data lands in the new table unless the histories are merged manually.

**Table deleted at source:** No new data is added to the destination table, but existing data and metadata remain queryable, so downstream queries on historical data continue to work. If a table is later recreated with the same name but a different structure, it should be treated as a brand-new stream to avoid field ID collisions.

### Schema-less Sources (MongoDB, Kafka JSON)

Some sources have no fixed schema at all. MongoDB documents in the same collection can have entirely different fields, and JSON messages on a Kafka topic carry no enforced structure either. For these sources, OLake cannot diff a schema upfront the way it can for PostgreSQL or MySQL. Instead it infers the schema from the records themselves, establishing an initial structure from the first records it reads and then evolving it as new fields appear. New keys are added to the destination schema as they appear, and removed keys stop showing up in new rows, the same add and drop behavior as any Iceberg column. The one JSON-specific case is renames: because a JSON key has no field ID to carry its identity, OLake cannot tell a rename from a drop plus an add, so it treats it as exactly that.
