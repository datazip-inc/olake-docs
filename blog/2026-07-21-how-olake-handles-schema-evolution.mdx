...
Whether a schema change is safe or destructive comes down to one thing: how the underlying table format identifies its columns. Getting that identification wrong is exactly the failure mode Iceberg, and therefore OLake, is built to avoid.
...
**Table renamed at source:** OLake treats a renamed table as a new table at the destination. From the source's change feed alone, it cannot reliably tell whether a table was renamed or dropped and recreated, so it does not carry the old table's identity over to the new name. A new table is created at the destination, and the old one stays in place with its historical data. To load the full dataset into the new table, run Clear Destination and sync again from scratch; since a source rename preserves every row, the full load copies the complete table into the new destination table.

**Table deleted at source:** No new data is added to the destination table, but existing data and metadata remain queryable, so downstream queries on historical data continue to work. If a table with the same name is later recreated at the source, note that OLake does not currently support reusing that name at the destination: the recreated table collides with the existing one and the sync fails. Since the original destination table still holds the deleted table's historical data, give the recreated table a different name. That preserves the old history and avoids the collision.
