...
<Faq showHeading={false} data={[
  {
    question: "Q1. How do I automate OLake data sync on AWS EC2 using Apache Airflow?",
    answer: <div>
      <p>Create an Airflow DAG that uses the AWS provider to dynamically provision an EC2 instance, SSH into it, and run OLake inside a Docker container. The DAG handles the full EC2 lifecycle:</p>
      <ol>
        ...
        <li><strong>Execute</strong> - <code>SSHOperator</code> SSHes into the instance and runs the OLake sync command</li>
        <li><strong>Terminate</strong> - <code>EC2TerminateInstanceOperator</code> shuts down the instance on completion</li>
      </ol>
      <p>OLake configuration files are stored in S3 and downloaded to the EC2 instance at runtime.</p>
    </div>
  },
  {
    question: "Q2. What AWS IAM permissions are required to run the OLake Airflow EC2 DAG?",
    answer: <div>
      ...
      <ul>
        <li>Read OLake configuration files from the S3 config bucket</li>
        <li>Read and write OLake state files to the S3 state path</li>
  ...
  {
    question: "Q3. How does OLake use S3 for state persistence when running on ephemeral EC2 instances?",
    answer: <div>
      <p>Since EC2 instances are terminated after each sync run, OLake's state (checkpoints, cursor positions) cannot be stored on the instance's local disk. Instead, the DAG configures OLake to read and write its state files to a dedicated S3 path:</p>
      <ul>
  ...
  {
    question: "Q4. What is the benefit of dynamically provisioning EC2 instances for each OLake sync run?",
    answer: <div>
  ...
  {
    question: "Q5. How do I configure Airflow connections for AWS and SSH to enable the OLake EC2 DAG?",
    answer: <div>
