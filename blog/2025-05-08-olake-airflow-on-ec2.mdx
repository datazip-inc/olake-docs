...
### Q1. How do I automate OLake data sync on AWS EC2 using Apache Airflow?
...
2. **Wait** — `EC2InstanceStateSensor` polls until the instance is ready
3. **Execute** — `SSHOperator` SSHes into the instance and runs the OLake sync command
4. **Terminate** — `EC2TerminateInstanceOperator` shuts down the instance on completion
 
OLake configuration files are stored in S3 and downloaded to the EC2 instance at runtime.

### Q2. What AWS IAM permissions are required to run the OLake Airflow EC2 DAG?
...
### Q3. How does OLake use S3 for state persistence when running on ephemeral EC2 instances?
...
### Q4. What is the benefit of dynamically provisioning EC2 instances for each OLake sync run?
...
- **Right-sizing** — You can choose the optimal EC2 instance type and size for each pipeline independently, rather than over-provisioning a shared persistent server
 ...
 ### Q5. How do I configure Airflow connections for AWS and SSH to enable the OLake EC2 DAG?
