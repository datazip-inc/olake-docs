...
<Faq showHeading={false} data={[
  ...
  },
  {
    ...
    answer: <div>
      <p>Not in the way this question implies. OLake stores array fields as a stringified value, so the entire array lands in a single string column rather than being exploded into separate child tables. If you need arrays in separate related tables, that happens in a downstream transformation after load.</p>
      <p>Debezium's built-in Single Message Transforms (SMTs) can handle simple field-level extractions inline, but they cannot perform the row-multiplication required for proper array-to-table flattening. That still requires Kafka Streams, ksqlDB, or a downstream transformation step.</p>
    </div>
  },
  {
    question: "Q4. What databases does OLake support for CDC compared to Debezium?",
    answer: <div>
      <p>OLake supports CDC for PostgreSQL, MySQL, and MongoDB. Debezium also offers CDC across a broad range of databases, but requires separate connector configurations and Kafka infrastructure for each one, whereas OLake handles multi-source ingestion through a unified UI and a consistent configuration model across all supported databases.</p>
    </div>
  },
  {
