...
## FAQs
### Q1. What is the difference between JSON, BSON, and JSONB?
...
### Q2. Why does MongoDB use BSON instead of plain JSON?
...
### Q3. When should you use JSONB over JSON in PostgreSQL?
...
- **GIN index support** — GIN indexes on JSONB specifically accelerate containment queries using the `@>` operator and key-existence queries using `?`, `?|`, and `?&`. Note that GIN indexes activate only for these specific operator-based queries — plain equality or expression queries on JSONB fields may still need expression B-tree indexes
...
### Q4. What are the performance differences between JSON, BSON, and JSONB for analytical queries?
...
### Q5. Which format should I choose for storing data in an operational database?
