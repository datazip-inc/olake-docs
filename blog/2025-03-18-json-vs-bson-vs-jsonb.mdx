...
jsonLd: |
  {
    ...
    "@type": "TechArticle",
    "headline": "JSON vs BSON vs JSONB - A Detailed Comparison",
    "alternativeHeadline": "Complete technical comparison of JSON, BSON, and JSONB data formats - understanding storage, performance, and use cases for modern databases",
    "author": {
      "@type": "Person",
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": "https://olake.io/blog/json-vs-bson-vs-jsonb"
    },
    "description": "In the world of data formats and databases, JSON, BSON, and JSONB have emerged as prominent ways to store and query structured data. Each format serves specific needs with different trade-offs in efficiency, storage, and query speed.",
    "keywords": "json, bson, jsonb, data formats, mongodb, postgresql, nosql, structured data, data storage, performance, query optimization, data engineering, etl, elt, database comparison, data serialization",
    ...
    "about": [
      {
        "@type": "Thing",
        "name": "JSON",
        "description": "JavaScript Object Notation - lightweight, text-based data format for data interchange"
      },
      {
        "@type": "Thing",
        "name": "BSON",
        "description": "Binary JSON - binary-encoded serialization format used primarily by MongoDB"
      },
      {
        "@type": "Thing",
        "name": "JSONB",
        "description": "Binary JSON - PostgreSQL's binary storage format for JSON data with indexing support"
      },
      {
        "@type": "Thing",
        "name": "Data Serialization",
        "description": "Process of converting data structures into a format suitable for storage or transmission"
      }
    ],
    "mentions": [
      {
    ...
    "mainEntity": {
      ...
      "mainEntity": [
        {
          "@type": "Question",
          "name": "What is the difference between JSON, BSON, and JSONB?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "JSON is a text-based format for data interchange, BSON is MongoDB's binary-encoded format that extends JSON with additional data types, and JSONB is PostgreSQL's binary storage format that optimizes JSON data with indexing and efficient storage."
          }
        },
        {
          "@type": "Question",
          "name": "Which format is faster for queries - JSON or JSONB?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "JSONB is generally faster for queries because it stores data in a binary format that supports indexing, while JSON stores data as text and requires parsing on every query. JSONB also removes whitespace and duplicate keys for better performance."
          }
        },
        {
          "@type": "Question",
          "name": "When should you use BSON vs JSONB?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Use BSON when working with MongoDB as it's the native format and supports additional data types like dates and binary data. Use JSONB when working with PostgreSQL for better query performance and indexing capabilities."
          }
        },
        {
          "@type": "Question",
          "name": "What are the storage implications of each format?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "JSON stores data as human-readable text, BSON uses binary encoding which can be larger than JSON but includes type information, and JSONB uses binary storage with compression and deduplication for efficient storage in PostgreSQL."
          }
        }
      ]
    }
