...
- **Bootstrap Servers**
    - Comma-separated broker addresses (e.g., `broker-1:9092,broker-2:9092`). Provide 2+ for HA; the rest are auto-discovered.
- **Protocol**
...
- **Consumer Group ID**
    - Optional. Use provided ID; otherwise OLake generates `olake-consumer-group-{timestamp}` and persists it for future syncs.
- **MaxThreads**
    - Default 3. Caps concurrent readers/writers. Higher = more throughput, more resources.
    - When `threads_equal_total_partitions` is false, partitions are round-robin across up to `max_threads` readers.
- **RetryCount**
    - Default 3. Retries transient failures with exponential backoff (~1 minute between attempts).
...
2. **Commit Action: Ensuring Destination Sync Before Kafka Offset Commit**
    ...
    - When: commit only after all partitions assigned to a reader reach the start-of-sync high watermark/partition-offset.
...
- Determine reader count from concurrency settings and the number of partitions with new data.
    ...
    For each selected stream, OLake performs a pre-flight check per partition and fetches three metadata points (`Partition Metadata`): first available offset, last available offset (high watermark), and last committed offset.
...
- Readers fetch, process, and write; progress is tracked via `[Topic:Partition] → Partition Metadata`. A reader stops when all assigned partitions reach their recorded high watermarks.

- If a thread is assigned partitions from X different topics, it will create X writers to parallelize writes.
