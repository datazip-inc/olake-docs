...
description: Explore OLake's Kafka source connector—featuring schema discovery, custom group balancing, partition-aware concurrency, and real-time streaming to Apache Iceberg with exactly-once semantics.
...
In the world of data pipelines, Kafka has become the backbone of real-time event streaming—trusted to handle petabytes of data with unmatched reliability. At OLake, we’ve built our Kafka source connector to go far beyond simple ingestion. It’s not just about syncing data; it’s about doing it intelligently.

Our connector brings together schema discovery, global state management, smart partition handling, and rock-solid concurrency controls—engineered specifically for destinations like Apache Iceberg and AWS S3. 

If you're familiar with OLake's technology, you’ll see how OLake connects effortlessly with Kafka’s distributed ecosystem through its flexible, pluggable design. In this post, we'll dissect the thought, rationale, and trade-offs behind our Kafka driver. Expect technical deep-dives, architectural diagrams, and insights into why we chose certain paths—like a custom group balancer or specific fetch sizes. By the end, your Kafka pipeline becomes clear and controllable—no more guessing, just smooth data flow.

## Why Kafka in OLake? A Quick Primer

OLake treats sources like Kafka as "streams" of data, where topics become logical streams with inferred schemas (e.g., JSON payloads augmented with Kafka metadata like offsets and partitions). OLake ingests Kafka topics in real time—no extra streaming tools required—while keeping Iceberg tables fresh with atomic commits and seamless schema evolution.
...
- **Bootstrap Servers**
    ...
    - This is **mandatory**. Without it, OLake does not know where to find the Kafka cluster.
- **Protocol**
    ...
    - The setting dictates the mechanism for authentication (SASL/PLAINTEXT) and encryption (SSL/TLS). This is also a **mandatory** field.
    - There are 3 things that come under Protocol:
        - **Security Protocol**
            - This defines the security level of the connection which is needed by the Kafka cluster. <br/> This is a **mandatory** field to be provided under Protocol. 
...
![Architecture diagram](/img/blog/2025/11/kafka_blog_img.png)

OLake’s abstraction methods wrap the Kafka-specific resources, handling Kafka-based fields and JSON-message schema discovery and CDC-inclined syncing mechanism. 
...
2. **Commit Action: Ensuring Destination Sync Before Kafka Offset Commit**
    ...
    Commit to Kafka consumer group will only take place after the sync of all the partitions for selected topics or streams as per `reader tasks`. Here sync includes writing to provided destination i.e. `Apache Iceberg` or `S3 Parquet` and commit from destination side first, then in Kafka’s side. 
...
4. **Operational Efficiency: Pre-Consumption Analysis and Partition Filtering**
    ...
    Otherwise, if OLake blindly tried to consume all partitions, it would waste CPU cycles, network bandwidth, and memory on empty or already-synced partitions. Without this, partitions could be repeatedly processed, potentially causing endless sync loops. Thread starvation will also be a huge problem, especially when syncing topics with hundreds of partitions — where most might be idle.

5. **Global State Persistence: Managing Consumer Group State Across Topics**
...
### The "Why" Behind Our Custom Group Balancer

Standard Kafka consumer group balancers can distribute partitions unevenly among members in a group. Even in go to libraries for Kafka (in Golang) like SegmentIO’s kafka-go which is very easy to read, understand and code, we faced this problem of not getting the right balancing algorithms which coordinated well with our concurrency methods. We realized the need for more scalable algorithm that stay true to the fundamental design principles, and respected OLake’s in-built concurrency.

This resulted in **OLake’s Custom Group Balancer**.
...
- First, OLake determines the optimal number of reader tasks to create based on the configured concurrency settings (`max_threads` or `threads_equal_total_partitions`) and the number of partitions containing new data across the selected topics or streams.
    ...
    For each stream selected for sync, OLake performs a pre-flight validation on every partition. During this step, it retrieves three key metadata points (`Partition Metadata`) — **the first available offset, the last available offset, and the last committed offset** for the respective consumer group.

- Then OLake creates precisely that many consumer group-based reader instances, each with a unique identifier.

- Our custom balancer, aware of this target number, uses a round-robin strategy to distribute all partitions evenly and exclusively among this limited set of active readers.
...
- Once deployed, a reader task begins its work of fetching, processing, and passing data to the destination. The termination of reading is governed by a dedicated mapping — `[Topic:Partition] → Partition Metadata` — which tracks the read progress of each partition and determines how many partitions are actively being processed versus those that have completed where completion indicates that the reader has consumed data up to the final offset.
...
**Case:** Two Kafka topics are selected for sync to Apache Iceberg, each containing 3 partitions — resulting in a total of 6 partitions. The following dry run demonstrates how varying the `max_threads` concurrency setting affects reader assignment.
...
|-------------|---------------------------------------------|------------|------------------------|------------------------|------------------|------------------|
| 6 | 3 + 3 | None | A1 → Reader-1<br/> A2 → Reader-2 <br/> A3 → Reader-3 | B1 → Reader-4 <br/> B2 → Reader-5 <br/> B3 → Reader-6 | 🟢 Full | Each partition has a unique reader; no reuse; optimal parallelism. |
| 5 | 3 + 2 | 1 ID reused | A1 → Reader-1 <br/> A2 → Reader-2 <br/> A3 → Reader-3 | B1 → Reader-3 (reused) <br/> B2 → Reader-4 <br/> B3 → Reader-5 | 🟢 High | Slight reuse between streams; near-optimal balancing with minor overlap. |
| 4 | 2 + 2 | 2 IDs reused | A1 → Reader-1 <br/> A2 → Reader-2 <br/> A3 → Reader-1 (reused) | B1 → Reader-2 (reused) <br/> B2 → Reader-3 <br/> B3 → Reader-4 | 🟡 Moderate | Balanced reuse; reduced concurrency due to limited threads. |
| 3 | 2 + 1 | 3 IDs reused | A1 → Reader-1 <br/> A2 → Reader-2 <br/> A3 → Reader-1 (reused) | B1 → Reader-2 (reused) <br/> B2 → Reader-3 (reused) <br/> B3 → Reader-3 (reused) | 🟠 Low | High reuse; only 3 threads available; partial overlap across streams. |
| 2 | 1 + 1 | 4 IDs reused | A1, A2, A3 → Reader-1 (reused) | B1, B2, B3 → Reader-2 (reused) | 🔴 Very Low | Heavy reuse; both streams share limited readers. |
| 1 | 1 + 1 | 5 IDs reused | A1, A2, A3 → Reader-1 | B1, B2, B3 → Reader-1 (same) | 🔴 Single-thread | All partitions share one reader; minimal concurrency; full reuse. |
...
During the initialization of reader instances, two key fetch parameters are configured to balance **latency** and **memory efficiency**:
    ...
    - **MinBytes: 1 byte** — Setting the minimum fetch size to one byte ensures that the broker responds immediately whenever data is available. This configuration minimizes latency, making OLake highly suitable for **real-time CDC and streaming workloads** where data freshness is critical.

    - **MaxBytes: 10 MB** — The maximum fetch size is capped at 10 MB to prevent excessive memory usage and mitigate the risk of **out-of-memory (OOM)** scenarios when processing high-throughput topics.

By deliberately configuring `MinBytes` to 1, OLake prioritizes **low-latency data delivery over throughput**, while the `MaxBytes` limit provides a **controlled safeguard** against resource exhaustion—ensuring stable and responsive performance even under heavy load.

## Wrapping Up: Kafka in OLake, Production-Ready

We've built OLake's Kafka source to tame the stream: from secure auth to partition-savvy readers, concurrency that scales with Iceberg, and a CDC loop that knows when to quit. Design decisions—like custom balancing or offset filtering—stem from real pain points: uneven loads, stalled syncs, and resource waste.
