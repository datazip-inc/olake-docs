...
### Q1. How do I deploy OLake on Kubernetes using Helm?
...
### Q2. What services does the OLake Helm chart deploy and what does each do?
...
2. **OLake Worker** — Kubernetes-native engine that creates dedicated pods for each sync job rather than running jobs within itself
3. **Temporal** — Workflow orchestrator that guarantees reliable execution with retries, state persistence, and full execution history
4. **PostgreSQL** — Stores all OLake application state and Temporal workflow state
5. **Elasticsearch** — Provides search and observability into workflow executions, integrated with Temporal's visibility layer
6. **Shared Storage (NFS server)** — Persistent volume for coordination between the UI and worker pods. The chart bundles a development-grade NFS server for quickstart — replace with a ReadWriteMany-capable solution for production
 
 ### Q3. What are the minimum requirements for deploying OLake on Kubernetes?
...
### Q4. How does OLake's Kubernetes deployment handle pipeline failures and retries?
...
### Q3. Can the OLake Helm chart be used in production or only for development?
...
### Q5. Can the OLake Helm chart be used in production or only for development?
