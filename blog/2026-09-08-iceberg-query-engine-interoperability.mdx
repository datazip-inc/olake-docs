...
Say you're in a data engineering team that needs to move an `orders` table out of Postgres and into Iceberg, so the rest of the org can query it as part of a shared lakehouse. You pick an ingestion engine (like [OLake Go](/olake-go/)) or write your own script to do the migration wherein it reads change events straight off Postgres and writes them into Iceberg. You wire it up, point it at cloud storage bucket and your catalog, and let it run.
...
To see which Iceberg capabilities Spark supports (catalog types, DML, merge-on-read, delete formats, and related items) in one place, use the [Apache Spark 3.3+ query engine page](/iceberg/query-engine/spark/) feature matrix and key features list.
...
To see which Iceberg capabilities Databricks supports (Unity Catalog integration, read and write paths, storage strategy, format versions, and related items) in one place, use the [Databricks Runtime 14.3 LTS+ query engine page](/iceberg/query-engine/databricks/) feature matrix and key features list.
...
If you ingest with OLake Go and need to see which query engines work with which catalog, and how each engine handles OLake Go written files, see [Query Engine Compatibility with OLake Iceberg Tables](/docs/understanding/compatibility-engines).
...
If you want an overview of Iceberg's delete file formats and which popular query engines support each one, including Spark and Databricks, see [Iceberg Delete Format Interoperability in OLake Go](/blog/iceberg-delete-formats-interoperability-olake-go/).
...
- [Deep dive into OLake Go architecture](/blog/olake-architecture-deep-dive/)
- [Apache Iceberg vs Delta Lake: Ultimate Guide for Data Lakes](/blog/apache-iceberg-vs-delta-lake-guide/)
- [OLake Go - now an Arrow-based Iceberg Ingestion Tool](/blog/olake-arrow-based-iceberg-ingestion/)
