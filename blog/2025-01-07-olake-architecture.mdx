...
### Q1. What is OLake's core architecture for database-to-lakehouse replication?
...
### Q2. How does OLake perform the initial historical data load for large tables?
...
### Q3. What destinations does OLake currently support for writing replicated data?
 
OLake supports:
 
- **Apache Iceberg** on S3-compatible object storage (AWS S3, MinIO, GCS, Azure Blob) as its primary lakehouse destination
- **Local Parquet files** for development and testing — writes to a local directory or uploads directly to an S3 bucket
...
When writing to Iceberg, OLake registers tables with any supported Iceberg catalog including REST catalogs (Lakekeeper, Tabular), AWS Glue, Hive Metastore, Nessie, Polaris, and Unity Catalog.
 ...
 ### Q4. How does OLake handle schema evolution during replication without data loss?
...
- **Compatible type promotions** (`int → bigint`, `float → double`) are applied automatically using Iceberg v2 widening promotion rules
- **Incompatible type changes** (e.g. `INT → STRING`) are handled by routing the conflicting values to a Dead Letter Queue (DLQ) column, preventing sync failures and keeping downstream models stable without data loss
 
 ### Q5. What makes OLake faster than traditional ETL tools for database replication?
...
- **Apache Arrow as the columnar memory format** — Arrow data is already in columnar layout, so writing to Parquet requires only encoding and compression with no O(n) restructuring cost and no memory copying — just pointer arithmetic
