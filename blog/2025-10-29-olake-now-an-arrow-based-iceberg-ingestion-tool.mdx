...
That's when we decided to turn to **Apache Arrow**.
...
Arrow operates entirely on batches. OLake writes an entire `RecordBatch` in one call — 10,000 rows processed in microseconds.
...
### Now, for Non-Partitioned Tables
...
An **equality delete file** is simply a Parquet file that tells a query engine to mark a row deleted by one or more column values. It is different from a positional delete file, which would also mention the Parquet file location along with the position of rows to skip.
