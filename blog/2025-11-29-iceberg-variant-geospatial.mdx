...
authors: [anshika]
date: 2025-11-29
image: /img/blog/2025/24/variant-geospatial-cover.webp
---
...
```sql
CREATE TABLE IF NOT EXISTS car.sales (
  record Variant
) USING iceberg;

INSERT INTO car.sales 
SELECT json_parse('{ 
    "date" : "2017-04-28", 
...
```sql
SELECT
  get_json_object(CAST(record AS STRING), '$.dealership')           AS dealership,
  get_json_object(CAST(record AS STRING), '$.customer[0].name')     AS customer_name,
  get_json_object(CAST(record AS STRING), '$.vehicle[0]')           AS vehicle_json
FROM car.sales
WHERE get_json_object(CAST(record AS STRING), '$.customer[0].name') LIKE 'Joyce%';
```
...
![Partition transforms for geospatial data](/img/blog/2025/24/partition-transforms.webp)
