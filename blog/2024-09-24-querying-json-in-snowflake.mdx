...
<Faq showHeading={false} data={[
  ...
  },
  {
    ...
    answer: <div>
      ...
      <ul>
        <li><strong><code>TABLE(FLATTEN(...))</code> vs <code>LATERAL FLATTEN(...)</code>.</strong> For single-level arrays, both produce the same result. For nested structures where you chain multiple <code>FLATTEN</code> calls, use <code>LATERAL</code> so each <code>FLATTEN</code> can reference the output of the previous one.</li>
