...
- The average row size is **144 bytes** for `trips` and **121 bytes** for `fhv_trips`.  
- OLake & Debezium were run on  **Azure Standard D64ls v5 VM (64 vCPUs, 128 GiB memory)**
...
| Tool                                               | Rows Synced | Throughput (rows / sec) | Relative to OLake  |
|----------------------------------------------------|-------------|-------------------------|--------------------|
| **OLake** <br/><span className='text-xs text-slate-500'>(as of 14th Nov 2025)</span>         | **4.0 B**  | **1,39,773 RPS**        | –                  |
| Fivetran <br/><span className='text-xs text-slate-500'>(as of 14th Nov 2025)</span>           | 4.0 B      | 73,087 RPS              | **1.91 × slower**   |
