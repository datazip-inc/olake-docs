...
| Tool               |   CDC Window | Throughput (rows / sec) | Relative to OLake |
| ------------------ | -----------: | ----------------------: | ----------------- |
| **OLake Go**          | **26.83 min** |          **31,055 RPS** | –                 |
| Fivetran           |       19.45 min |              42,194 RPS | **1.36 × faster**  |


**Key takeaway:** For incremental workloads Fivetran leads the pack, moving 50 million MSSQL changes into Iceberg **35.8 % faster than OLake Go**

#### 3. Cost Comparison (Vendor List Prices)

| Tool          | Scenario        |                                                                           Spend (USD) |  Rows Synced |
| ------------- | --------------- | ------------------------------------------------------------------------------------: | -----------: |
| **OLake Go**     | Full Load / CDC  | Cost of a `GCP Custom N2 VM (64 vCPUs, 128 GiB memory)` running for 3.21 hours **< $ 11** | 4.0 B / 50 M |
| Fivetran      | Full Load       |                                                                 $ 0 (free full sync) | 4.0 B |
| Fivetran      | CDC             |                                                                 $ 2, 375.80 | 50 M |
...
:::::note
For MSSQL, we performed a full-load and CDC sync of the `trips` and `fhv_trips` tables into Apache Iceberg.
:::::
