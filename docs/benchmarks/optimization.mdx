...
- **Fusion runtime instance:** GCP `e2-standard-8` (**8 vCPUs, 32 GB memory**).
- **Fusion compaction resources:** <br/> Master: GCP `c4a-standard-8` (**8 vCPU, 32 GB RAM**) <br/> Workers: 2 x GCP `c4a-highmem-16` (**16 vCPU, 128 GB RAM each**).
- **Spark compaction resources:** <br/> Master: GCP `c4a-standard-8` (**8 vCPU, 32 GB RAM**) <br/> Workers: 2 x GCP `c4a-highmem-16` (**16 vCPU, 128 GB RAM each**).
...
- **Analytical query execution:** After the first **6 update cycles**, we started running **TPC-H Query 6** on the Iceberg tables; the query was executed **20 times** with a **30-second pause** between consecutive runs.
...
- **Parallel compaction phase:** After completing **10 ingestion cycles**, we started compaction jobs while both CDC-style updates and repeated TPC-H Query 6 executions continued in parallel, specifically to measure how concurrent compaction impacts query latency and runtime stability.

### Apache Spark

Spark compaction was executed using Apache Iceberg's `rewrite_data_files` procedure through a scheduled Spark job.

The compaction job was scheduled with each run triggered at a fixed interval of **15 minutes**, allowing compaction to operate alongside ongoing ingestion activity and repeated analytical query execution.
...
The compaction was executed using Fusion's scheduled self-optimization flow across three compaction levels.
...
- `self-optimizing.target-size`: 512 MB (Target file size after optimization)
- `self-optimizing.max-task-size-bytes`: 512 MB (Maximum data volume assigned to a single optimization task)

### Compaction Results

#### 1. Speed Comparison

This compaction comparison evaluates end-to-end compaction runtime for Apache Spark and OLake Fusion on the same Iceberg workload, providing a direct baseline of total time taken and relative execution speed.

| Engine | Total Compaction Time | Relative to Fusion |
|---|---|---|
| OLake Fusion | 27 mins 2 secs | - |
| Apache Spark | 55 mins 51 secs | **2.07 x slower** |
