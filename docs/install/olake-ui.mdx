...
OLake UI provides a complete Docker Compose stack for running the OLake data replication platform. This containerized deployment includes all necessary services: the main UI for job management, Temporal workflow engine for orchestration, PostgreSQL database for persistence, and other supporting services. The stack can be used for development, testing, evaluation, or production deployments.

## Overview

The Docker Compose setup includes:
- **OLake UI**: Main web interface for job management and configuration (port 8000)
- **Temporal Worker**: Background worker for processing data replication jobs
- **PostgreSQL**: Primary database for storing job data, configurations, and sync state (port 5433)
- **Temporal Server**: Workflow orchestration engine for managing job execution (port 7233)
- **Temporal UI**: Web interface for monitoring workflows and debugging (port 8081)
...
- Ports 8000, 8081, 5433, and 7233 available on the system
...
- `temporal-ui`
- `elasticsearch`
- `signup-init` (will exit after creating admin user) */}
