...
:::warning
Use block storage. The index needs POSIX locking and reliable `fsync`.
:::
...
- **Size increases**: When you increase the `size` field, the PersistentVolumeClaim automatically expands on the next job run. This requires `allowVolumeExpansion: true` to be set on your StorageClass.
