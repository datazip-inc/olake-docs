...
- The following node-class suggestions are **best practices**, not hard requirements. OLake will run on mixed capacity, but scheduling the components below on stable, non-preemptible (on-demand) nodes lowers the chance of spot or preemptible churn interrupting core services and data replication jobs:
