...
:::warning
Use block storage. The index needs POSIX locking and reliable `fsync`, so a shared RWX filesystem (NFS, EFS, Azure Files) is not a supported backing store for it.
:::
