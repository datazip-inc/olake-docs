...
Volumes are only provisioned for jobs where the `index_required` advanced setting is enabled. This setting is automatically turned on when any stream in the job uses positional deletes. Jobs without these features receive no storage.
