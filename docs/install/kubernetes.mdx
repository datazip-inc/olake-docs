...
:::note
Ensure all required images are mirrored to your private registry before deploying. The images that need mirroring are:
...
- `registry.k8s.io/sig-storage/nfs-provisioner:v4.0.8` (if using built-in NFS server, mirrored as `1234567890123.dkr.ecr.us-east-1.amazonaws.com/dockerhub_mirror/sig-storage/nfs-provisioner:v4.0.8`, configured via `nfsServer.image`)
- The main application images defined in `olakeUI.image`, `olakeWorker.image`, `temporal.image` etc.
:::
