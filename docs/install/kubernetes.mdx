...
Volumes are only provisioned when any stream in the job uses positional deletes.
...
| `cacheSizeMB` | `512` | Pebble block cache size per stream. |
| `maxOpenFiles` | `1000` | Meximum number of SSTable file descriptors that pebble db keeps open per stream. |
| `labels` / `annotations` | `{}` | Custom labels and annotations for the Index PVC. |
...
:::warning
Per-job index storage (for positional delete writes) requires the versions below. Upgrade every component, missing an update on single component can cause job failures:
