...
OLake sends anonymous usage telemetry to help improve the product. To opt out, set `TELEMETRY_DISABLED` under `global.env` (so it also reaches the Activity Pods running syncs):
