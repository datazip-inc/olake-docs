...
You can set the `securityContext` for `olakeUI`, `olakeWorker`, `elasticsearch`, `postgresql` and `temporal` in your `values.yaml`.
For eg,
...
The default PostgreSQL image (Alpine-based) used by OLake requires a specific UID/GID to function correctly. If you are enforcing `runAsNonRoot: true`, you **must** set the UID, GID, and FSGroup to **70**.
```yaml
postgresql:
  securityContext:
    runAsUser: 70
