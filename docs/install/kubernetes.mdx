...
When writing Iceberg positional deletes or (deletion vectors coming soon), OLake Go maintains a Pebble index during the write operation. Without persistent storage, this index is lost when the sync pod terminates and must be rebuilt from scratch on the next run. To avoid this rebuild cost, the chart provisions a dedicated PersistentVolumeClaim for each job that needs it.
...
Volumes are only provisioned for jobs where the `index_required` advanced setting is enabled. This setting is automatically turned on when any stream in the job uses positional deletes or deletion vectors. Jobs without these features receive no storage.
...
| Field | Default | Notes |
|-------|---------|-------|
| `size` | `20Gi` | Increasing Only. |
| `storageClass` | `global.storageClass`, then the cluster default | Block storage. |
| `accessModes` | `[ReadWriteOnce]` | Block storage is single-attach |
...
Different fields behave differently when modified:

- **Size increases**: When you increase the `size` field, the PersistentVolumeClaim automatically expands on the next job run. This requires `allowVolumeExpansion: true` to be set on your StorageClass. Expanding of storage happens without deleteing the older indexes.
- **Size decreases**: You cannot decrease the `size` field. Kubernetes does not support shrinking volumes, so any attempt to reduce the size will be rejected.
- **Other configuration fields**: Changes to fields like `storageClass`, `accessModes`, or `mountPath` only apply when creating new PersistentVolumeClaims. These fields cannot modify claims that already exist.
...
:::info
For OLake Fusion related configuration options that are not covered in this page, see the [OLake Fusion Kubernetes installation guide](/docs/fusion/install/kubernetes-compaction.mdx).
:::
