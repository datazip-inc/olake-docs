...
- Verify StorageClass is available and configured correctly
  - Check if there is a default StorageClass
      ...
      ```
  - In case, a non-default StorageClass is preferred, set it under `global.storageClass` in Helm Values file
...
- In case, setup is done with External PostgreSQL Configuration, check the following:
  ...
  - Check if `ssl_mode` is correct for your database in the kubernetes secret for the database
