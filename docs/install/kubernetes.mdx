...
#### Scheduling Fusion Pods (Node Selector, Tolerations, Anti-Affinity)

Fusion scheduling can be controlled using `fusion.nodeSelector`, `fusion.tolerations`, and `fusion.affinity` in the Helm values file.

- `fusion.nodeSelector`: restricts Fusion pods to nodes with specific labels.
- `fusion.tolerations`: allows Fusion pods to be scheduled on tainted nodes.
- `fusion.affinity.podAntiAffinity`: prevents Fusion pods from running on the same node as selected pods.
...
fusion:
  ...
  affinity:
    podAntiAffinity:
      requiredDuringSchedulingIgnoredDuringExecution:
        # Prevent scheduling on nodes already used by other OLake pods.
        - labelSelector:
            matchExpressions:
...
This anti-affinity pattern can be used to keep Fusion and Spark driver/executor pods away from nodes that run other OLake components.
