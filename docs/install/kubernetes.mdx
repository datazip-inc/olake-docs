...
<TabItem value="es" label="with ES (used in versions < v0.0.11)">

**Basic Installation:**

```bash
helm install olake olake/olake --set elasticsearch.enabled=true
```
...
<details>
<summary><b>Migrating to v0.0.11 (Elasticsearch Removal)</b></summary>
...
If installation is currently using Elasticsearch and is upgraded without additional configuration, OLake will **lose visibility of existing job task logs and history**.
...
#### For New Installations (PostgreSQL Visibility)
New installations without Elasticsearch require worker version greater than v0.2.7 to function correctly with PostgreSQL-based visibility storage. Ensure you are pulling the most recent image tags.

#### Post-Upgrade: Resume Stopped Syncs
After upgrading, if jobs appear to be stopped or workers are not responding as expected, perform a rollout restart of the olake-workers:
