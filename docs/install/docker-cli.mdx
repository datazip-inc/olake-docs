...
***What's Next*** : Once the source has been configured, the next step is to define the destination where the ingested data will be written.
...
- [Working with Iceberg REST Catalog](/docs/writers/iceberg/catalog/rest)  

***What's Next*** : Once both source and destination are configured, the next step is to discover available streams.
...
***What's Next*** : Understanding the streams.json file to modify the required fields.
...
| **`partition_regex`** | string        | `"/{dropoff_datetime, year}"` | A pattern defining how to partition the data. To read more, refer the [Partition Regex for S3](/docs/writers/parquet/partitioning#partition-regex-attributes) or [Partition Regex for iceberg](/docs/writers/iceberg/partitioning#partition-spec-syntax-used-by-the-writer)                                             |
...
| **`append_mode`**     | boolean       | `false`                                                                      | Disables upserts in Iceberg when set to `true` .                                                                                             |
...
| **`type_schema`**                | *(JSON object with properties)*                  | Defines the structure of the records in the stream. |
| **`supported_sync_modes`**       | `["full_refresh", "cdc", "incremental","strict_cdc"]`         | Lists the sync modes the stream supports. Includes `"full_refresh"`, `"cdc"`, `"strict_cdc"` and `"incremental"`. To read more about sync modes read ‼️                                                       |
...
| **`cursor_field`**               | `"updated_at"`                                   | Defines the cursor field used to track incremental sync.  To read more about incremental sync read this ‼️                                             |
...
An `olake.log` file is automatically created inside `<TYPE_OF_COMMAND>_<TIMESTAMP_IN_UTC>` (for example, sync_2025-2-17_11-41-40) within the same folder where the configuration files are located.

This file contains the complete snapshot of all logs generated while the command was running.

![logs](/img/docs/features/olake-logs.png)

The console logs appear in the following format:

![logs console](/img/docs/features/logs-console.png)


and has the color specs for easy identification:
