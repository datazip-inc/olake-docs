...
| **`type_schema`**                | *(JSON object with properties)*                  | Defines the columns in streams or tables, including the destination column name. The source column name is normalized by replacing special characters with underscores (`_`) and is used as the final column name in the destination table. |
...
| **`destination_database`**       | `"prefix:my_db"`                                 | Specifies the destination database name where data will be stored. By default, the connector name is used as the prefix, but you can override it using the `--destination-database-prefix` flag. The source namespace is always appended to the database name. All special characters are replaced with (`_`), and the database is automatically created if it doesn’t exist. |                                                                                                      |
| **`destination_table`**          | `"table1"`                                       | Specifies the name of the destination table where the data will be stored. The table name is derived from the source stream name and normalized by replacing special characters with underscores (`_`). |                                                                                                       |
