---
title: "Deploy OLake Optimization on Kubernetes"
description: Deploy OLake Optimization on Kubernetes with Helm. Enable optimization services, configure Spark optimizer settings, and run compaction workflows for Iceberg tables.
sidebar_label: Kubernetes Optimization Installation
sidebar_position: 2
---

# OLake Optimization Kubernetes Installation with Helm

This guide details the process for deploying OLake Optimization on Kubernetes using the official Helm chart. The Fusion components run alongside the standard OLake stack and provide Iceberg table optimization services.
...
- **Fusion Spark Optimizer**: Spark-on-Kubernetes execution layer for optimization jobs
...
Optimization components provide Iceberg table compaction and optimization services. The documentation for optimization can be found [here](https://olake.io/docs/install/kubernetes/#optimization-configuration).

To enable Optimization, set `fusion.enabled` to `true` in the `values.yaml` file:
...
#### Customizing Spark Optimizer Configuration

The Spark optimizer in Optimization can be customized by providing additional Spark configurations via `fusion.optimizer.spark.extraConfig` and other container-specific properties via `fusion.optimizer.spark.properties`.
...
<details>
<summary><strong>Optimization Scheduling</strong></summary>

Configure `fusion.nodeSelector`, `fusion.tolerations`, and `fusion.affinity` in `values.yaml` to define scheduling behavior for Optimization workloads.
