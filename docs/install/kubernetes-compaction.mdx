...
Fusion is by default disabled. To enable either use `--set fusion.enabled=true` or override using `values.yaml` as below:
...
### To enable Fusion on an existing Ingestion-only release:
