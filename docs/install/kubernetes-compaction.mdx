...
This guide details the process for deploying OLake Maintenance on Kubernetes using the official Helm chart. The Fusion components run alongside the standard OLake stack and provide Iceberg table maintenance services.
...
- **Fusion Spark Optimizer (Compactor)**: Spark-on-Kubernetes execution layer for Maintenance jobs
...
:::note
These node-class suggestions are **best practices**, not hard requirements. OLake will run on mixed capacity, but scheduling the components below on stable, non-preemptible (on-demand) nodes lowers the chance of spot or preemptible churn interrupting core services and compaction work:
- nfs-server (if enabled)
- PostgreSQL (if enabled)
- Temporal
- OLake workers
- OLake UI
- Fusion and Spark optimizer (Compactor) workloads
:::
...
**Basic Required Stack Installation (ingestion + platform services):**
```bash
helm install olake olake/olake --set global.storageClass="gp2"
```
This installs the core OLake stack (UI, Worker, Temporal, PostgreSQL, etc.) with default settings. Fusion/compaction is not enabled by default.
...
With Fusion enabled, OLake runs Iceberg maintenance workloads - Lite, Medium, and Full compaction - according to the schedules you set in the UI. See the [Compaction overview](https://olake.io/docs/iceberg-maintenance/compaction/overview/) for what each type does and how they differ.

To enable Compaction, set `fusion.enabled` to `true` in the `values.yaml` file:
...
#### Customizing Spark Optimizer (Compactor) Configuration

The Spark optimizer (compactor) can be customized by providing additional Spark configurations via `fusion.optimizer.spark.extraConfig` and other container-specific properties via `fusion.optimizer.spark.properties`.
...
### Ingestion-Specific Scheduling and Encryption
`JobID-based Scheduling` and `OLAKE_SECRET_KEY` encryption are ingestion-focused configuration options. For details, refer to the ingestion install guide: [Deploy OLake on Kubernetes](https://olake.io/docs/install/kubernetes/).
...
- `olakego/fusion`, `olakego/fusion-spark`
