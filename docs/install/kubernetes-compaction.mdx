...
:::note
The following node-class suggestions are **best practices**, not hard requirements. OLake will run on mixed capacity, but scheduling the components below on stable, non-preemptible (on-demand) nodes lowers the chance of spot or preemptible churn interrupting core services and compaction work:
...
**Ingestion + Platform Services:**
```bash
helm install olake olake/olake --set global.storageClass="gp2"
```
This installs the core OLake stack with default settings. Fusion is not enabled by default.
...
To enable Fusion, set `fusion.enabled` to `true` in the `values.yaml` file:
...
With Fusion enabled, OLake runs Iceberg maintenance workloads — Lite, Medium, and Full compaction — according to the schedules you set in the UI. See the [Compaction overview](https://olake.io/docs/iceberg-maintenance/compaction/overview/) for what each type does and how they differ. To add catalogs, enable maintenance on tables and run your first jobs in the UI, follow [Configure your first compaction](https://olake.io/docs/getting-started/configure-first-compaction).
...
:::note
The ingestion-focused configurations are documented in **[Ingestion Installation with Helm](https://olake.io/docs/install/kubernetes/#configuration-options)**.
:::
