...
### Index DB Setup for Positional Delete Writes
...
2. **Positional deletes** — requires a local Pebble DB index store to track file and row positions.

Configure the index database through environment variables on the `temporal-worker` service in your `docker-compose.yml` file only when you want **positional deletes** as the delete format.

#### Configuration

When using positional deletes, set the following environment variables in the `temporal-worker` service within the docker-compose configuration for [olake-ui](https://github.com/datazip-inc/olake-ui). The values below are defaults and can be adjusted based on your storage and performance requirements:
