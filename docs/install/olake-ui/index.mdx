...
:::tip Want Ingestion + Maintenance in one setup?
OLake bundles both capabilities together. Get started with a single command — [see the setup here](/docs/fusion/getting-started/quickstart/#one-command-setup).
:::
...
OLake Go supports encryption of source and destination configurations stored in the database. Configure the encryption key in `docker-compose.yml`:

- **Custom String**: Provide any string (OLake Go generates SHA-256 hash):
...
OLake Go supports using an **external PostgreSQL instance** instead of the built-in Postgres service included in the docker-compose file. This PostgreSQL stores all job data, configurations, sync state, and Temporal workflow data.

Within the [compose file](https://raw.githubusercontent.com/datazip-inc/olake-ui/master/docker-compose-v1.yml), an extension field named `x-db-envs` defines all database configuration values used by OLake Go and Temporal. These values can be modified to point to the external PostgreSQL instance.
...
OLake Go sends anonymous usage telemetry to help improve the product. To opt out, set `TELEMETRY_DISABLED` under `x-envs.shared`:
...
If you are pulling images from a private or self-hosted registry, add the registry settings to the shared environment block used by the compose stack.

First, log in to the registry from a terminal:

```bash
docker login registry.your-company.com -u 'robot$username' -p 'ABCD'
```

Then add the following variables under `x-envs.shared` in the compose file:

```yaml
x-envs:
  shared: &sharedEnvs
    CONTAINER_REGISTRY_BASE: "registry.your-company.com"
    CONTAINER_REGISTRY_USERNAME: "<username>"
    CONTAINER_REGISTRY_PASSWORD: "<password>"
    CONTAINER_REGISTRY_INSECURE: "<true/false>"
    CONTAINER_REGISTRY_TLS_SKIP_VERIFY: "<true/false>"
    CONTAINER_REGISTRY_CA_CERT: "<PEM-encoded-CA-certificate>"
```

These values are used by the compose stack when resolving image pull credentials and registry TLS settings.

### Data Persistence

The Docker Compose setup creates the following data storage:

#### OLake Go Config Directory
...
### Log Retention
OLake Go includes an automated log retention system that helps manage disk space by automatically cleaning up old log files. This prevents log files from accumulating indefinitely and consuming excessive storage space.
...
OLake Go includes a configurable retention period for Temporal visibility data that controls how long job and sync history are stored and visible in the system. This helps manage database storage by automatically cleaning up old workflow execution history.
...
### Index DB Setup for Positional Delete/Deletion Vector Writes

When writing in upsert mode, OLake Go supports two Iceberg delete formats:

1. **Equality deletes:**  the default. No index DB setup is required.
2. **Positional deletes/Deletion Vector:**  requires a local Pebble DB index store to track file and row positions.

Configure the index database through environment variables on the `temporal-worker` service in your `docker-compose.yml` file only when you want **positional deletes or deletion vector** as the delete format (upsert type).

#### Configuration

When using positional deletes or deletion vector, set the following environment variables in the `temporal-worker` service within the docker-compose configuration for [olake-ui](https://github.com/datazip-inc/olake-ui). The values below are defaults and can be adjusted based on your storage and performance requirements:
...
services:
  temporal-worker:
    environment:
      ...
      OLAKE_INDEX_DB_DIR: /index-db
      OLAKE_INDEX_DB_CACHE_SIZE: 512  # in MB
      OLAKE_INDEX_DB_MAX_OPEN_FILES: 1000  # max open files
```

- **`OLAKE_INDEX_DB_DIR`** (string): File path where indexes are stored inside the driver container.
- **`OLAKE_INDEX_DB_CACHE_SIZE`** (integer): LRU cache size for the Pebble DB, in megabytes (MB).
- **`OLAKE_INDEX_DB_MAX_OPEN_FILES`** (integer): Maximum number of files Pebble DB keeps open while reading from the index store. 

:::warning
Positional deletes, deletion vector and the index DB environment variables above require the versions mentioned below. Upgrade every listed service, missing an update on single component can cause job failures:
...
:::info
For OLake Fusion related configuration options that are not covered in this page, see the [OLake Fusion Docker installation guide](/docs/fusion/install/olake-ui/#service-configuration).
:::
