...
<Tabs>
  ...
  <TabItem value="fusion" label="Optimization">
    ...
    **Note**: Your data and configurations will be preserved as they are stored in persistent volumes and the `olake-data` directory. The same `ENABLE_OPTIMIZATION="true"` and `--profile fusion` flags apply on `down` and `up` so Fusion profile services stop and start consistently.
  </TabItem>
