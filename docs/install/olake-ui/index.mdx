...
#### Configuration
Set the `TEMPORAL_RETENTION_PERIOD` environment variable in the `temporal` service within the docker-compose configuration for [olake-ui](https://github.com/datazip-inc/olake-ui) to control how long visibility data is retained:
...
services:
  temporal:
    environment:
      temporal_retention_period: "168" #Keeps data visible for 7 days (by default)
```
...
The retention period is specified in **hours** as a numeric string. Only job history and sync history within the configured retention period will be visible in the OLake UI and Temporal UI interfaces.
