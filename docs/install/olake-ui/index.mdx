...
{() => {
  ...
  const configHeadings = new Set([
    'service-configuration', 'changing-admin-credentials',
    'updating-olake-ui-version', 'encryption-key-configuration',
    'customizing-data-directory', 'external-postgresql-configuration',
    'service-environment-variables', 'data-persistence',
    'olake-go-config-directory', 'docker-volumes',
    'log-retention', 'what-it-does', 'configuration', 'configuration-1',
    'monitoring-log-cleaner', 'temporal-retention-period'
  ]);

  const installHeadings = new Set([
    'prerequisites', 'quick-start', 'one-command-setup',
  ...
  React.useEffect(() => {
    const patch = () => {
      ...
      tocLinks.forEach(link => {
        ...
        if (!hash) return;
        if (configHeadings.has(hash)) {
          link.setAttribute('href', `?setup-mode=configuration#${hash}`);
        } else if (installHeadings.has(hash)) {
          link.setAttribute('href', `?setup-mode=installation#${hash}`);
        }
      });
    };

    // Run immediately and after a short delay (TOC may render late)
    patch();
