...
Configure the index database through environment variables on the `temporal-worker` service in your `docker-compose.yml` file only when you want **positional deletes** and in the case of **equality deletes** no index DB setup is required.
