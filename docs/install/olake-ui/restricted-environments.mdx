---
title: Restricted Environments (AWS)
description: OLake UI installation in air-gapped and restricted network environments on AWS
sidebar_position: 1
---

# OLake UI for Restricted Environments (AWS)

OLake UI can be deployed in restricted or air‑gapped AWS environments by using an Amazon ECR pull‑through cache to mirror required Docker images. This guide outlines how to configure the pull-through cache, pre‑pull connector images, and run OLake UI.
## Prerequisites
...
## 5. Configure VPC Endpoints for Isolated Environments
...
- Ensure VPC endpoints are properly set up for isolated environments
...
For assistance with restricted environment deployments:
