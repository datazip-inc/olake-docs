...
- All contributions must target the OLake Go [staging branch](https://github.com/datazip-inc/olake/tree/staging). Create feature branches from staging and open PRs against staging.
...
For every pull request, please record and attach a short video demo of your change. This helps reviewers quickly understand the behavior and verify that it works end-to-end with OLake Go, and also ensures contributors actually run OLake Go with their changes.

Your video should:

- Quickly walk through the relevant code changes. 
- Execute a sync using OLake Go that exercises those changes.
