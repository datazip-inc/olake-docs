...
- You'll create `source.json` and `destination.json` configuration files in the steps below.
:::
...
- ### Sync with State enabled
    ...
    2. **Update Record**: Update 1 record in the source database and observe the corresponding update in the destination table. Run the following command:
       ...
       docker exec -it primary_postgres psql -U main -d main -c "UPDATE public.sample_data SET num_col = 150, str_col = 'First record updated' WHERE id = 1;"
        ```
        Run the sync command with state (as shown above), then execute the SQL query on `localhost:8888`.
    ...
    "configurations": [
        {
            ...
            "args": [
                "sync",
                "--config",
                "{PATH_TO_UPDATE}/source.json",
                "--catalog",
                "{PATH_TO_UPDATE}/streams.json",
                "--destination",
                "{PATH_TO_UPDATE}/destination.json",
                // "--state",
                // "{PATH_TO_UPDATE}/state.json",
            ]
        }
    ]
}
