...
To use Full Refresh as the sync mode, add `"sync_mode": "full_refresh"` in `streams.json` for the table you are syncing.

For the full refresh, run the sync command mentioned below:

```bash
./build.sh driver-postgres sync --config $(pwd)/source.json --catalog $(pwd)/streams.json --destination $(pwd)/destination.json
```

After this initial sync completes, a `stats.json` file is automatically generated. To know more about the `stats.json` file, refer to the [Stats File](/docs/install/docker-cli/#stats) section.

</TabItem>

<TabItem value="incremental" label="Incremental Sync">

**Incremental Sync** is an operation that only syncs new records based on cursor fields. You configure a primary cursor field (and optionally a secondary cursor field) in your `streams.json`. On the first run, a full refresh occurs automatically. After that, OLake Go only syncs records where the cursor field value is greater than the maximum value stored from the previous sync. 

:::note
Any modifications or deletions to existing records in the source will **not** be reflected in the destination.
:::

To use Incremental as the sync mode, add `"sync_mode": "incremental"` in `streams.json` for the table you are syncing.

Run the first sync command without the `--state` flag enabled because only after the first sync, the `state.json` file is generated.

```bash
./build.sh driver-postgres sync --config $(pwd)/source.json --catalog $(pwd)/streams.json --destination $(pwd)/destination.json
```

After this initial sync completes, a `state.json` and `stats.json` file are automatically generated. The `state.json` file contains the necessary resume tokens and metadata that OLake Go uses for Incremental sync operations. Essentially, it tells OLake Go from where to resume or start the next sync. To know more about the `stats.json` file, refer to the [Stats File](/docs/install/docker-cli/#stats) section.

Run the sync commands with the `--state` flag enabled so that OLake Go can resume from the last sync point:
...
**CDC (Change Data Capture)** is an operation that tracks and replicates all changes made in the source database to the destination. It checks for any inserts, updates, or deletes in the source and performs similar changes in the destination, ensuring the destination stays in sync with the source.
...
To use CDC as the sync mode, add `"sync_mode": "cdc"` in `streams.json` for the table you are syncing.
...
Run the sync command with the `--state` flag enabled so that OLake Go can resume from the last sync point:
