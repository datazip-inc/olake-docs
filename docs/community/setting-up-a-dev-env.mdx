...
- **Parquet destination**  
  - OLake writes data as **plain Parquet files** directly into a bucket in **MinIO** (an S3-compatible object storage running locally).
...
<YouTubeEmbed videoId="IcAJmW72d2A" className="max-w-6xl" />
...
This guide explains how to test changes made in the `olake-cli` repository within the `olake-ui` environment. This is useful for testing new features, or JSON schema changes

#### 1. Build Custom Images

Before running the tests, build custom Docker images for the driver under test.

Execute the following commands from the root of the `olake-cli` repository on your current working branch.
...
# Build destination image for MySQL
docker build \
  --build-arg DRIVER_NAME=mysql \
  -t olakego/destination-mysql:<version_tag> \
  .
```
...
Set the following environment variables to the `olake-ui` service in the `docker-compose.yml` file:
