...
Watch this comprehensive video tutorial that walks you through the entire process of setting up OLake Go, syncing data, and debugging with a live demonstration:
...
Before setting up and running OLake Go, ensure you have all the following installed on your system:
...
The diagram below illustrates the high-level architecture of OLake Go, showing how data flows from various sources to different destinations:

![OLake Go data flow from MongoDB, Postgres, MySQL sources to Iceberg and Parquet destinations](/img/community/setting-up-a-dev-env/olake_diagram.webp)

<details>
<summary>How this local OLake Go architecture works</summary>

The diagram represents a **complete local OLake Go pipeline** that you can run on your laptop using Docker Compose and the OLake CLI.
...
In the **middle of the diagram** is **OLake Go**, which acts as the **replication and transformation engine**:
...
On the **right side of the diagram** are the **analytics destinations**, where OLake Go writes the data:

- **Iceberg destination**  
  - OLake Go writes rows into **Apache Iceberg tables**.  
  ...
  - OLake Go handles creating and updating Iceberg table files and metadata, so you can query these tables via the **Spark + Iceberg** service that is part of this local setup.

- **Parquet destination**  
  - OLake Go writes data as **plain Parquet files** directly into a bucket. In local mode, the bucket is in **MinIO** (an S3-compatible object storage running locally), and in real deployments it can use **Amazon S3** or any S3-compatible storage.
...
Putting it together, the image shows that **OLake Go sits between your transactional databases (Postgres/MySQL/MongoDB) and your analytical storage (Iceberg tables or Parquet files on MinIO/S3)**.  
You can swap sources and destinations just by changing the Docker profile and your `source.json`/`destination.json`, while OLake Go consistently takes care of **moving and organizing the data from source to lakehouse**.
</details>

### Fork and Clone

Clone the OLake Go repository and navigate to the project directory:
...
## 2. Source and Destination Setup

OLake Go needs two things to test data replication locally: a **source database** to read data from, and a **destination** to write data to. We'll use Docker Compose to spin up both, and OLake provides convenient Makefile commands to manage these services.

All commands in this section should be run from the repository root directory (`olake/`).

### Starting a Source Database

Let's start with a source database. We'll use Postgres as an example, but you can use any supported source.

**Start Postgres:**

```bash
make olake.postgres.start
```

This command starts a Postgres container via Docker Compose and waits until it's ready to accept connections. Once it completes, you'll have a running Postgres instance ready for testing.

**Other source databases:**

OLake supports multiple sources. Use the same pattern to start any of them:

- **MongoDB**: `make olake.mongodb.start`
- **MySQL**: `make olake.mysql.start`
- **Kafka**: `make olake.kafka.start`
- **Oracle**: `make olake.oracle.start`
- **MSSQL**: `make olake.mssql.start`
- **DB2**: `make olake.db2.start`
- **S3**: `make olake.s3.start`

### Starting the Destination Stack

Now let's start the destination services. OLake Go writes data to Iceberg tables, and for local testing, we need a complete Iceberg stack.

**Start the destination stack:**

```bash
make olake.destination.all.start
```

This command starts multiple services that work together to create a local Iceberg environment:
- **MinIO**: Object storage (S3-compatible) where your data files will be stored
- **Iceberg REST Catalog**: Metadata service that tracks your Iceberg tables
- **Spark Connect**: Query engine for testing and validating your Iceberg tables
- **Related services**: Supporting infrastructure for Iceberg and Parquet testing

Once this completes, you'll have a fully functional local data lakehouse ready to receive data.

### Starting Everything at Once

If you want to start both source and destination services together, use:

```bash
make olake.all.start
```

This starts all source stacks plus the destination stack in one command. It's the fastest way to get a complete local environment running.

### Managing Your Services

Once your services are running, you'll need to manage them during development. Here are the most common lifecycle commands:

| Action | Source (Postgres) | Destination |
|--------|-------------------|-------------|
| **Start** | `make olake.postgres.start` | `make olake.destination.all.start` |
| **Stop** (keep data) | `make olake.postgres.stop` | `make olake.destination.all.stop` |
| **Wipe + restart** | `make olake.postgres.refresh` | `make olake.destination.all.refresh` |
| **Teardown** (remove completely) | `make olake.postgres.teardown` | `make olake.destination.all.teardown` |

:::tip
- Use **stop** when you want to pause services but keep your data intact
- Use **refresh** when you want a clean slate—this wipes all data and restarts from scratch
- Use **teardown** when you're done and want to free up system resources
:::

### Discovering More Commands

OLake's Makefile includes many more targets for building, testing, and managing services. To see the full list:

```bash
make help
```

This will show you all available commands, including:
- Start/stop/teardown/restart/refresh for all sources
...
## 3. Commands to run OLake Go code
...
The sync command is used to sync data from the source to the destination. OLake Go supports three different sync modes:
...
After this initial sync completes, a `state.json` and `stats.json` file are automatically generated. The `state.json` file contains the necessary resume tokens and metadata that OLake Go uses for CDC (Change Data Capture) or Incremental sync operations. Essentially, it tells OLake Go from where to resume or start the next sync.

</TabItem>

<TabItem value="incremental" label="Incremental Sync">

**Incremental Sync** is an **append-only operation** that syncs only new records based on cursor fields. You configure a primary cursor field (and optionally a secondary cursor field) in your `streams.json`. On the first run, a full refresh occurs automatically. After that, OLake Go only syncs records where the cursor field value is greater than the maximum value stored from the previous sync. 
...
To learn how Incremental sync works in OLake Go, including how it tracks changes and resumes from previous sync points, watch the following video:
...
This ensures that OLake Go continues from where it left off, only syncing new or changed data since the last sync operation.
...
To learn how CDC sync works in OLake Go, including how it tracks changes and resumes from previous sync points, watch the [comprehensive video tutorial](#setup-video-tutorial) above.
...
- If you want to know more about OLake Go generated variables like `op_type`, check out the [OLake Go Generated Columns](/docs/understanding/terminologies/olake#olake-generated-columns) section.
:::
...
### A Deep Dive into How to Debug OLake Go:
