...
:::info
In the above docker-compose.yml file, containers for all the sources are provided, which also includes the data-loading in respective source. \
...
If want to use Hive Catalog for Iceberg as destination, use: `docker compose -f {docker compose file path} --profile hive up -d`
...
- ### Sync
    ...
    SELECT * FROM {catalog_name}.{destination_database_name}.{table_name};
    ``` 
    ...
    1. Inserting 2 records in the source database and confirm that both the records are replicated in the destination table.
    2. Updating 1 record in the source database and observing corresponding update in the destination table.
    3. Deleting 2 records in the source database and confirming the records reflect the deletion in the destination.
    ...
    - `"d"` for deletions (with other row columns as NONE)
