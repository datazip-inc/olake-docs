...
This section describes all commands available in the OLake CLI as of [v0.2.3](https://github.com/datazip-inc/olake/releases/tag/v0.2.3).
...
- **SOURCE-NAME** → The source driver being used.
...
```
./build.sh driver-[SOURCE-NAME] check [FLAG]
```
#### Description:
...
### 2. Spec
```
./build.sh driver-[SOURCE-NAME] spec [FLAG]
```
#### Description:
...
When no flag is provided, the spec of the defined `SOURCE-NAME` is generated.
...
### 3. Discover
```
./build.sh driver-[SOURCE-NAME] discover [FLAG]
```
...
- [**`--destination-database-prefix`**](/docs/community/commands-and-flags#7-destination-database-prefix) → Adds a custom prefix to the destination database name.
    ...
    ```
       If the source database is sales-db and the driver is mysql:
        - **Default** → mysql_sales_db
        - **With prefix** → olake_sales_db
        - **After normalization (special characters replaced with _)** → olake_sales_db
...
### 4. Sync
    ```
    ./build.sh driver-[SOURCE-NAME] sync [FLAG]
    ```
    #### Description:
...
``` 
**Description:** 
- Clears data in the destination only for the streams defined in `streams.json`.
...
- Adds a custom prefix to the database name created in the destination.<br/>
    Example:
    - Without prefix → mysql_sales_db
    ```
    ./build.sh driver-[SOURCE-NAME] [COMMAND] --destination-database-prefix olake
    ```
    - With `--destination-database-prefix olake` → olake_sales_db
