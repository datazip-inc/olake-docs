...
<Head>
  {[
    ...
    },
    {
      ...
      keywords: 'OLake docs, installation, Apache Iceberg, connectors, architecture, CDC, Iceberg maintenance, optimisation, Glue, Trino, Athena, setup guide'
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
...
**OLake** is a blazing-fast open-source platform for both data replication and Iceberg table maintenance, moving data from diverse sources into Apache Iceberg and Parquet while keeping those tables optimised for real-time lakehouse analytics, without the pain of ETL scripts or vendor lock-in.
...
- Optimise storage costs and query performance.
...
- **Iceberg table maintenance** → Keep lakehouse tables healthy after ingestion with scheduled optimisation, better query performance, and lower storage costs.  
...
OLake is not limited to replication: it includes **Iceberg table maintenance** so you can configure and run optimisation workflows in the same product you use for CDC and ingestion.

- **[Catalogs](/docs/iceberg-maintenance/catalogs)** — Register and validate Iceberg catalogs (including OLake-managed catalogs from ingestion) so tables are available for maintenance workflows.  
- **[Optimisation](/docs/iceberg-maintenance/optimisation/overview)** — Configure scheduled **minor**, **major**, and **full** optimisations to reduce small files, merge delete files, and keep query performance predictable as data grows. Details: [Optimisation configuration](/docs/iceberg-maintenance/optimisation/configuration).  
- **[Runs & Logs](/docs/iceberg-maintenance/runs-and-logs)** — Inspect each maintenance run and open task-level logs for troubleshooting.  
- **[Metrics](/docs/iceberg-maintenance/metrics)** — Review table and run metrics to see file counts, sizes, and the effect of each optimisation.  

Start with the [Iceberg Table Maintenance overview](/docs/iceberg-maintenance/overview) or walk through [Configure your first optimisation](/docs/getting-started/configure-first-optimisation).
