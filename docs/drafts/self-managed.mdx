...
- MySQL Server 5.6 or higher (preferably 5.7 or 8.0 for better JSON and GTID support, etc.) installed on a server you control.
...
1. **Enable Binary Logging in MySQL Configuration:** Open the MySQL configuration file (often `/etc/my.cnf` or `/etc/mysql/mysql.conf.d/mysqld.cnf` on Linux). Under the `[mysqld]` section, add or modify the following:
   ...
   - (Optional) If using GTIDs: `gtid_mode = ON`, `enforce_gtid_consistency = ON` (also requires a restart).
...
2. **Create a Replication-Privileged User:** Connect to MySQL as root:
   ...
   - Optionally, grant `LOCK TABLES` if your CDC tool might use locks for snapshot consistency:
     ```sql
     GRANT LOCK TABLES ON *.* TO 'cdc_user'@'%';
     ```
     Some tools might want to lock tables or do flush tables (which would require RELOAD). Evaluate based on tool:
     - Debezium avoids global locks; it uses REPEATABLE READ.
     - If using a tool like `mysqldump` for initial load then tail binlog, it might use locks.
...
4. **Connect the CDC Connector:** On your CDC application (Debezium, Maxwell, etc.), configure the connection:
   ...
   - If you enabled GTID and want Debezium to use GTID, set `gtid.source.enabled=true` (Debezium 1.5+).
...
- **MySQL won’t start after enabling binlog:** If MySQL fails to start or gives error on startup after config change:
  ...
  - If you set GTID options incorrectly (like turning GTID on in a system with existing binlogs not using GTID), you might get errors. For a fresh enable, set `gtid_mode=ON` and `enforce_gtid_consistency=ON` together and ensure `log_bin` is on. If you have existing data, you might need to set `gtid_mode=ON,OFF_PERMISSIVE` first for a transition – beyond scope if not needed.
