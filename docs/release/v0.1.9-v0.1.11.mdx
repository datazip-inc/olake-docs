...
2. **Incremental Sync: MySQL and PostgreSQL -** <br/>Added incremental synchronization for MySQL and PostgreSQL. This adds change‑only replication for both the sources so OLake transfers new/updated documents since the last run, reducing latency and data volume for recurring pipelines.
