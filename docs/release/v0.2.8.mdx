...
1. **Clear destination in Iceberg (CLI only)-** <br/> Introduced a clear destination feature for Iceberg (for CLI only) that enables selective stream cleanup. This feature deletes all the streams that were selected from the destination for that particular job. It’s useful when stream configurations like normalization, filters, or partitions are misconfigured and needs to be changed and a fresh sync is needed.
