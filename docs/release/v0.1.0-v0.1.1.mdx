...
2. **Strict CDC Sync Mode -** <br/>Adds a new mode that applies only change events and skips any full‑refresh backfill during syncs, guaranteeing CDC‑only behavior. This reduces load on sources/targets and avoids accidental re‑snapshots in continuous pipelines.
