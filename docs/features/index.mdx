...
Parallel chunking is a technique that breaks down large datasets or collections into smaller virtual chunks, which can then be read and processed in parallel.
...
OLake ensures that data syncs **resume automatically from the last checkpoint** after interruptions. (Applicable for Full Refresh + CDC and Full Refresh + Incremental sync modes)
...
Data Filter allows you to replicate only the specific data you need during full refreshes. This reduces load on the source database, saves storage and processing resources, and helps downstream queries run faster and more efficiently.
...
Data deduplication ensures you store and process only what's unique — saving space, cost, and improving data quality.

### 2. Hive Style Partitioning

This feature allows data to be organized using both **Iceberg-native partitioning rules and AWS S3-style partitioning**.
...
OLake automatically handles changes in your table's schema without breaking downstream jobs.
...
Append Mode continuously adds new data to existing destination tables without altering or deleting existing records, preserving historical data and ensuring consistent, reliable analytics

### 5. DLQ

Dead Letter Queue (WIP)
