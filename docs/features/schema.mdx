...
<Tabs queryString="handling-incompatible-data-types" groupId="handling-incompatible-data-types">
  ...
  <TabItem value="parquet" label="Parquet">
    ...
    **BOOLEAN** → **INT32** → **INT64 (LONG)** → **FLOAT64 (DOUBLE)** → **STRING**

    **FLOAT32** → **FLOAT64 (DOUBLE)** → **STRING**

    This means a type can be converted to any type on its right. For example:
    - **BOOLEAN** can be converted to **INT32**, **INT64 (LONG)**, **FLOAT64 (DOUBLE)**, or **STRING**
    - **INT32** can be converted to **INT64 (LONG)**, **FLOAT64 (DOUBLE)**, or **STRING**
    - **INT64 (LONG)** can be converted to **FLOAT64 (DOUBLE)** or **STRING**
    - **FLOAT32** can be converted to **FLOAT64 (DOUBLE)** or **STRING**
    - **FLOAT64 (DOUBLE)** can be converted to **STRING**
  </TabItem>
</Tabs>
...
- OLake Go discovers the renamed table as a **new** table in the streams list on the next run
- The table must be explicitly selected again for sync
- Once selected, OLake Go creates a new destination table for the new name
- The old destination table keeps its previous name and is not populated with new data
...
When a column data type is `INT32` and it encounters `INT64(LONG)`, when an `INT64(LONG)` column encounters `FLOAT64(DOUBLE)`, or when a `FLOAT32` column encounters `FLOAT64(DOUBLE)`:
...
- **BOOLEAN → INT32**
- **BOOLEAN → INT64 (LONG)**
- **BOOLEAN → FLOAT64 (DOUBLE)**
- **BOOLEAN → STRING**

When a column data type is `BOOLEAN` and it encounters `INT32`, `INT64(LONG)`, `FLOAT64(DOUBLE)`, or `STRING`:
