...
<Tabs queryString="handling-incompatible-data-types" groupId="handling-incompatible-data-types">
  ...
  <TabItem value="parquet" label="Parquet">
    For Parquet destinations, OLake supports widening type conversions in this order:
...
<Tabs queryString="unsupported-data-type-conversions" groupId="unsupported-data-type-conversions">
  ...
  <TabItem value="parquet" label="Parquet">
    The data type changes listed below are unsupported in Parquet and OLake. Attempting these will result in sync failure.
...
- OLake automatically detects the new table on the next scheduled run and lists it in the UI
- The table is synced only when you explicitly enable it for the job
- Once enabled, OLake creates the destination table and starts syncing using your configured sync mode
