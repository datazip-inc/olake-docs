...
{() => {
  ...
  React.useEffect(() => {
    const patch = () => {
      ...
      // Top-level tab anchors — detected from TOC order
      const tabRoots = { 'trino': 'trino', 'presto': 'presto', 'spark': 'spark' };
      ...
      tocLinks.forEach(link => {
        ...
        // If this is a tab root heading, update currentTab context
        if (tabRoots[hash]) {
          currentTab = tabRoots[hash];
        }

        // Patch the href with the correct ?engine= param
        if (currentTab) {
          link.setAttribute('href', `?engine=${currentTab}#${hash}`);
        }
      });
    };
