...
OLake Go Playground is a self-contained environment for exploring lakehouse architecture using [Apache Iceberg](/iceberg/why-iceberg). It comes preconfigured with all the required components, allowing you to experience the complete workflow without manual setup.
...
- **OLake Go**    – Schema discovery and CDC ingestion via an intuitive UI  
...
<TOCTabLinker
  ...
  headingMap={{
    'trino': [
      'trino', 'architecture', 'prerequisites', 'configuration--set-up',
      '1-start-the-demo-stack', '2-accessing-services', '3-query-in-trino',
      'service-validation', '1-view-logs', '2-check-service-status', '3-verify-data-load', '4-test-trino-connection',
      'troubleshooting', '1-trino-cant-connect-to-iceberg', '2-mysql-connection-issues', '3-minio-access-issues',
      'cleanup',
    ],
    'presto': [
      'presto', 'architecture-1', 'prerequisites-1', 'configuration--set-up-1',
      '1-start-the-demo-stack-1', '2-accessing-services-1', '3-query-in-presto',
      'service-validation-1', '1-view-logs-1', '2-verify-data-load', '3test-presto-connection',
      'troubleshooting-1', '1-presto-cant-connect-to-iceberg', '2-mysql-connection-issues-1', '3-minio-access-issues-1',
      'cleanup-1',
    ],
    'spark': [
      'spark', 'architecture-2', 'prerequisites-2', 'configuration--set-up-2',
      '1-start-the-demo-stack-2', '2-accessing-services-2', '3-query-data-with-spark',
      'option-1-using-spark-sql-shell', 'option-2-using-jupyter-notebook',
      'troubleshooting-2', '1-spark-cant-connect-to-iceberg', '2-minio-access-issues-2',
      'cleanup-2',
    ],
  }}
/>
...
└─────────────┘    └─────────────┘    └─────────────┘
    ...
    **Step 3: Navigate and start services**
    ...
    3. Create and Configure a Job:
        ...
        - Set up the Destination:
          ...
          - **S3 Endpoint (for Iceberg data files written by OLake Go workers):** `http://host.docker.internal:9090`
          - **AWS Region:** `us-east-1`
          - **S3 Access Key:** `minio`
          - **S3 Secret Key:** `minio123`
          ![Setting up Destination Configuration](/img/docs/getting-started/playground/destination-config.webp)
        - On the Jobs page, click on the **"Create Job"** button. Give your job a name and select a replication frequency. Then select the **source** and **destination** configurations that you created in the previous steps. Once done click on the **Next** button.
    ...
    Run Queries via Trino CLI:

    Replace `{job_name}` with the name of the job you set when creating the job in OLake Go.

    ```bash
    docker exec -it olake-trino-coordinator trino \
      --catalog iceberg --schema {job_name}_weather \
    ...
    - Ensure the data pipeline in OLake Go has run successfully
    ...
    - Check MinIO credentials in docker-compose-v1.yml match OLake Go destination config
    ...
    **Step 2: Stop base OLake Go stack**
    ...
    **Step 3: Navigate and start services**
    ...
    3. Create and Configure a Job:
        ...
        - Set up the Destination:
          ...
          - **Iceberg REST Catalog URL:** `http://host.docker.internal:8181`
          - **Iceberg S3 Path:** `s3://warehouse/weather/`
          - **S3 Endpoint (for Iceberg data files written by OLake Go workers):** `http://host.docker.internal:9000`
          - **AWS Region:** `us-east-1`
          - **S3 Access Key:** `minio`
          - **S3 Secret Key:** `minio123`
          ![Setting up Destination Configuration](/img/docs/getting-started/playground/destination-config.webp)
