...
Enable developers to experiment with an end-to-end, Iceberg-native lakehouse in minutes. Simply run a single [Docker Compose](/docs/getting-started/setup-olake) `docker-compose up` command to launch the full stack — no service stitching, no configuration files required.
