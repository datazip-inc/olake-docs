...
- Replicate only data where `emp_id` >= `10` (basically replicate data of employees with id 10 and above).
...
To replicate only data of employees having id 10 and above, we'll use a **Data Filter** to filter the data based on the `emp_id` column.
