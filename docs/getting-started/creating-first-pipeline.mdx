...
- For the ingestion mode, select **Upsert** and choose **Equality Deletes**/**Position Deletes**/**Deletion Vectors** as the delete format. See [Upsert](/docs/understanding/terminologies/olake/#upsert) for what Upsert does and how the delete formats differ.
    :::important Delete Format Changes
    - Switching from **equality deletes** to **positional deletes** or **deletion vectors** on existing jobs is supported. The new format applies to subsequent syncs. 
    - However, switching back from **positional deletes/deletion vector** to **equality deletes** on existing jobs is **not supported** and will trigger **clear destination** operation.
