...
Navigate to **Jobs** section and select **+ Create Job** button in the top right corner. This opens the Job creation wizard, starting with the **Schedule Job** step.
...
Here, you can choose your preferred [sync mode](/docs/understanding/terminologies/olake#2-sync-modes) and configure [partitioning](/docs/writers/parquet/partitioning) and [Destination Database](/docs/understanding/terminologies/olake)  as well as other stream-level settings here.
...
- Destination Database can be updated if needed.
...
To update the **Destination Database**, click on the Destination Database field (either Iceberg DB or S3 Folder) and configure it.
