...
sidebar_label: Configure Your First Ingestion Job
sidebar_position: 3
---

# Get Started With First Ingestion Job!

This guide is for end-users who want to replicate data between the various sources and destinations that OLake Go supports. Using the OLake UI, you can configure a **source**, set up a **destination**, and create a **job** to move data between them.

By the end of this tutorial, you’ll have a complete replication workflow running in OLake Go.
...
A job in OLake Go is a pipeline that defines how data should be synchronized from a **source** (where your data comes from) to a **destination** (where your data goes).
...
- If you don't have a source or destination set up yet but still want to try OLake, head over to the [OLake Playground](https://olake.io/docs/getting-started/playground/) for a ready to use sandbox environment.
:::
...
Once the test connection succeeds, OLake Go shows a success message, and by clicking on destinations button it takes you to the destination configuration step.
...
For this guide, select **Apache Iceberg** from the connector dropdown, and keep the **OLake Go version** set to the latest stable version.
...
Once the test connection succeeds, OLake Go shows a success message and by clicking on create job button it takes you to the job configuration step. 
...
- **Iceberg**
  ...
  | Nessie Catalog | [Config](/docs/writers/iceberg/catalog/rest?rest-catalog=nessie#configuration-2) |
  | LakeKeeper | [Config](/docs/writers/iceberg/catalog/rest?rest-catalog=lakekeeper#configuration-1) |
  | S3 Tables | [Config](/docs/writers/iceberg/catalog/rest?rest-catalog=s3-tables#configuration-3) |
  | Polaris | [Config](/docs/writers/iceberg/catalog/rest?rest-catalog=polaris#configuration-5) |
  | Unity | [Config](/docs/writers/iceberg/catalog/rest?rest-catalog=unity#configuration-4) |
  | BigLake | [Config](/docs/writers/iceberg/catalog/rest?rest-catalog=biglake#configuration-6) |
...
In **Advanced Settings**, you can optionally select a **Target Query Engine**. This filters the delete format options shown in the Streams page to only those supported by your chosen query engine. To learn more about Target Query Engine, refer to the [Target Query Engine](/docs/getting-started/job-level-properties/#2-target-query-engines) section.
...
Here, you can choose your preferred [sync mode](/docs/understanding/terminologies/olake#2-sync-modes) and configure [partitioning](/docs/writers/parquet/partitioning) and [Destination Database](/docs/understanding/terminologies/olake/#destination-database) as well as other stream-level settings here.
...
- Replicate the `employees` stream (name of the table).
- Use [**Full Refresh + CDC**](/docs/understanding/terminologies/olake/#2-sync-modes) as the sync mode.
- Enable **data Normalization**.
- For the ingestion mode, select **Upsert** and choose **Equality Deletes**/**Position Deletes**/**Deletion Vectors** as the upsert type. See [Upsert](/docs/understanding/terminologies/olake/#upsert) for what Upsert does and how the delete formats differ.
    :::important Upsert Type Changes
    - Switching from **equality deletes** to **positional deletes** or **deletion vectors** on existing jobs is supported. Subsequent syncs use the new format, and OLake Go builds indexes to migrate existing equality delete files to the new format. 
    - Switching back from **deletion vector** to **equality deletes** on existing jobs will trigger **clear destination** operation because this is an Iceberg version downgrade (v3 to v2).
    - Switching between **positional deletes** and **deletion vector** on existing jobs is **not supported** and will trigger **clear destination** operation.
    :::
- Modify Destination Database name (if required).
- Replicate only data where `id` >= `10` (basically replicate data of employees with id 10 and above).
- Partition the data by the **company** extracted from a string column in the selected stream.
- Run the sync every day at 12:00 AM.

Let's start by selecting the `employees` stream (or any stream from your source) by checking its checkbox to include it in the replication.
Click the stream name to open the stream-level settings panel on the right side.
In the panel, set the **sync mode** to [**Full Refresh + CDC**](/docs/understanding/terminologies/olake/#2-sync-modes), and enable **Normalization** by toggling the switch on.
...
In our case, the `employees` stream has a string column named `company`, which we will partition by **company**. Learn more about partitioning in the [Partitioning Guide](/docs/writers/parquet/partitioning).
...
To replicate only data of employees having id 10 and above, we'll use a **Data Filter** to filter the data based on the `id` column.
...
To edit the **Destination Database** name, select the edit icon beside the [Destination Database](/docs/understanding/terminologies/olake/#destination-database) (Iceberg DB or S3 Folder) and make the changes.
...
You can verify the sync status by checking the last run status column. Possible statuses include **Running**, **Failed**, and **Completed**.
You can also monitor the sync logs by selecting [**Job Logs and History**](/docs/getting-started/job-level-properties/#8-job-logs--history) from the job options menu.
