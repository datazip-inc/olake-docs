...
- For the ingestion mode, select **Upsert** and choose **Equality Deletes**/**Position Deletes**/**Deletion Vectors** as the upsert type. See [Upsert](/docs/understanding/terminologies/olake/#upsert) for what Upsert does and how the delete formats differ.
    ...
    - Switching back from **positional deletes/deletion vector** to **equality deletes** on existing jobs will trigger **clear destination** operation.
    - Switching between **positional deletes** and **deletion vector** on existing jobs will trigger **clear destination** operation.
    :::
