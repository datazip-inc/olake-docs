...
### 2. Target Query Engines

The **Target Query Engine** setting allows you to specify which query engine(s) you plan to use to read the Iceberg tables after data is synced. This is an optional dropdown available in the **Advanced Settings** during job configuration.

**Purpose:**  
Different query engines support different row-level delete formats in Apache Iceberg. By selecting your target query engine(s), OLake Go automatically resolves the delete format (upsert type) options shown in the Streams page to only those supported by your chosen engine(s). This prevents configuration mismatches where data is written in a format your query engine cannot read.

**How it works:**

- **Single query engine selected:** The Streams page shows only the delete formats (upsert types) supported by that engine.
  
- **Multiple query engines selected:** The Streams page shows only the common delete formats (upsert types) supported by **all** selected engines (the intersection of supported formats).

- **No query engine selected:** All three delete format (upsert type) options (Equality Deletes, Positional Deletes, and Deletion Vectors) are available for selection.

:::caution No Common Format
If you select multiple query engines that do not share any common delete format (upsert type), you will receive an error: **"No common delete file format (upsert type) is supported by the selected query engines."** In this case, you must either reduce your selection or choose a different combination of engines.
:::

For a complete list of which query engines support which delete formats, see [Delete Format Support by Query Engine](/docs/understanding/compatibility-engines/#delete-format-support-by-query-engine).
