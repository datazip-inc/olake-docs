...
**Purpose:**  
Different query engines support different row-level delete formats in Apache Iceberg. By selecting your target query engine(s), OLake Go automatically resolves the upsert type options shown in the Streams page to only those supported by your chosen engine(s). This prevents configuration mismatches where data is written in a format your query engine cannot read.

**How it works:**

- **Single query engine selected:** The Streams page shows only the upsert types supported by that engine.
  
- **Multiple query engines selected:** The Streams page shows only the common upsert types supported by **all** selected engines (the intersection of supported formats).

- **No query engine selected:** All three upsert types options (Equality Deletes, Positional Deletes, and Deletion Vectors) are available for selection.

:::caution No Common Format
If you select multiple query engines that do not share any common upsert type, you will receive an error at the streams page: **"No common delete file (upsert type) is supported by the selected query engines."** In this case, you must either reduce your selection or choose a different combination of engines.
:::
