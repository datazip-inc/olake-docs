...
**Purpose:**  
Different query engines support different row-level delete formats in Apache Iceberg. By selecting your target query engine(s), OLake Go automatically filters the delete format options shown in the Streams page to only those supported by your chosen engine(s). This prevents configuration mismatches where data is written in a format your query engine cannot read.

**How it works:**

- **Single query engine selected:** The Streams page shows only the delete formats supported by that engine.
  
- **Multiple query engines selected:** The Streams page shows only the common delete formats supported by **all** selected engines (the intersection of supported formats).

- **No query engine selected:** All three delete format options (Equality Deletes, Positional Deletes, and Deletion Vectors) are available for selection.

:::caution No Common Format
If you select multiple query engines that do not share any common delete format, you will receive an error: **"No common delete file format is supported by the selected query engines."** In this case, you must either reduce your selection or choose a different combination of engines.
:::
