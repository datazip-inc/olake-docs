...
Thread Count (or Concurrency) is the number of parallel processes or threads used to read, transform, and write data simultaneously. Increasing concurrency often speeds up ingestion and improves throughput, but it can also put additional load on the source system. In OLake, this is managed using the --max thread count (or a similar flag), which defines how many chunks or streams are processed in parallel.

## 7. Writer
A Writer in OLake is the component responsible for formatting and writing data from the source to the chosen destination, such as local Parquet files, Amazon S3 with Iceberg tables, Snowflake, or BigQuery. By writing directly from the source to the destination, OLake removes the need for an intermediate data queue, which reduces latency and simplifies the overall pipeline.

## 8. Parquet
Parquet is a columnar storage file format designed for efficient data compression and encoding, making it highly optimized for analytical workloads. In OLake, data is stored in Parquet format so that downstream systems like Spark, Trino, or Snowflake external tables can query it efficiently, improving both performance and storage utilisation.
...
## 13. Catalog
A catalog in Apache Iceberg is the metadata and namespace service that manages Iceberg tables. It acts as the central registry where tables are created, organized, and discovered . It stores table metadata locations (not the data itself) and provides a namespace structure (like databases & schemas in SQL). It also facilitates engines like Spark, Flink, Trino, Hive to consistently find and query Iceberg tables
...
## 19. Golang (Go)
Golang (Go) is a programming language created by Google, known for being very fast and good at handling many tasks at the same time. In data engineering, it's often used to build tools and services that move and process data, such as ETL pipelines, real-time streaming systems, and microservices. Go is also popular because it works well with cloud storage (like AWS S3) and databases, making it a strong choice for modern data platforms like OLake.
...
## 26. Debezium
Debezium is an open-source tool that captures database changes in real time (CDC – Change Data Capture). It works with databases like MySQL, PostgreSQL, and MongoDB, and sends those changes to systems like Kafka.

**Example:** A company can use Debezium to track updates in a MySQL database and stream them to Kafka so analytics systems always have the latest data.

## 27. Polymorphic (or Heterogeneous) Data
...
## 28. Table Format (e.g., Apache Iceberg, Delta Lake, Hudi)
...
## 33. Orchestration (e.g., Airflow, Luigi, Dagster)
...
## 36. Aggregation / Analytical Query
