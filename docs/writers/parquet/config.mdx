...
### Using AWS Amazon S3 Credentials
:::info
- For more information on the S3 config, refer to this [`here`](#s3-configuration-definition).
:::
<S3Config/>
...
For more information, refer to this [`here`](#creation-on-hmac-keys).
:::
...
- Using Docker
  :::note
  If using docker, mounted path has to be used. `"local_path": "/mnt/config"`, where "/mnt/config" is the local directorwhere Parquet files wily inside the Docker container l be stored. This path is mapped to your host file system via a Docker volume.
  :::
  <DockerParquetConfig/>
