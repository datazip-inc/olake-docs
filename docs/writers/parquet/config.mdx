...
<Tabs queryString="olake-s3">

<TabItem value="OLake-UI" label="OLake UI S3 config" default>

### S3 UI configuration definition
<S3ConfigUIDetails/>
---

:::tip[Option 1 — OLake UI]

## Create an S3 Destination in OLake UI

:::

Steps to get started with S3 Destination using OLake UI. Assuming OLake UI runing locally [`localhost:8000`](localhost:8000) or using [`http://ui.olake.io/`](http://ui.olake.io/)
...
OLake supports direct syncing of data from source driver to AWS S3 using Amazon-powered AWS S3 credentials.\
For this, refer to [IAM permission](permission.mdx) needed for Amazon-powered AWS S3.
User needs to provide,
- AWS S3 bucket path
- AWS S3 access key and secret key
- AWS S3 region
- AWS S3 path (if any)
- AWS S3 endpoint (no required as configured internally but can be given. Refer to [`S3 Endpoint Doc`](https://docs.aws.amazon.com/general/latest/gr/s3.html))
...
For role based permissions, refer to [IAM permission](permission.mdx).
...
- Create Service Account HMAC keys, which will have an access key and corresponding secret key.
...
<TabItem value="OLake-CLI" label="OLake CLI S3 config" default>

### S3 CLI configuration definition
<S3ConfigDetails/>
---
:::tip[Option 2 — OLake CLI]
## Create an S3 Destination in OLake CLI
:::
...
- Sample command for discover and sync:
  - Docker: 
     ```bash
...
### Using AWS Amazon S3 Credentials
:::info
- For more information on the keys and value of S3 config, refer to this [`here`](#s3-cli-configuration-definition).
:::
<S3Config/>
...
For more information on creation of HMAC keys, refer to this [`here`](/docs/writers/parquet/config?olake-s3=OLake-UI#creation-of-hmac-keys).
:::
