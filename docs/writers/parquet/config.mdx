...
5. Click on `Create ->`.
6. OLake Go will test the destination connection and display the results. If the connection is successful, you will see a success message. If there are any issues, OLake Go will provide error messages to help you troubleshoot.


This will create a S3 destination in OLake Go, now you can use this destination in your [Jobs Pipeline](../../jobs/overview) to sync data from any [Source](../../connectors/overview) to [AWS S3](../../writers/parquet/s3).
...
OLake Go supports direct syncing of data from source to AWS S3 using Amazon S3 credentials.\
...
OLake Go supports writing data to Google Cloud Storage (GCS) in parquet format. 
...
- Copy the request endpoint and provide it as the S3 endpoint in OLake Go.
...
OLake Go supports S3-compatible MinIO as well in its S3 destination configuration.
