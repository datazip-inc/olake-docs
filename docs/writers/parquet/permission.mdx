...
## AWS (Amazon Web Services) IAM Policy 
...
## GCS (Google Cloud Storage) IAM Policy 

To successfully sync source data using GCS-compatible S3, ensure that GCS IAM policy grants at least minimal permission for listing, creating, reading, and writing to S3 bucket.

### Using GCloud CLI
- Create a project, service account in GCS. Install Google Cloud SDK (gcloud CLI). Initialize the SDK using `gcloud init` command.
