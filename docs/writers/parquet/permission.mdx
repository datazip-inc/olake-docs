...
To successfully sync source data using GCP-compatible S3, ensure that GCS IAM policy grants at least minimal permission for listing, creating, reading, and writing to S3 bucket.
...
- Create a GCS bucket and HMAC keys for the service account for whom the above role is defined. 
    :::info
    For more information on HMAC keys, refer to this [`here`](/docs/writers/parquet/config?olake-s3=OLake-UI#creation-of-hmac-keys).
    :::
