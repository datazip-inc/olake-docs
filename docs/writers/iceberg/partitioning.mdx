...
OLake allows seamless writing into **partitioned Iceberg tables**, supporting all built-in transformations in **Apache Iceberg™**. Partitioning improves query performance by organizing data into logical groups, enabling efficient data pruning during queries.
...
<br></br>
Together, you can add it as `/{field_name, transform}` in the **Partition regex** field.
...
> **Special case:** You may use `now()` as a pseudo-column that evaluates to the writer's current timestamp (useful when your records lack a suitable time field).
