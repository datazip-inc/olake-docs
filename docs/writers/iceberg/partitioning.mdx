...
```text
/{tenant_id, bucket[128]}/{event_date, day}
```
...
```json
{
  ...
  "partition_regex": "/{tenant_id, bucket[128]}/{event_date, day}"
}
```
...
```
tenant_id_bucket=057/event_date_day=2025-04-24/part-00211.parquet
```
...
WHERE  tenant_id = 'acme-corp'
  AND  event_date = DATE '2025-04-24';
```


### 4  IoT device telemetry
**Partition Spec**
...
### 5  Modernising a Hive `yyyy/mm/dd` layout
**Partition Spec**
...
### 6  GDPR / CCPA expiry window (day-level)
**Partition Spec**
...
### 7  Pure identity on low-cardinality code
**Partition Spec**
...
### 8  Hierarchical calendar (year → month → day)
**Partition Spec**
...
### 9  Arrival-day + hash bucket (late data)
**Partition Spec**

```text
/{ingest_ts, day}/{record_id, bucket[128]}
```
...
```json
{
  "stream_name": "late_pipeline",
  "partition_regex": "/{ingest_ts, day}/{record_id, bucket[128]}"
}
```
...
```
ingest_ts_day=2025-04-24/record_id_bucket=77/part-00042.parquet
```
...
WHERE  ingest_ts_day = DATE '2025-04-24';
```


### 10  Prefix truncate on long strings
**Partition Spec**
...
### 11  Composite “device-day”
**Partition Spec**

```text
/{device_id, bucket[64]}/{event_ts, day}
```
...
```json
{
  "stream_name": "chatty_devices",
  "partition_regex": "/{device_id, bucket[64]}/{event_ts, day}"
}
```
...
```
device_id_bucket=12/event_ts_day=2025-04-24/part-00891.parquet
```

**Query**

```sql
DELETE FROM chatty.devices
WHERE  device_id = 'dev-a1b2'
  AND  event_ts_day = DATE '2025-04-24';
```


### 12  Boolean flag shield
**Partition Spec**
...
### 14  Delete-after window (month-level)
**Partition Spec**
...
```json
{
  "stream_name": "retention_monthly",
  "partition_regex": "/{delete_after, month}"
}
```
...
```
delete_after_month=2025-05/part-00009.parquet
```

**Query (clean-up)**

```bash
hadoop fs -rm -r .../delete_after_month=2025-05
```
