...
| **`year`** | Time-series data where queries slice whole years | Extracts the calendar year as an integer | `/{created_at, year}` | `created_at = 2025-04-24 15:42` → `created_at_year=2025/` |
| **`month`** | Monthly reporting, rollups, retention | Extracts year-month as YYYY-MM | `/{created_at, month}` | `created_at = 2025-04-24 15:42` → `created_at_month=2025-04/` |
| **`day`** | Daily dashboards, 30-day TTL jobs | Extracts the date as days since epoch (1970-01-01) | `/{event_date, day}` | `event_date = 2025-04-24` → `event_date_day=2025-04-24/` |
| **`hour`** | High-volume logs where analysts drill into single hours | Extracts timestamp truncated to hour | `/{event_time, hour}` | `event_time = 2025-04-24 15:42` → `event_time_hour=2025-04-24-15/` |
...
> * `identity` and `truncate[N]` keep original data types
> * `year` produces integers, while `month`, `day`, and `hour` produce formatted strings for readability
> * `bucket[N]` produces integers (0 to N-1)
> * All transforms maintain type consistency for efficient partition pruning
...
```
event_time_hour=2025-04-24-15/user_id_bucket=344/data-00001.parquet
```
...
```
order_date_month=2025-04/ship_country=IN/data-00037.parquet
```
...
```
event_date_month=2025-04/device_id_bucket=22/data-00003.parquet
```
...
### 4. Modernizing a Hive `yyyy/mm/dd` Layout
**Partition Spec**
...
```
order_ts_year=2025/order_ts_month=2025-04/order_ts_day=2025-04-24/data-99983.parquet
```
...
```
delete_after_day=2025-05-24/data-00017.parquet
```
...
```
status=active/data-00009.parquet
```
...
```
event_ts_year=2025/event_ts_month=2025-04/event_ts_day=2025-04-24/data-00571.parquet
```
...
```
domain=exam/data-00001.parquet          # holds rows like 'example.com'
```
...
```
is_test=true/data-00005.parquet
```
...
```
schema_version=v3/update_ts_month=2025-04/data-00077.parquet
```
