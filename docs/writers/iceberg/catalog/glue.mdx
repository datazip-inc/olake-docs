...
OLake Go integrates with AWS Glue Catalog to provide full support for **Apache Iceberg tables**.
...
**Click `Next ->`** to test the connection and verify that OLake Go can validate both Glue Catalog and S3 access.
...
- <span >Database does not exist in Glue Catalog</span>
  ...
  - Fix: OLake Go will automatically create the database if you have `glue:CreateDatabase` permissions. Verify permissions or create manually:
    ```bash
...
- <span >Table already exists with different schema</span>
  ...
  - Fix:
    ...
    - Drop and recreate table if schema change is intended:
      ...
      ```
    - Or use OLake Go's schema evolution capabilities for compatible changes
