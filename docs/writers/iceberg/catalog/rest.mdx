...
```
  ...
  <TabItem value="horizon" label="Horizon">
    ...
    - Snowflake account with Horizon Catalog and Iceberg REST access enabled
    - Snowflake database to use as the Iceberg warehouse
    - Snowflake role with privileges to create and write Iceberg tables in the target database
    - Session role scope for the Snowflake role OLake Go should use (for example, `session:role:HORIZON_ROLE`)
    - Cloud storage region for your account (run `SELECT CURRENT_REGION();` in Snowflake)

    For Horizon setup, prerequisites, and IAM configuration, refer to the [Snowflake Horizon Catalog documentation](https://docs.snowflake.com/en/user-guide/tables-iceberg-access-using-external-query-engine-snowflake-horizon).
