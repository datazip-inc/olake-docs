...
<Tabs queryString="rest-catalog" groupId="rest-catalog">
  ...
  **Permissions**: 
    - REST Catalog service user requires **read/write permissions** to object store buckets used by Iceberg tables.
    - **Database Permissions**: Catalog user must have **full DDL/DML privileges** on the Iceberg metadata database/tables.
  ...
  - <p className="text-blue-500 text-lg"> Your authentication credentials are invalid ... unauthorized_client</p>
    - **Resolution**:
  ...
  - <p className="text-blue-500 text-lg"> User: `<ARN>` is not authorized to perform: sts:AssumeRole </p>
    - **Resolution**:
  ...
  <TabItem value="lakekeeper" label="Lakekeeper">
    ...
    **Permissions**:
    ...
    <Tabs>
      ...
      <TabItem value="nessie-cli" label="OLake CLI">
        ...
        ```json title="destination.json"
        {
          ...
          "writer": {
            "catalog_type": "rest",
              "rest_catalog_url": "http://<REST_HOSTNAME>:19120/iceberg/",
    ...
    - <p className="text-blue-500 text-lg">Invalid Reference (Branch/Tag not found)</p>
      - **Resolution**:
        - Ensure the branch or tag (e.g., `main`) exists before running operations like sync or queries.
        - Re-issue new tokens or refresh secrets if expired.
    ...
    - <p className="text-blue-500 text-lg"> Commit Conflicts </p>
      - **Resolution**:
        - If multiple writers update the same table, Nessie may reject the commit. Retry after pulling the latest state.
...
<Tabs>
  ...
  - <p className="text-blue-500 text-lg">Permission denied (AccessDenied)</p>
    - **Resolution**:
      - Ensure your AWS IAM user or role has the following permissions on the target bucket and namespace:
        - `s3tables:*`
        - `s3:*`
      - Verify bucket policies or resource-based policies do not block access.

  - <p className="text-blue-500 text-lg">Invalid or expired credentials</p>
    - **Resolution**:
      - Double-check your AWS Access Key ID and Secret Access Key.
      - Confirm that temporary credentials (if using roles or sessions) have not expired.
      - Refresh or rotate keys as necessary.

  - <p className="text-blue-500 text-lg">Incorrect REST Catalog URI</p>
    - **Resolution**:
  ...
  <TabItem value="polaris" label="Apache Polaris">
    ...
    **Required permissions**:
    ...
    - **IAM Role and Trust Policy**  
      ...
      - Include permissions like `s3:ListBucket`, `s3:GetObject`, `s3:PutObject`, and `s3:DeleteObject`.
    ...
    **Important Notes:**
    ...
    - <p className="text-blue-500 text-lg">Invalid OAuth2 credentials (401 Unauthorized)</p>
      - **Resolution**:
        - Ensure you’re using the correct Polaris catalog client credentials (`olake_user` client), not root or other unrelated credentials.
        - Verify that your `client_id`, `client_secret`, and `scope` match the Polaris configuration.
      
    - <p className="text-blue-500 text-lg">Permission Denied on S3 Bucket</p>
      - **Resolution**:
        - Check that the IAM Role ARN attached to the Polaris catalog has the correct bucket policy permissions, including:
          - `s3:GetObject`
          - `s3:PutObject`
          - `s3:DeleteObject`
          - `s3:ListBucket`
    ...
    - <p className="text-blue-500 text-lg">Connection Errors to Polaris REST API</p>
      - **Resolution**:
