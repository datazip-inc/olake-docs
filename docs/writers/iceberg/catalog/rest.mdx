...
<Tabs queryString="rest-catalog" groupId="rest-catalog">
  ...
  **Permissions**: 
  ...
  <Tabs queryString="tab">
        ...
        <TabItem value="rest-cli" label="OLake CLI">
  ...
  - <span> User: `<ARN>` is not authorized to perform: sts:AssumeRole </span>
      ...
      **Permissions**:
      ...
      <Tabs queryString="rest-auth-type" groupId="lakekeeper-rest-auth">
        <TabItem value="none" label="None" default>
          <Tabs queryString="tab">
            <TabItem value="lakekeeper-ui" label="OLake UI" default>
            <br/>
            ...
            <TabItem value="lakekeeper-cli" label="OLake CLI">
              Create a `destination.json` with the following configuration:
    ...
    services:
      ...
      minio:
        image: olakego/minio:RELEASE.2025-04-03T14-56-28Z
        container_name: minio
        environment:
      ...
      mc:
        depends_on:
          - minio
        image: olakego/mc:RELEASE.2025-04-03T17-07-56Z
        container_name: mc
        networks:
          iceberg_net:
        environment:
  ...
  <TabItem value="polaris" label="Apache Polaris">
    ...
    **Required permissions**:
    ...
    <Tabs queryString="rest-auth-type" groupId="polaris-rest-auth">
      ...
      <TabItem value="polaris-ui" label="OLake UI" default>
      <br/>
      ...
      <TabItem value="polaris-cli" label="OLake CLI">
...
```
  ...
  :::note BigLake Catalog Supported for v0.9.1 and above
  BigLake catalog support is available in OLake Go version **0.9.1** and above.
  :::
...
```
  ...
  <TabItem value="horizon" label="Horizon">
    ...
    OLake Go supports [Snowflake Horizon Catalog](https://docs.snowflake.com/en/user-guide/tables-iceberg-access-using-external-query-engine-snowflake-horizon) as an Iceberg REST catalog destination. Configure External OAuth, key-pair authentication or programmatic access tokens (PAT) to authenticate OLake Go to the Horizon REST endpoint.
    
    :::note Horizon Catalog Supported for v0.11.0 and above
    Horizon catalog support is available in OLake Go version **0.11.0** and above.
    :::
    ...
    - **Database**: A Snowflake database where OLake Go will create Iceberg tables. The database must have its catalog and external volume configured as SNOWFLAKE and SNOWFLAKE_MANAGED respectively:
      ...
      :::note
      If you use external storage (AWS, GCP, Azure) instead of Snowflake-managed storage, set `EXTERNAL_VOLUME` to the name of your external volume being used.
      :::
    - **Role**: A Snowflake role with database usage privileges and permission to create schemas, assigned to the user OLake Go will use for authentication. If the database uses an external volume other than `SNOWFLAKE_MANAGED`, this role must also have access to that external volume.
