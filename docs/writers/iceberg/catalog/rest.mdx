  ...
  - <span> User: `<ARN>` is not authorized to perform: sts:AssumeRole </span>
    ...
    - Check using this command:
      ...
      **Required services**:
      ...
      - **Object Store** – e.g., AWS S3, MinIO, or any S3-compatible storage backend for table data and snapshots.  
      ...
      <Tabs queryString="tab">
        <TabItem value="rest-ui" label="OLake UI" default>
          ...
          **Lakekeeper Configuration Parameters:**
          ...
          |----------------------|-------------------------|-----------------------------------------------------------------------------|
          | **REST Catalog URI** | `http://<LAKEKEEPER_ENDPOINT>:8181` | Endpoint for the Lakekeeper REST catalog service.                           |
        ...
        <TabItem value="jdbc-cli" label="OLake CLI">
          ...
          ```json title="destination.json"
          {
            ...
            "writer": {
              "catalog_type": "rest",
              "rest_catalog_url": "http://<LAKEKEEPER_ENDPOINT>:8181",
          ...
          **Lakekeeper Configuration Parameters:**
          ...
          | **rest_catalog_url** | `http://<LAKEKEEPER_ENDPOINT>:8181` | Endpoint for the Lakekeeper REST catalog service.                           |
    ...
    services:
      lakekeeper:
        ...
        environment:
          LAKEKEEPER__PG_DATABASE_URL_READ: jdbc:postgresql://postgres:5432/lakekeeper
          LAKEKEEPER__PG_DATABASE_URL_WRITE: jdbc:postgresql://postgres:5432/lakekeeper
          LAKEKEEPER__PG_DATABASE_USER: lakekeeper
          LAKEKEEPER__PG_DATABASE_PASSWORD: password
          LAKEKEEPER__PG_ENCRYPTION_KEY: This-is-NOT-Secure!
          LAKEKEEPER__S3_ENDPOINT: http://minio:9090
          LAKEKEEPER__S3_REGION: us-east-1
          LAKEKEEPER__S3_ACCESS_KEY: admin
          LAKEKEEPER__S3_SECRET_KEY: password
          LAKEKEEPER__S3_BUCKET: warehouse
        ports:
          - 8181:8181
        depends_on:
          postgres:
    ...
    - <span>Invalid Database Connection</span>
      - Fix:
        ...
        - Test with:
          ```bash
          psql -h localhost -U lakekeeper -d lakekeeper -c "\dt"
          ```
    ...
    - <span>Lakekeeper REST API not reachable</span>
      - Fix:
        - Confirm the service is listening on port 8181:
          ```bash
          curl http://localhost:8181/api/v1/config
          ```
        - Check Docker network connectivity or firewall rules.
