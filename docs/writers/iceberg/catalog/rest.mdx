...
<TOCTabLinker
  ...
  headingMap={{
    ...
    'biglake':    ['prerequisites-6', 'configuration-6', 'troubleshooting-6'],
  }}
/>
      ...
      <Tabs queryString="tab">
        <TabItem value="rest-ui" label="OLake UI" default>
          ...
          **Lakekeeper Configuration Parameters:**
          ...
          | **S3 Path**  | `<WAREHOUSE_NAME>`       | The name of the warehouse you created in Lakekeeper                 |
    ...
    <Tabs queryString="tab">
      ...
      <TabItem value="nessie-ui" label="OLake UI" default>
        ...
        **Nessie Configuration Parameters:**
        ...
        | **Catalog Name**  | `olake_iceberg`                  | Enter a name for your catalog.                                           |
        | **S3 Path**  | `s3://<BUCKET_NAME>`                    | Determines the S3 path or storage location for Iceberg data in the warehouse bucket. <br/>Example: `s3://warehouse` for the local testing |
        | **S3 Endpoint**      | `http://<YOUR_MACHINE_IP>:9000/`        | Endpoint for the MinIO S3-compatible service. <br/>**Linux:** Use machine's IP (e.g., `http://192.168.1.100:9000/`) <br/>**macOS:** Use `http://host.docker.internal:9000/` |
        | **AWS Access Key**   | `<S3_ACCESS_KEY>`                       | MinIO access key for authentication (Optional). <br/>Default from docker-compose: `minio` |
        | **AWS Secret Key**   | `<S3_SECRET_KEY>`                       | MinIO secret key for authentication (Optional). <br/>Default from docker-compose: `minio123` |
        | **AWS Region**       | `<S3_REGION>`                           | Specifies the AWS region associated with the S3 bucket where the data is stored. <br/>Example: `us-east-1` |
        | **Enable Arrow Writes**      | `false`                            | Writes data and delete files using Apache Arrow based writer and registers them in Iceberg.                                       |
...
<Tabs queryString="tab">
  ...
  <TabItem value="s3tables-ui" label="OLake UI" default>
      ...
      **S3 Tables Configuration Parameters:**
      ...
      | **Catalog Name**  | `olake_iceberg`                  | Enter a name for your catalog.                                           |
      | **S3 Path**  | `arn:aws:s3tables:<REGION>:<ACCOUNT_ID>:bucket/<BUCKET_NAME>` | Determines the S3 Tables ARN for the bucket and namespace. |
  ...
  **Required permissions**:
  ...
  ```
      ...
      **BigLake Configuration Parameters**
      ...
      | **iceberg_s3_path**  <br/> `required`| **Single-bucket catalog:**<br/>`gs://<BUCKET_NAME>`<br/><br/>**Multiple-bucket catalog:**<br/>`bl://projects/<PROJECT_ID>/catalogs/<CATALOG_ID>` | Name of the BigLake catalog to use, OLake Go uses this to look up the storage configuration (e.g. Cloud Storage buckets) already defined in BigLake.<br/><br/>**Single-bucket catalog:** Use the format `gs://BUCKET_NAME`. This configuration restricts your catalog to a single bucket and locks the catalog name to the bucket name.<br/><br/>**Multiple-bucket catalog (recommended):** Use the format `bl://projects/PROJECT_ID/catalogs/CATALOG_ID`. This configuration lets your catalog associate multiple buckets and lets you name your catalog independently of any bucket name.                                                |
