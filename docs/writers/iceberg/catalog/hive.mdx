...
<HiveIcebergWriterCLIConfigDetails/>
 ...
 - How to create a job in OLake UI: see [OLake UI guide](/docs/getting-started/creating-first-pipeline#3-configure-destination)
...
For local development and testing, you can set up a complete Hive Metastore environment using Docker Compose. This setup includes Hive Metastore, PostgreSQL, MinIO (S3-compatible storage), and Spark with Iceberg support.
...
And then, we are good to go for creating a job
...
- <span style={{ color: '#1a73e8' }}>Hive Metastore JAR Dependencies Missing</span>
...
- <span style={{ color: '#1a73e8' }}>Database Backend Connection Failed</span>
...
- <span style={{ color: '#1a73e8' }}>S3/Object Storage Access Denied</span>
...
- <span style={{ color: '#1a73e8' }}>Path Style Access Error with MinIO</span>
