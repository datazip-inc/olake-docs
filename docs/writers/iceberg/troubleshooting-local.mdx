...
{() => {
  ...
  const troubleshootingHeadings = new Set([
    'troubleshooting',
  ...
  const localTestingHeadings = new Set([
    'local-testing'
  ]);
  ...
  React.useEffect(() => {
    const patch = () => {
      ...
      tocLinks.forEach(link => {
        ...
        if (troubleshootingHeadings.has(hash)) {
          link.setAttribute('href', `?view=troubleshooting#${hash}`);
        } else if (localTestingHeadings.has(hash)) {
          link.setAttribute('href', `?view=local#${hash}`);
        }
      });
    };
