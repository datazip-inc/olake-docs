...
A read-only secondary replica allows only `SELECT` operations and blocks `INSERT`, `UPDATE`, and `DELETE` statements. User can use such a secondary database for OLake sync to offload read traffic from the primary database.
