...
:::info
CDC change tables retain events for **3 days** by default (configurable via `sys.sp_cdc_change_job`). Reducing retention raises the risk of losing unconsumed events during downtime; if events expire before processing, a **full backfill** is required to restore consistency.
:::

#### 1. Required Permissions

Grant the `VIEW DATABASE STATE` permission (or, in newer versions of SQL Server, the `VIEW DATABASE PERFORMANCE STATE` permission) to the user so it can read CDC-related metadata for the database.
...
| Manage Capture Instance             | When enabled it automatically creates and manages CDC capture instances.                        | `enabled`                                                                                                | 
...
| Manage Capture Instance             | When enabled it automatically creates and manages CDC capture instances.                         | `enabled`                                                                                                | Boolean | 
