...
The OLake MySQL Source connector supports multiple sync modes and MariaDB flavor as a CDC source. It also offers features like parallel chunking, checkpointing, and automatic resume for failed full loads. This connector can be used within the OLake UI or run locally via Docker for open-source workflows.
