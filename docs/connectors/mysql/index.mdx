...
The OLake MySQL Source connector supports multiple synchronization modes: **Full Load**, **Incremental(Bookmark/cursor)**, and **Change Data Capture (CDC)** using MySQL binlogs. It offers features like parallel chunking, checkpointing, and automatic resume for failed full loads. This connector can be used within the OLake UI or run locally via Docker for open-source workflows.
...
<Tabs>
  ...
  :::info MySQL to Destination(iceberg or parquet) Data Type Mapping
  When syncing data from MySQL to Destination(iceberg or parquet), OLake converts source types to compatible Destination(iceberg or parquet) types. The table below summarizes the mapping used during writes:

  | MySQL Data Types | Destination(iceberg or parquet) Data Type |
  |---|---|
  ...
  <TabItem value="cli" label="Olake CLI">
    ...
    To configure the MySQL source locally, edit the existing or create a new `source.json` file. Below is an example configuration for connecting to a MySQL instance:
