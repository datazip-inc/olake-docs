...
services:
  ...
  primary_mysql:
    ...
    command:
      ...
      # Optional — enable GTID only if you need CDC timestamp microsecond precision.
      # enforce_gtid_consistency must be set before gtid-mode.
...
Enable GTID only if you need CDC timestamp microsecond precision. CDC works without it.
...
If enabled, both should return `ON`.
