...
Edit the cluster parameter group:
  ...
  - Set `binlog_row_metadata = FULL`. 
   ...
   - Optional (required only for CDC timestamp microsecond precision):
      - `enforce_gtid_consistency = ON` must be set before enabling GTID mode. MySQL requires GTID consistency enforcement before `gtid_mode` can be turned on.
      - `gtid_mode = ON`
...
Because `binlog_format` is static property for the cluster, reboot the Aurora cluster’s primary instance (writer). After reboot, the binary log will start recording transactions. Verify the settings on the Aurora MySQL instance:
   ...
   ```
   They should show `ROW`, `FULL`, `FULL`, and `ON` for `binlog_format`, `binlog_row_image`, `binlog_row_metadata`, and `log_bin` respectively. If you enabled GTID for microsecond timestamp precision, `enforce_gtid_consistency` and `gtid_mode` should both be `ON`.
...
- **GTID mode fails to enable:** If setting `gtid_mode = ON` fails with an error like `GTID_MODE = ON requires ENFORCE_GTID_CONSISTENCY = ON`, set `enforce_gtid_consistency = ON` in the cluster parameter group first, reboot the writer instance, then set `gtid_mode = ON`. Verify both with:
  ```sql
  SHOW VARIABLES LIKE 'enforce_gtid_consistency';
  SHOW VARIABLES LIKE 'gtid_mode';
  ```
