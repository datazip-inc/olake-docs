...
Edit the cluster parameter group:
   - Set `binlog_format = ROW`. 
      ...
      Row-based logging is required for CDC so that every data change is recorded as an event. (Aurora supports MIXED or STATEMENT formats too, but CDC tools rely on ROW format.)
   
   - Set `binlog_row_image = FULL`. 
   
      This ensures the full before-and-after images of rows are logged. (FULL is the safest for CDC tools, capturing all columns. Minimal or NOBLOB images might omit unchanged columns; some CDC tools can handle minimal images, but FULL simplifies data completeness.)
  ...
  - Set `binlog_row_metadata = FULL`. 
   ...
   - Ensure `log_bin` is effectively enabled. 
   
      Aurora doesn't have an explicit `log_bin` parameter (it manages binlog internally). Setting `binlog_format` to any value other than OFF essentially enables `log_bin`. If there is a parameter `binlog_format = OFF` (which is possible to turn off binlog), we definitely want it ON (ROW).

![mysql-aurora-pg-2](/img/docs/cdc/mysql/mysql-aurora-pg-2.png)

:::info
   - (Optional) If you plan to use Global Transaction Identifiers (GTIDs) for replication consistency, also set `gtid-mode = ON` and `enforce_gtid_consistency = ON`. This can help if you have failovers or want to use GTID position for CDC resume. Aurora MySQL v5.7/v8.0 do support GTID.
:::

   - Save the parameter group changes.


![mysql-aurora-pg-3](/img/docs/cdc/mysql/mysql-aurora-pg-3.png)


## Associate this new cluster parameter group with your Aurora cluster
In the RDS console, modify the Aurora cluster to use the new cluster parameter group. Also ensure the instance parameter group (if separate) does not override these settings (Aurora MySQL’s `binlog_format` is cluster-level).



### Reboot the Writer Instance:
Because `binlog_format` is static for the cluster, reboot the Aurora cluster’s primary instance (writer). After reboot, the binary log will start recording transactions. Verify the settings on the Aurora MySQL instance:
   ```sql
...
### Increase Binlog Retention Period:
...
It’s best not to use the master user for CDC. Create a dedicated user with replication privileges:
   ...
   - `SELECT ON *.*` is added to get snapshot data from tables or perform any selects (OLake will do an initial snapshot by default, hence read access is required).
...
{/* If you have **GTID enabled**, this makes it seamless for the connector to continue via GTID position.  */}
