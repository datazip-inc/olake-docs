...
It’s best not to use the master user for CDC. Create a dedicated user with replication privileges:
   ...
   - (optional) In Aurora (and MySQL in general), you might also consider `SHOW VIEW` if your tables include views and the CDC needs them, and possibly `LOCK TABLES` . For example, to be safe, you could do: 
     ```sql
...
5. **Verify Binary Log is Active:** On the Aurora cluster, run:
   ...
   If you are on v8.4, use `SHOW BINARY LOG STATUS;` instead.
