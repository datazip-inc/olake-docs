...
# Optional: Enable GTID mode (required for CDC timestamp microsecond precision)
...
# For MySQL 8.0+, include table metadata
...
| `binlog-row-image` | `FULL` | Include full row values in change events |
| `expire-logs-days` | `7` | Retain logs for 7 days |
| `log-slave-updates` | `1` | Enable if using read replicas |
| `binlog-row-metadata` | `FULL` | Include column metadata in change events |

**Optional — GTID mode (for CDC timestamp microsecond precision):**

CDC works without GTID. Set these only if you need microsecond timestamp precision:

| Parameter | Recommended Value | Purpose |
|-----------|-------------------|---------|
| `enforce_gtid_consistency` | `ON` | Enforce GTID consistency (must be set before `gtid_mode`) |
...
- **GTID mode fails to enable:**
  If MySQL fails to start or setting `gtid_mode = ON` returns an error like `GTID_MODE = ON requires ENFORCE_GTID_CONSISTENCY = ON`, add `enforce_gtid_consistency = ON` to `my.cnf` before `gtid_mode = ON`, then restart MySQL. Verify both with:
  ```sql
  SHOW VARIABLES LIKE 'enforce_gtid_consistency';
  SHOW VARIABLES LIKE 'gtid_mode';
  ```
