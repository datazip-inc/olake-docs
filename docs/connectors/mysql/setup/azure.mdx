...
| `binlog_row_image` | `FULL` | Include full row values in change events |
| `binlog_row_metadata` | `FULL` | Include column metadata in change events |
...
CDC works without GTID. Enable these only if you need microsecond timestamp precision:
...
- **GTID mode fails to enable:**
  If setting `gtid_mode = ON` fails with an error like `GTID_MODE = ON requires ENFORCE_GTID_CONSISTENCY = ON`, set `enforce_gtid_consistency = ON` in server parameters first, restart the server, then set `gtid_mode = ON`. Verify both with:
  ```sql
  SHOW VARIABLES LIKE 'enforce_gtid_consistency';
  SHOW VARIABLES LIKE 'gtid_mode';
  ```
