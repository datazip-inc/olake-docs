...
3. Add/modify the following flags:
   ...
   - (verify) `binlog_row_metadata` is `FULL`
   - (optional) `binlog_expire_logs_seconds = 604800` _(MySQL 8.0+)_ **or** `expire_logs_days = 7` _(MySQL 5.7)_

  :::info GTID Mode (Optional)
  Google Cloud SQL automatically enables GTID mode on all MySQL instances. This provides microsecond precision for CDC timestamps without requiring any configuration.
  
  How OLake Uses GTID:
  - OLake Go uses the microsecond-precision timestamp from the GTID event when available
  - If GTID is not available, OLake Go falls back to the seconds-precision timestamp from the binlog header
  - **Requirement:** MySQL 8.0.1 or higher for microsecond-precision timestamp support 
  :::
...
They should show `ON`, `ROW`, `FULL`, and `FULL` for `log_bin`, `binlog_format`, `binlog_row_image`, and `binlog_row_metadata` respectively. If you enabled GTID for microsecond timestamp precision, `enforce_gtid_consistency` and `gtid_mode` should both be `ON`.
...
- **MySQL fails to start with GTID error:**
  If MySQL crashes during startup with the error:
  ```
  [ERROR] [MY-010912] GTID_MODE = ON requires ENFORCE_GTID_CONSISTENCY = ON.
  [ERROR] [MY-010119] Aborting
  ```
  This means you tried to enable `gtid_mode = ON` without setting `enforce_gtid_consistency = ON` first. Set `enforce_gtid_consistency = ON` as a database flag first, restart the instance, then set `gtid_mode = ON`. Verify both with:
  ```sql
  SHOW VARIABLES LIKE 'enforce_gtid_consistency';
  SHOW VARIABLES LIKE 'gtid_mode';
  ```
