...
3. Add/modify the following flags:
   ...
   - (optional) `gtid_mode` to `ON` (required only for CDC timestamp microsecond precision)
...
- **GTID mode fails to enable:**
  If setting `gtid_mode = ON` fails with an error like `GTID_MODE = ON requires ENFORCE_GTID_CONSISTENCY = ON`, set `enforce_gtid_consistency = ON` as a database flag first, restart the instance, then set `gtid_mode = ON`. Verify both with:
  ```sql
  SHOW VARIABLES LIKE 'enforce_gtid_consistency';
  SHOW VARIABLES LIKE 'gtid_mode';
  ```
