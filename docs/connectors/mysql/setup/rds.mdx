...
4. **Set Binlog Retention Period:** Use the RDS stored procedure on the RDS MySQL instance to set retention:
   ...
   ```
   This example sets 24 hours retention. You can set up to 168 hours (7 days). The ideal value depends on how long you expect the CDC connector might be down or how much lag you can tolerate. Check with:
   ```sql
...
5. **Create a Replication User:** Similar to Aurora:
   ...
   FLUSH PRIVILEGES;
   ```
   We add `LOCK TABLES, RELOAD` in case a snapshot method requires table locks (though in RDS, global locks requiring SUPER are not allowed; 
...
- **CDC User Privileges:** If your CDC connector reports errors like *"The MySQL server replied with an error: ACCESS DENIED"*:
  ...
  - Ensure the connector isn’t trying to perform disallowed operations. For instance, some tools might try to flush logs or set variables – those would fail. 
...
- **Multi-AZ Failover:** If your RDS MySQL is Multi-AZ and a failover happens, the binlog file name might change (the replica that takes over will continue with perhaps a different file naming, but GTIDs would remain consistent if enabled). 
