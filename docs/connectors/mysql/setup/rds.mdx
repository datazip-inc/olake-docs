...
![mysql-rds-pg-1](/img/docs/cdc/mysql/mysql-rds-pg-1.webp)
   ...
   - Edit the parameters:
     ...
     - Optional (required only for CDC timestamp microsecond precision):
          - `gtid_mode = ON` 
          - Set `enforce_gtid_consistency = ON` must be set before enabling GTID mode. MySQL requires GTID consistency enforcement before `gtid_mode` can be turned on.
...
- **GTID mode fails to enable:** If setting `gtid_mode = ON` fails with an error like `GTID_MODE = ON requires ENFORCE_GTID_CONSISTENCY = ON`, set `enforce_gtid_consistency = ON` in the DB parameter group first, reboot the instance, then set `gtid_mode = ON`. Verify both with:
  ```sql
  SHOW VARIABLES LIKE 'enforce_gtid_consistency';
  SHOW VARIABLES LIKE 'gtid_mode';
  ```
