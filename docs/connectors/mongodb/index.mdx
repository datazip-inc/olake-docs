...
- If OLake is able to connect with Mongo DB `{"connectionStatus":{"status":"SUCCEEDED"},"type":"CONNECTION_STATUS"}` response is returned.
...
|-------|----------------|----------|
| Aug 27, 2025 | [v0.1.11](https://github.com/datazip-inc/olake/releases/tag/v0.1.11) | Performance testing added for all drivers, improved data loading by fixing filter-chunking logic, corrected default columns handling in both full_refresh and incremental modes, updated append mode docs, and adjusted Discover timeout configuration. |
---
