...
- **Multi-Format Support**: CSV files (`.csv`, `.csv.gz`), JSON files in JSONL and Array formats (`.json`, `.jsonl`, `.json.gz`), XML files (`.xml`) and Parquet files
...
When normalization is set to **OFF** in the Streams section, OLake Go parses the XML into a JSON tree and stores it as a JSON string in a column.
...
```json
{"orders":{"order":[{"id":"1001","status":"NEW","order_date":"2024-08-01","customer":{"id":"CUST_55","name":"Alice"}}]}}
```
...
| `items` | Table with column `data` and one row: Banana (Apple gets overwritten)  |
