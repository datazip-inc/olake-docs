...
:::note
If the **Row Identifier** is not present in the XML file, stream discovery will fail.
:::
...
|-------|-------------|---------------|
| S3 Bucket Name `required` | S3 bucket name where Iceberg table data and metadata files will be stored. | `my-data-warehouse` |
| AWS Region `required` | AWS Region where your S3 bucket exists. Required so OLake Go calls the correct regional S3 endpoint. | `us-east-1` |
