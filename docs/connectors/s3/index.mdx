...
| Field | Description | Example Value |
|-------|-------------|---------------|
| Bucket Name `required` | MinIO bucket name | `source-data` |
| Region `required` | Region (can be any value for MinIO) | `us-east-1` |
| Endpoint `required` | MinIO server endpoint URL | `http://minio:9000` |
| Access Key ID `required` | MinIO access key | `minioadmin` |
| Secret Access Key `required` | MinIO secret key | `minioadmin` |
| Path Prefix | Optional path prefix | `csv/` |
| File Format `required` | Format of files to sync | `csv` |
| Max Threads | Concurrent file processors | `5` |

:::tip MinIO Endpoint Format
Use the format `http://hostname:9000` or `https://hostname:9000`. Do not include bucket name in endpoint.
:::

</TabItem>

<TabItem value="localstack" label="LocalStack">

| Field | Description | Example Value |
|-------|-------------|---------------|
| Bucket Name `required` | LocalStack bucket name | `test-bucket` |
| Region `required` | AWS region (LocalStack default) | `us-east-1` |
| Endpoint `required` | LocalStack S3 endpoint | `http://localhost:4566` |
| Access Key ID `required` | LocalStack access key | `test` |
| Secret Access Key `required` | LocalStack secret key | `test` |
| File Format `required` | Format of files to sync | `json` |

:::info LocalStack Port
LocalStack S3 service runs on port 4566 by default. Adjust if using custom configuration.
:::
...
<TabItem value="minio-cli" label="MinIO">

<S3SourceConfigMinIO />

</TabItem>

<TabItem value="localstack-cli" label="LocalStack">
