...
| JDBC URL Parameters                                  | Extra JDBC URL parameters for fine-tuning the connection (left empty if none are needed). | `{"test-param": "test-value"}` |
| SSL Mode                                             | SSL configuration for the database connection. Contains details such as the SSL mode.     | `{"mode": "disable"}`          |
...
- In case of connection failure refer the Troubleshooting section.
...
|---|---|---|
|27 April 2025 | 0.1.11 |https://github.com/datazip-inc/olake/releases/tag/v0.1.11 |