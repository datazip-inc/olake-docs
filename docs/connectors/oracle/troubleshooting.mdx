...
**Solution**: To prevent this timeout, increase the idle connection duration by passing a `TIMEOUT` parameter (in seconds) under the `jdbc_url_params` section of the `config.json`. This ensures the Oracle driver maintains the session during long-running operations.
