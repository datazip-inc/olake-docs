...
| consumer_group_id   |   Unique identifier for the consumer group used to track message and coordinate with other consumer members. If not provided, OLake automatically generates one.                                                                                                                         | `example-consumer-group`                                                                                              | String  |
...
- If messages are null or empty, OLake Go will read the messages but **skip writing them to the destination**. For AVRO format, a **Confluence Schema Registry must be configured**—schema registry is mandatory only for AVRO data and if not provided OLake will consider the messages as JSON format.
:::
