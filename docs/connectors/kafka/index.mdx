...
| **messages** | The message payload is shown as stringified JSON. |
...
- **Normalization = `true`**: The JSON payload in the `value` column is Level 1 flattened, and each nested field becomes a separate column in the destination table. This makes querying individual fields easier.
