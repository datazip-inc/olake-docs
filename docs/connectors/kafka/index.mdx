...
| Topic Pattern | Regular expression to filter which Kafka topics are discovered during source setup. Use this to include only topics matching a naming pattern, or exclude specific topics from discovery. | `^orders-.*` - Only topics starting with `orders-` are discovered |
...
| topic_pattern | Regular expression to filter which Kafka topics are discovered during source setup. Use this to include only topics matching a naming pattern, or exclude specific topics from discovery. | `^orders-.*` - Only topics starting with `orders-` are discovered | STRING |
