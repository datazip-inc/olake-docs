...
{() => {
  ...
  const uiHeadings = new Set([
    '1-navigate-to-the-source-configuration-page',
    '2-provide-configuration-details',
    '3-test-connection'
  ]);

  const cliHeadings = new Set([
    '1-create-configuration-file',
    '2-provide-configuration-details-1',
  ...
  React.useEffect(() => {
    const patch = () => {
      ...
      tocLinks.forEach(link => {
        ...
        if (uiHeadings.has(hash)) {
          link.setAttribute('href', `?config-type=olake-ui#${hash}`);
        } else if (cliHeadings.has(hash)) {
          link.setAttribute('href', `?config-type=olake-cli#${hash}`);
        }
      });
    };
