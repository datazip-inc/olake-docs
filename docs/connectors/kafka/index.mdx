...
If a consumer group rebalance is triggered while a sync is in progress, the sync shuts down gracefully meaning that all messages read up to that point are written to the destination, and the next sync resumes from where it left off.

:::warning Consumer Group ID Requirements
- Each job must use a **unique consumer group ID**. 
- The consumer group configured for OLake Go must be **dedicated exclusively to OLake Go** and must not be shared with other consumers or applications.
:::
