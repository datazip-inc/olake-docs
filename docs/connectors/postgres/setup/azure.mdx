...
5. **Create Publication and Verify pgoutput:** Create a publication and a test pgoutput slot:
   ...
   ❌ **Plugin not found:** Verify `wal_level = logical`, PostgreSQL 10+, and server restart

   List existing slots: `SELECT * FROM pg_replication_slots;`

6. **Test Logical Decoding (Optional):** Create sample data and verify CDC functionality with pgoutput:
   ...
   -- Add table to publication and peek binary changes
   ...
   Expected output: binary changes representing the insert transaction.
