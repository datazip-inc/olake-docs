    ...
    const patch = () => {
      ...
      tocLinks.forEach(link => {
        ...
        if (uiHeadings.has(hash)) {
          link.setAttribute('href', `?config-type=ui#${hash}`);
        } else if (cliHeadings.has(hash)) {
          link.setAttribute('href', `?config-type=cli#${hash}`);
        }
      });
    };
...
<TabItem value="ui" label="Use OLake UI for Postgres" default>
...
<TabItem value="cli" label="Use OLake CLI for Postgres">
