...
The OLake DB2 Source connector supports multiple synchronization modes. It offers features like parallel chunking, checkpointing, and automatic resume for failed full loads. This connector can be used within the OLake UI or run locally via Docker for open-source workflows.
...
- DB2 Version 0.10.1.0 or higher.

### Connection Prerequisites

- Read access to the tables for the DB2 user.

### IBM DB2 CLI Driver Installation

:::info
This prerequisite is specific to OLake CLI (`.buildsh`) runs. For OLake UI and CLI (`docker run`), you do not need to worry about this prerequisite.
:::

When using `.buildsh` to run DB2 LUW, you need to ensure the IBM Data Server ODBC and CLI driver is installed on your machine.

**Steps to run DB2 LUW in your machine:**

1. Download the IBM Data Server ODBC and CLI driver as per your OS (links to be added):
   - [Linux (x86-64)](https://public.dhe.ibm.com/ibmdl/export/pub/software/data/db2/drivers/odbc_cli/linuxx64_odbc_cli.tar.gz)
   - [macOS (arm64)](https://public.dhe.ibm.com/ibmdl/export/pub/software/data/db2/drivers/odbc_cli/macarm64_odbc_cli.tar.gz)
   - [macOS (64-bit Intel/Apple Silicon)](https://public.dhe.ibm.com/ibmdl/export/pub/software/data/db2/drivers/odbc_cli/macos64_odbc_cli.tar.gz)
   - [Windows (64-bit)](https://public.dhe.ibm.com/ibmdl/export/pub/software/data/db2/drivers/odbc_cli/ntx64_odbc_cli.zip)

   :::tip
   We recommend using Linux (x86-64) as the preferred machine for working with DB2.
   :::

   :::danger
   IBM Data Server ODBC and CLI driver is **not supported** for Linux (arm64). Avoid using Linux (arm64) machines for DB2 workloads.
   :::

2. Extract the downloadable content from the above URLs. A folder named `clidriver` will be created in your directory.

3. Set the environment variables in your terminal, necessary to run DB2 LUW:
