...
- **OLake Go ingestion compute:** Azure `Standard D64ls v5` (**64 vCPUs, 128 GiB memory**).
...
- **Data preparation:** Generated TPCH data at 300 GB scale, loaded it into Azure PostgreSQL server, and ingested only the `lineitem` table into a GCP Iceberg bucket using OLake Go (full load).
...
The compaction job was configured with two trigger tiers:
