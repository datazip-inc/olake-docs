---
title: "Deploy OLake Maintenance on Kubernetes"
description: Deploy OLake Maintenance on Kubernetes with Helm. Enable Maintenance services, configure Spark optimizer (compactor) settings, and run compaction workflows for Iceberg tables.
sidebar_position: 2
---
...
# OLake Fusion Kubernetes Installation with Helm
...
:::info
To use Fusion, please ensure the following minimum versions are met:

1. If the installed OLake Helm chart is older than **`olake-0.0.15`** (or **`v0.3.4`**), upgrade to the **latest version**. See [**Upgrading Chart Version**](#upgrading-chart-version) for instructions.
2. If the installed OLake UI version is older than **`v0.4.0`**, upgrade to the **latest version**. See [**Updating OLake UI Version**](#updating-olake-ui-version) for instructions.
:::
...
:::note
Sync pod sizing depends on the data volume to be synced. Use [JobID-Based Scheduling](/docs/install/kubernetes#jobid-based-scheduling) to map jobs to nodes with sufficient capacity.
:::
...
With Fusion enabled, follow [Configure your first compaction](https://olake.io/docs/getting-started/configure-first-compaction) for step-by-step guidance on adding catalogs and scheduling maintenance on the Iceberg tables in the OLake UI.
...
Refer to the [Spark documentation](https://spark.apache.org/docs/latest/configuration.html) for a comprehensive list of configurable properties for Spark containers.

<details>
<summary><strong>Compaction Scheduling</strong></summary>
...
- Separate OLake, Temporal and Fusion databases created on the PostgreSQL instance
...
kubectl create secret generic external-postgres-secret \
  ...
  --from-literal=fusion_database="fusionDB" \
...
OLake Fusion sends anonymous usage telemetry to help improve the product. To opt out, set `TELEMETRY_DISABLED` under `global.env` (so it also reaches the Activity Pods running syncs):
...
{() => {
  ...
  const configurationHeadings = new Set([
    'configuration-options',
    'updating-olake-ui-version',
    'initial-user-setup',
    'ingress-configuration',
  ...
  React.useEffect(() => {
    const patch = () => {
      ...
      tocLinks.forEach(link => {
        ...
        if (installationHeadings.has(hash)) {
          link.setAttribute('href', `?setup-mode=installation#${hash}`);
        } else if (configurationHeadings.has(hash)) {
          link.setAttribute('href', `?setup-mode=configuration#${hash}`);
        }
      });
    };
