...
- This benchmark uses the `trips` and `fhv_trips` tables from the [NYC Taxi Data](https://github.com/datazip-inc/nyc-taxi-data-benchmark/tree/remote-postgres) dataset, totaling **4,008,587,913 rows**.  
- The average row size is **144 bytes** for `trips` and **121 bytes** for `fhv_trips`.  
- The tests were performed on an **Azure Standard D64ls v5 VM (64 vCPUs, 128 GiB memory)**.
