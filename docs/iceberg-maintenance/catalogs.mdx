...
A **catalog** is a entity from where OLake fetches Iceberg tables. It stores metadata such as table names, schemas, and file locations. OLake must know which catalog (and database) a table belongs to before optimization can run on it.
