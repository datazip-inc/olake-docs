...
| **Custom Glue Endpoint Configuration** | `true` | Enable custom Glue endpoint configuration. |
...
| **Glue Endpoint** | `https://glue.us-east-1.amazonaws.com` | Custom endpoint URL for AWS Glue or a Glue-compatible catalog service. |
| **Glue Region** | `us-east-1` | AWS region for the Glue catalog, if different from the S3 region. Falls back to AWS Region if not set. |