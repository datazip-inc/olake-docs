...
| **s3_endpoint**      | `http://<S3_ENDPOINT>:9000`   | S3 API endpoint for writing Iceberg data files. Optional for AWS S3 but required for S3-compatible storage like MinIO.                                                              |
| **aws_region**       | `<S3_REGION>`                 | AWS Region where your S3 bucket exists. Required so OLake Go calls the correct regional S3 endpoint for writing Iceberg data files.                             |
