...
{() => {
  ...
  const generalHeadings = new Set([
    ...
    '25-binlog-mysql',
    '26-oplog-mongodb',
    '27-table-format-eg-apache-iceberg-delta-lake-hudi',
    '28-schema-evolution',
    '29-acid-transactions',
    '30-avro',
    '31-orchestration-eg-airflow-luigi-dagster',
  ...
  React.useEffect(() => {
    const patch = () => {
      ...
      tocLinks.forEach(link => {
        ...
        if (olakeGoHeadings.has(hash)) {
          link.setAttribute('href', '?terminology-type=olake#' + hash);
        } else if (generalHeadings.has(hash)) {
          link.setAttribute('href', '?terminology-type=general#' + hash);
        }
      });
    };
