...
<TOCTabLinker
  queryParam="terminology-type"
  headingMap={{
    'olake': ['olake-go-terminologies', '1-source', '2-destination', '3-jobs', '4-olake-go-generated-columns', '1-operation-types-_op_type', '2-timestamps', '3-record-identification-_olake_id', '4-cdc-metadata-columns', 'mongodb', 'mysql', 'postgres', 'mssql'],
    'general': ['1-data-lake', '2-data-lakehouse', '3-data-warehouse', '4-snapshot', '5-polymorphic-or-heterogeneous-data', '6-chunking', '7-thread-countconcurrency', '8-writer', '9-parquet', '10-apache-iceberg', '11-equality-deletes-iceberg', '12-real-time-replication', '13-state-management', '14-catalog', '15-flattening-json-flattening', '16-performance-benchmarks', '17-monitoring--alerting', '18-byoc-bring-your-own-cloud', '19-query-engines-trino-spark-flink-snowflake', '20-grpc', '21-aws-s3', '22-etl-extract-transform-load', '23-elt-extract-load-transform', '24-wal-write-ahead-log', '25-binlog-mysql', '26-oplog-mongodb', '27-table-format-eg-apache-iceberg-delta-lake-hudi', '28-schema-evolution', '29-acid-transactions', '30-avro', '31-orchestration-eg-airflow-luigi-dagster', '32-batch-vs-streaming', '33-polyglot-persistence', '34-aggregation--analytical-query', '35-federation--federated-query', '36-ingestion-latency', '37-idempotency', '38-observability-logging-metrics-tracing', '39-change-data-capture', '40-full-refresh', '41-incremental', '42-position-deletes-iceberg', '43-small-files'],
  }}
/>

<Tabs queryString="terminology-type" groupId="terminologies">
<TabItem value="olake" label="OLake Go Terminologies">
...
### 2. Timestamps:
    - `_olake_timestamp` - 
        - Captures the exact time when OLake Go processed and wrote the record to the destination.
...
OLake Go also writes **driver-specific CDC ordering metadata columns**. These fields capture the exact log position and ordering information from each source system’s native CDC mechanism and is supported for **MongoDB**, **Postgres**, **MySQL** and **MSSQL** drivers.
...
JSON Flattening converts nested JSON fields into top-level columns for simpler and faster querying. OLake Go currently supports Level-1 JSON flattening through its ‘Normalization’ feature, converting top-level nested fields into separate columns for easier querying.
...
BYOC (Bring Your Own Cloud) is the approach of running software within the user’s chosen cloud or infrastructure instead of being tied to a vendor-managed environment. In OLake Go, this means the platform is cloud-agnostic, supporting deployments across AWS, GCP, Azure, or even on-premises setups, giving users flexibility without vendor lock-in.
...
### 44. Deletion Vectors (Iceberg):
Deletion Vectors are a row-level delete format introduced in Apache Iceberg v3 that tracks deleted rows using a bitmap (vector) of row positions. Unlike equality deletes or position deletes, deletion vectors store which rows are deleted as a compact bitmap structure, making them more efficient for both write and read operations. In OLake Go, deletion vectors can be used for upserts when writing to Iceberg v3 tables, providing faster delete performance and improved query speeds, especially when working with query engines that support this format.
