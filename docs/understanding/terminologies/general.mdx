...
JSON Flattening converts nested JSON fields into top-level columns for simpler and faster querying. OLake currently supports Level-0 JSON flattening through its ‘Normalization’ feature, converting top-level JSON objects into separate columns for easier querying.
