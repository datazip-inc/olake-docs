...
| `_cdc_timestamp` | **Source database change timestamp -** Actual date and time when the original change occurred in the source database. |
...
### 1. Operation Types (`_op_type`):
    ...
    - **Delete ('d') -**
        ...
        - When a record is deleted, primary key column and OLake-generated columns retain their values for tracking, while all other columns are set to `None`.
