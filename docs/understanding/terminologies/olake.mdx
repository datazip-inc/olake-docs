...
- Newly discovered tables default to Upsert when discovery completes and the next sync runs. To use Append (or vice versa), adjust the per-table Ingestion Method in the Streams section or apply a global selection before running.
