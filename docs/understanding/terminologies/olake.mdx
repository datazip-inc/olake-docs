...
<a id="source-naming-convention"></a>

- **Source Naming Convention:**
  - By default, this toggle is **disabled**, ensuring that column names are normalised to follow [destination naming conventions](/docs/understanding/terminologies/olake/#6-tablecolumn-normalization--destination-database-creation) before being written to the destination.
...
![Global Ingestion Method](/img/docs/terminologies/all_append_upsert.webp)
...
- When using Upsert mode, OLake Go offers three upsert type options for writing to Iceberg destinations:

  - **Equality Deletes**: Identifies rows to delete by matching primary key values (Iceberg V2).
  - **Positional Deletes**: Identifies rows to delete by file path and row position (Iceberg V2).
  - **Deletion Vectors**: Identifies rows to delete using a compact bitmap per data file (Iceberg V3).

  The default delete format is determined by your selected query engine and follows this order: **equality deletes**, **positional deletes**, then **deletion vectors**, selecting the first format your engine supports. Different streams can have different upsert type in a particular job. 
  
  See [Delete Format Support by Query Engine](/docs/understanding/compatibility-engines/#delete-format-support-by-query-engine) to check which formats different query engines support. The upsert functionality remains the same, only the format of delete files changes.
  ...
  :::warning
  Delete format options require every component below to meet these minimum version requirements. 
  - OLake Version **0.11.0 and above**.
  - OLake UI **0.6.0 and above**.
  - Helm chart **0.0.28 and above**.
  - OLake worker **0.3.11 and above**.

  For instance, if the OLake UI is updated but the OLake worker, Helm chart, or OLake Version is still on an older version, sync jobs might fail.
  :::

  :::info
  Positional deletes and deletion vector need persistent index storage. To learn how to set that up for your deployment, see:
  - **Kubernetes (Helm):** [Index Storage for Positional Delete/Deletion Vector Writes](/docs/install/kubernetes/#index-storage-for-positional-delete-writes)
  - **Docker Compose (OLake UI):** [Index DB Setup for Positional Delete/Deletion Vector Writes](/docs/install/olake-ui/#index-db-setup-for-positional-delete-writes)
  :::
...
- **Equality deletes:** How duplicates are avoided: A delete file is maintained that stores the `_olake_id` of the previous version.
...
- **Positional deletes:** How duplicates are avoided: A positional delete file records the data file path and row position of the previous version. OLake Go maintains an index that maps each `_olake_id` to where its current row lives in the table.
  - Before change (T1 ingested): destination has `id = 1` with amount = 100, status = "created" in a data file (for example, `.../data-00001.parquet` at row position 42).
  - On change (T2 ingested via CDC): the delete file marks that file path and row position as deleted; the new row for `id = 1` with amount = 120, status = "closed" is written to destination.
  - Result: destination retains only the latest row for `id = 1` (T2), so no duplicates.

- **Deletion vectors:** How duplicates are avoided: A binary bitmap (deletion vector) is maintained per data file, stored in a Puffin file, with one bit per row position. OLake Go maintains an index that maps each _olake_id to the data file and row position where its previous version lives, same as with positional deletes.
  - Before change (T1 ingested): destination has `id = 1` with amount = 100, status = "created" in a data file (for example, `.../data-00001.parquet` at row position 42).
  - On change (T2 ingested via CDC): the bit at position 42 in the deletion vector for `data-00001.parquet` is set to `1` (deleted); the new row for `id = 1` with amount = 120, status = "closed" is written to destination in a new data file.
  - Result: destination retains only the latest row for id = 1 (T2), so no duplicates. Readers skip position 42 by checking the bitmap instead of scanning multiple delete files.
...
![Ingestion Method](/img/docs/terminologies/ingest_section.webp)
