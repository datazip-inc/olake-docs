...
:::info CDC Filter Behavior
  - From **OLake connector v0.5.1** onward, data filter is now available for **CDC sync** as well, and filter that is configured during **Full Refresh** will be applied during subsequent **CDC** syncs.
  - Data filtering for CDC is available **only for jobs created on OLake connector v0.5.1 or later**. For jobs created on earlier versions, even if a data filter is configured for Full Refresh, it will not be applied during CDC, even if the OLake version is upgraded; new job must be created to use data filtering for CDC.
