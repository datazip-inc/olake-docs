...
- **DB2:** Similar to Postgres and MySQL, filters are applied within each chunk’s scan, guaranteeing only records satisfying conditions are ingested.
  ...
  - From **OLake v0.4.2** onward, data filter is now available for **CDC sync** as well, and filter that is configured during **Full Refresh** will be applied during subsequent **CDC** syncs.
  - CDC data filtering is available **only for jobs created on OLake v0.4.2 or later**. Jobs created on earlier versions will not have CDC data filters, even if the OLake version is upgraded; you must create a new job to use CDC data filtering.
