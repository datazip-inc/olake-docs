...
- **Column:** `_cdc_resume_token`
    - `_cdc_resume_token` stores the MongoDB Change Streams resume token that identifies the exact position in the oplog where this change event was captured. This token can be used to resume reading from the same point in the change stream.
    - **Example:** If a document with `_id = "507f1f77bcf86cd799439011"` is inserted, then updated twice in the same transaction, all three events will have the same `_cdc_timestamp` but different `_cdc_resume_token` values. You can use `_cdc_resume_token` to sort these events in the exact order they occurred in MongoDB's oplog.
...
- **Columns:** `_cdc_binlog_file_name`, `_cdc_binlog_file_pos`
    ...
    - **Example:** If a row with `id = 100` is inserted, then updated twice within the same transaction, all three events will share the same `_cdc_timestamp` (the commit time). However, `_cdc_binlog_file_name` and `_cdc_binlog_file_pos` will increment sequentially (for example, `mysql-bin.000003` at positions `1027`, `1150`, `1273`), allowing you to order the events exactly as they appeared in the binlog.
...
- **Column:** `_cdc_lsn`
    ...
    - **Example:** If a row with `id = 42` is inserted, then updated twice in the same transaction, all three events will have the same `_cdc_timestamp` but different `_cdc_lsn` values (for example, `16/B374D848`, `16/B374D850`, `16/B374D858`). Sorting by `_cdc_lsn` will reconstruct the exact order of operations as they occurred in Postgres.
...
- **Columns:** `_cdc_start_lsn`, `_cdc_seqval`
    ...
    - **Example:** If a row with `order_id = 500` is inserted, then updated twice within the same transaction, all three events will share the same `_cdc_timestamp` but have different `_cdc_start_lsn` and `_cdc_seqval` combinations. Sorting by `(_cdc_start_lsn, _cdc_seqval)` will order the events exactly as they were recorded in SQL Server's CDC change tables.

:::note Backward compatibility
For **jobs created before** this feature, these columns are always present for Parquet-on-S3 destinations, but for **Iceberg** they appear only when Normalization was turned **on** and will not be present for older jobs where Iceberg Normalization was **off**.
:::
