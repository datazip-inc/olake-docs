...
- When using Upsert mode, OLake Go offers three delete format (upsert type) options for writing to Iceberg destinations:
  ...
  The default delete format is determined by your selected query engine and follows this order: **equality deletes**, **positional deletes**, then **deletion vectors**, selecting the first format your engine supports. Different streams can have different delete format (upsert type) in a particular job. 
