...
### 8. Different types of modes to insert the data (Append / Upsert)

These modes control how records are written to the destination for CDC and Incremental syncs. They appear in the Streams panel after discovery.

::::info
- By default, all streams are set to **Upsert**.
- You can switch all streams to **Append** with the global selector `All Append`, or override per table using the stream’s **Ingestion Method**.
::::
...
- Recommended for transactional tables where you need the latest state (e.g., orders, customers, products).

#### Append
- Adds all incoming records to the destination without deduplicating. Every change event becomes a new row.
- Example: Using the same sequence for `id = 1`, the destination will contain both T1 and T2 as separate rows. This preserves the change history so you can analyze how the record evolved over time.
...
- Example: keep the global setting as **Upsert** for all tables, but set `logs` to **Append** to retain full history; transactional tables like `orders` can remain **Upsert** for latest state.
- You can set different modes for multiple tables as needed.
- UI indicator: When `All Upsert` or `All Append` is applied globally, that particular option gets highlighted. If you select a mix (e.g., some tables Upsert and some Append), a `Custom` state is highlighted to indicate a combination is in effect.

::::tip[Important to Know!]
- Newly discovered tables default to Upsert when you discover them and run the next sync. If you prefer Append (or vice versa), adjust in the Streams section via the per-table Ingestion Method or apply a global selection before running.
