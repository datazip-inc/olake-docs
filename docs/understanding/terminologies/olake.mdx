...
- When using Upsert mode, OLake Go offers two delete format options for writing to Iceberg destinations:
  ...
  Equality deletes are selected by default. Choose positional deletes for faster reads or if your query engine only supports positional deletes. The upsert functionality remains the same, only the format of delete files changes.
  ...
  For instance, if the OLake UI is updated but the OLake worker, Helm chart, or OLake Version is still on an older version, sync jobs using that delete format will fail.
  :::

  :::info
  Positional deletes need persistent index storage. To learn how to set that up for your deployment, see:
  - **Kubernetes (Helm):** [Index Storage for Positional Delete Writes](/docs/install/kubernetes/#index-storage-for-positional-delete-writes)
  - **Docker Compose (OLake UI):** [Index DB Setup for Positional Delete Writes](/docs/install/olake-ui/#index-db-setup-for-positional-delete-writes)
  :::
