...
OLake also writes **driver-specific CDC ordering metadata columns**. These fields capture the exact log position and ordering information from each source system’s native CDC mechanism and is supported for **MongoDB**, **Postgres**, **MySQL** and **MSSQL** drivers. These columns are written for both the supported destinations (i.e. **Iceberg** and **Parquet on S3**) in both Normalized and Non-Normalized modes.
...
- Derived from MongoDB Change Streams / oplog resume token, so each row carries the operation ordering and resume position used by OLake internally.

#### MySQL

- Columns: `_cdc_binlog_file_name`, `_cdc_binlog_file_pos`
- Captures the underlying binlog ordering information (binlog file name + position) that OLake used when reading each change event.

#### Postgres

- Column: `_cdc_lsn`
- Stores WAL-based ordering details (LSN) per record, reflecting the source transaction log position.

#### MSSQL

- Columns: `_cdc_start_lsn`, `_cdc_seqval`
- Exposes SQL Server CDC ordering metadata (start LSN and sequence value from change tables) alongside each record.

:::note Backward compatibility
- For **jobs created after** this feature was released, these CDC metadata columns are always present for supported drivers, regardless of whether Normalization is enabled or disabled.
- For **jobs created before** this feature, these columns appear only when Normalization was turned **on**, and will not be present for older jobs where Normalization remained **off**.
:::