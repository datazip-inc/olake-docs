...
### 2. Timestamps:
    ...
    - `_cdc_timestamp` - 
        ...
        - Whenever a full load is performed, snapshot records `(_op_type = 'r')` will have the `_cdc_timestamp` set to the epoch time **(1970-01-01)** indicating that it is not a CDC record.
...
### 3. Record Identification (`_olake_id`):
    ...
    - **Composite primary key present in the source table -** 
        ...
        - **Example:** If the composite primary key is `(order_id, product_id)` with values (456, 789), then `_olake_id` will be a hash(456, 789).
    - **With no primary key in the source table -** 
        ...
        - **Example:** If a record has columns `(name, age, city)` with values (Alice, 30, NY), then `_olake_id` will be hash(Alice, 30, NY).
