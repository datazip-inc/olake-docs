...
## OLake-Generated Variables

When OLake replicates data from source databases (like PostgreSQL, MySQL, or MongoDB) to destination formats (Apache Iceberg, Parquet), it automatically adds several metadata columns to track the lifecycle and processing history of each record. These columns help users understand how and when each row was captured and written to the destination.

OLake adds these metadata columns to every destination table:

| Column | Purpose & behaviours |
|--------|----------------------|
| `_op_type` | **Database operation type -** Identifies whether the row represents a snapshot read (r), new insert (c), record update (u), or record deletion (d) from the source database. |
| `_olake_id` | **OLake-assigned record identifier -** Unique sequential number assigned by OLake to track and order each record as it flows through the replication pipeline. |
| `_olake_timestamp` | **OLake processing timestamp -** Exact date and time when OLake finished processing this record and wrote it to the destination table. | 
| `_cdc_timestamp` | **Source database change timestamp -** Actual date and time when the original change occurred in the source database (shows epoch placeholder 1970-01-01 for initial snapshot records) |

### 1. Operation Types (`_op_type`):
    - **Read/Snapshot ('r') -** 
        - Appears during initial full table loads.
        - Represents the state of a record as it existed during the snapshot.
        - These records show the "baseline" data before CDC monitoring begins.
        - Example: When you first sync a table i.e. full load, all existing rows get `_op_type = 'r'`. <br/>
    - **Create/Insert ('c') -** 
        - Generated when new records are inserted into the source database.
        - Captured through Change Data Capture (CDC) after the initial snapshot.
        - Contains the full data of the newly inserted record.
    ...
    - **Update ('u') -**
        ...
        - Represents the new state of the record after the update.
        - Represents the new state of the record after the update, while keeping the old version for history and auditing
    ...
    - **Delete ('d') -**
        ...
        - When a record is deleted, key column and OLake-generated system columns retain their values for tracking, while all other columns are set to `None`.
        - The primary key fields remain to identify which record was deleted.
...
### 2. Timestamps:
    ...
    - `_cdc_timestamp` - 
        ...
        - Whenever a full load is performed, snapshot records `(_op_type = 'r')` will have the `_cdc_timestamp` set to the epoch time **(1970-01-01)** indicating that entire data has been transferred (i.e. no CDC).
...
### 3. Record Identification (`_olake_id`):
    - A unique sequential identifier assigned by OLake to each record as it is processed.
    - It be used for deduplication, ordering, and internal processing tracking.
