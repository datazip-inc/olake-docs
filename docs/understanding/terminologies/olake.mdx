...
- When using Upsert mode, OLake Go offers three delete format options for writing to Iceberg destinations:
  ...
  - **Equality Deletes** (default): Identifies rows to delete by matching primary key values.
  - **Positional Deletes**: Identifies rows to delete by file path and row position.
  - **Deletion Vectors**: Identifies rows to delete using a compact bitmap per data file (Iceberg V3).

  **Equality deletes** are selected by default. Choose **positional deletes** or **deletion vector** for faster reads or to match the delete format the downstream query engine supports. The upsert functionality remains the same, only the format of delete files changes.
  ...
  :::info
  Positional deletes need persistent index storage. To learn how to set that up for your deployment, see:
