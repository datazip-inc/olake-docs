...
- **Sync new columns automatically:**
  - By default, this feature is **enabled**, ensuring that any new columns added to the source are automatically selected and synced in the next sync.
...
- **Column Selection:**
  ...
  - **What happens during selection?**
    ...
    - If new columns are added to the source and selected, they will sync data in future syncs. Prior records in the destination will not contain data for these newly added columns.
    - If you want data for newly selected columns to be populated for **all existing rows** in the destination, you must use the **Clear Destination** function to clear all existing data in the destination and perform a **full load** again.

:::note Version Compatibility
Column selection and automatic sync of new columns features are available starting from **v0.3.18**. <br/> For **jobs created before** this version, you must update to v0.3.18 or later to use these features. For any **newly created jobs**, these features are automatically available.
:::
