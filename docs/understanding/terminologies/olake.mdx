...
- When using Upsert mode, OLake Go offers three delete format options for writing to Iceberg destinations:

  - **Equality Deletes** (default): Identifies rows to delete by matching primary key values (Iceberg V2).
  ...
  **Equality deletes** are selected by default. Choose **positional deletes** or **deletion vector** for faster reads or to match the delete format the downstream query engine supports. See [Delete Format Support by Query Engine](/docs/understanding/compatibility-engines/#delete-format-support-by-query-engine) to check which delete formats different query engine supports. The upsert functionality remains the same, only the format of delete files changes.
