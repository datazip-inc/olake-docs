...
:::info CDC/Incremental Filter Behavior
  - From **OLake connector v0.5.1** onward, data filter is now available for **CDC and Incremental sync** as well, and filter that is configured during **Full Refresh** will be applied during subsequent **CDC and Incremental** syncs.
  - Data filtering for CDC and Incremental is available **only for jobs created on OLake connector v0.5.1 or later**. For jobs created on earlier versions, even if a data filter is configured for Full Refresh, it will not be applied during CDC and Incremental, even if the OLake version is upgraded; new job must be created to use data filtering for CDC and Incremental.
